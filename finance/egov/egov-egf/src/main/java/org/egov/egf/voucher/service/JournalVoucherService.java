...
public class JournalVoucherService {
	...
	/**
	 * Existing JV creation entry point. Existing callers continue using the
	 * original workflow handling.
	 */
	@Transactional
	public CVoucherHeader create(final CVoucherHeader voucherHeader, final Long approvalPosition,
		...
		return createInternal(voucherHeader, approvalPosition, approvalComent, additionalRule, workFlowAction, null);
	}

	/**
	 * Refund-specific entry point. approvalPosition must already be resolved and
	 * validated through HRMS.
	 */
	@Transactional
	public CVoucherHeader createForRefund(final CVoucherHeader voucherHeader, final Long approvalPosition,
			final String approvalComent) {

		if (voucherHeader == null) {
			throw new IllegalArgumentException("Journal Voucher is mandatory");
		}

		if (voucherHeader.getId() != null || voucherHeader.getState() != null) {
			throw new IllegalArgumentException("Refund JV creation requires a new voucher");
		}

		if (!FinancialConstants.JOURNALVOUCHER_NAME_GENERAL.equalsIgnoreCase(voucherHeader.getName())) {
			throw new IllegalArgumentException("Refund JV creation supports only General Journal");
		}

		if (approvalPosition == null || approvalPosition <= 0) {
			throw new IllegalArgumentException("Valid HRMS approver position is mandatory");
		}

		// ID carrier only; do not persist this as a local position record.
		final Position hrmsPosition = new Position();
		hrmsPosition.setId(approvalPosition);

		return createInternal(voucherHeader, approvalPosition, approvalComent, null, FinancialConstants.BUTTONFORWARD,
				hrmsPosition);
	}

	/**
	 * Shared voucher persistence and numbering logic. Called within the transaction
	 * started by either public entry point.
	 */
	private CVoucherHeader createInternal(final CVoucherHeader voucherHeader, final Long approvalPosition,
			final String approvalComent, final String additionalRule, final String workFlowAction,
			final Position refundHrmsPosition) {
		...
		voucherHeader.getVouchermis().setSourcePath("/services/EGF/voucher/journalVoucherModify-beforeModify.action"
				+ "?voucherHeader.id=" + voucherHeader.getId());
		...
		if (workFlowAction.equals(FinancialConstants.CREATEANDAPPROVE)) {
			...
			if (refundHrmsPosition == null) {
				// Existing JV flow: retain the original workflow method.
				createVoucherHeaderRegisterWorkflowTransition(savedVoucherHeader, approvalPosition, approvalComent,
						additionalRule, workFlowAction);
			} else {
				// New refund flow only: use the validated HRMS position.
				startRefundJournalVoucherWorkflow(savedVoucherHeader, refundHrmsPosition, approvalComent,
						additionalRule);
			}
		}
	...
	/**
	 * Starts the refund-generated JV workflow using its HRMS owner. Does not change
	 * workflow handling for existing JV callers.
	 */
	private void startRefundJournalVoucherWorkflow(final CVoucherHeader voucherHeader, final Position hrmsPosition,
			final String approvalComment, final String additionalRule) {

		if (voucherHeader.getState() != null) {
			throw new IllegalArgumentException("Refund Journal Voucher workflow already exists");
		}

		if (hrmsPosition == null || hrmsPosition.getId() == null || hrmsPosition.getId() <= 0) {
			throw new IllegalArgumentException("Valid HRMS workflow owner is mandatory");
		}

		final User user = securityUtils.getCurrentUser();

		if (user == null || user.getId() == null || user.getId() <= 0) {
			throw new IllegalArgumentException("Authenticated user is required to create the JV workflow");
		}

		final WorkFlowMatrix workflowMatrix = cvoucherHeaderRegisterWorkflowService
				.getWfMatrix(voucherHeader.getStateType(), null, null, additionalRule, "", null);

		if (workflowMatrix == null || StringUtils.isBlank(workflowMatrix.getNextState())) {
			throw new IllegalArgumentException("Initial Journal Voucher workflow matrix is not configured");
		}

		voucherHeader.transition().start().withSenderName(user.getUsername() + "::" + user.getName())
				.withComments(approvalComment == null ? "" : approvalComment.trim())
				.withStateValue(workflowMatrix.getNextState()).withDateInfo(new Date()).withOwner(hrmsPosition)
				.withNextAction(workflowMatrix.getNextAction())
				.withNatureOfTask(FinancialConstants.WORKFLOWTYPE_VOUCHER_DISPLAYNAME).withCreatedBy(user.getId())
	...
	public void createVoucherHeaderRegisterWorkflowTransition(final CVoucherHeader voucherHeader,
		...
		if (FinancialConstants.BUTTONREJECT.toString().equalsIgnoreCase(workFlowAction)) {
			...
			voucherHeader.transition().progressWithStateCopy()
					...
					.withNextAction("").withNatureOfTask(FinancialConstants.WORKFLOWTYPE_VOUCHER_DISPLAYNAME);
		} else {
			if (null != approvalPosition && approvalPosition != -1 && !approvalPosition.equals(Long.valueOf(0)))
				pos = positionMasterService.getPositionById(approvalPosition);
			WorkFlowMatrix wfmatrix;
			if (null == voucherHeader.getState()) {
				wfmatrix = cvoucherHeaderRegisterWorkflowService.getWfMatrix(voucherHeader.getStateType(), null, null,
				...
				voucherHeader.setStatus(FinancialConstants.CREATEDVOUCHERSTATUS);
				wfmatrix = cvoucherHeaderRegisterWorkflowService.getWfMatrix(voucherHeader.getStateType(), null, null,
						additionalRule, voucherHeader.getCurrentState().getValue(), null);
				voucherHeader.transition().progressWithStateCopy()
						.withSenderName(user.getUsername() + "::" + user.getName()).withComments(approvalComent)
	...
	public Long getApprovalPositionByMatrixDesignation(final CVoucherHeader voucherHeader, final String additionalRule,
		...
		final WorkFlowMatrix wfmatrix = cvoucherHeaderRegisterWorkflowService.getWfMatrix(voucherHeader.getStateType(),
				null, null, additionalRule, voucherHeader.getCurrentState().getValue(), null);
