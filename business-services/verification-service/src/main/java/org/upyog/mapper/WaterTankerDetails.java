...
public class WaterTankerDetails implements CommonDetailsMapper {
    ...
    public CommonDetails mapJsonToCommonDetails(JsonNode json) {
        ...
        String deliveryTime = wtDetailNode.path("deliveryTime").asText(NA);
        ...
        return CommonDetails.builder().applicationNumber(applicationNumber).fromDate(deliveryDate).toDate(deliveryTime).address(fullAddress.isEmpty() ? NA : fullAddress).name(name).mobileNumber(mobileNumber).status(status).moduleName(moduleName)
                .build();
    }
}
