...
import org.upyog.dashboard.model.OAuthTokenResponse;
...
public class OAuthTokenService {
    ...
    private void fetchNewToken() {
        log.info("Fetching new OAuth token for system user [{}] tenant [{}]", dashboardProperties.getUsername(), dashboardProperties.getTenantId());

        java.util.Map<String, String> body = java.util.Map.of(
                "username", dashboardProperties.getUsername(),
                "password", dashboardProperties.getPassword(),
                "tenantId", dashboardProperties.getTenantId(),
                "userType", dashboardProperties.getUserType(),
                "scope", "read",
                "grant_type", "password"
        );
        String url = dashboardProperties.getOauthHost() + dashboardProperties.getOauthPath();
        ...
        while (true) {
            ...
            try {
                OAuthTokenResponse response = userFeignClient.fetchToken(
                        ...
                        dashboardProperties.getBasicAuthHeader(),
                        body
                );
                ...
                if (attempt >= dashboardProperties.getOauthMaxAttempts()) {
                ...
                long backoff = RetryUtil.calculateBackoffWithJitter(attempt, dashboardProperties.getOauthBaseDelayMs(), dashboardProperties.getOauthMaxDelayMs());
    ...
    private void fetchUserInfo() {
        log.info("Fetching userInfo for system user [{}] via user search endpoint", dashboardProperties.getUsername());
        ...
        while (true) {
            ...
            try {
                ...
                if (body != null && body.getUser() != null && !body.getUser().isEmpty()) {
                    ...
                    log.info("Fetched userInfo for [{}]: uuid={}", dashboardProperties.getUsername(), cachedUserInfo.getUuid());
                    return;
                } else {
                    log.error("User search at {} returned no user for username [{}] tenant [{}]. "
                            + "Confirm the correct endpoint/payload shape with the team.", url, dashboardProperties.getUsername(), dashboardProperties.getTenantId());
                    return;
                }
            } catch (Exception exception) {
                if (exception instanceof feign.FeignException feignEx && feignEx.status() >= 400 && feignEx.status() < 500) {
