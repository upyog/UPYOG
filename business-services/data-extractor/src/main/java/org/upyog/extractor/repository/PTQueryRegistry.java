...
/**
 * Registry repository containing optimized PostgreSQL query templates for Property Tax (PT) 
 * dashboard metric extraction.
 * 
 * <p>Queries are written for {@link org.springframework.jdbc.core.namedparam.NamedParameterJdbcTemplate} 
 * using named bind parameters:
 * <ul>
 *   <li>{@code :startTime} — Start of target date range in epoch milliseconds</li>
 *   <li>{@code :endTime} — End of target date range in epoch milliseconds</li>
 *   <li>{@code :tenantId} — The target ULB tenant ID (e.g. {@code "pg.citya"})</li>
 * </ul>
 */
public class PTQueryRegistry {

	/**
	 * Combined query to extract all single-value scalar metrics in one database round-trip.
	 */
	...
	/**
	 * Combined query to retrieve raw daily payments, transaction details, and 
	 * apportioned tax account detail amounts.
	 */
	public static final String COLLECTION_METRICS_QUERY = "SELECT " +
			"  CASE " +
	...
	/**
	 * Query to retrieve cumulative counts of registered properties grouped by financial year.
	 */
	public static final String PROPERTIES_REGISTERED_QUERY = "SELECT " +
			"  CASE " +
			...
			"  COUNT(*) AS value " +
			"FROM eg_pt_property " +
			"WHERE creationreason = 'CREATE' AND tenantid = :tenantId " +
			"GROUP BY name " +
			"ORDER BY name";

	/**
	 * Query to count properties assessed today, grouped by mapped usage category.
	 */
	public static final String ASSESSED_PROPERTIES_QUERY = "SELECT " +
			"  CASE " +
			"    WHEN UPPER(p.usagecategory) LIKE 'RESIDENTIAL%' THEN 'RESIDENTIAL' " +
			"    WHEN UPPER(p.usagecategory) LIKE '%COMMERCIAL%' THEN 'COMMERCIAL' " +
			"    WHEN UPPER(p.usagecategory) LIKE '%INDUSTRIAL%' THEN 'INDUSTRIAL' " +
	...
	/**
	 * Query to retrieve counts of workflow application transitions made today, grouped by action name.
	 */
	public static final String MOVED_APPLICATIONS_QUERY = "SELECT " +
			"  action AS name, " +
			"  COUNT(*) AS value " +
