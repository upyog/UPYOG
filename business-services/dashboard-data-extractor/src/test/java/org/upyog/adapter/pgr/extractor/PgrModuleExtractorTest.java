...
import org.upyog.adapter.extractor.impl.PgrModuleExtractor;
...
class PgrModuleExtractorTest {
	...
	void setUp() throws Exception {
		...
		// Mock and inject AdapterProperties
		AdapterProperties adapterProperties = mock(AdapterProperties.class);
		when(adapterProperties.getMetricUlb()).thenReturn("pg.citya");
		when(adapterProperties.getMetricWard()).thenReturn("Ward 1");
		when(adapterProperties.getMetricRegion()).thenReturn("City A");
		when(adapterProperties.getMetricState()).thenReturn("PG");
		when(adapterProperties.getDbMaxAttempts()).thenReturn(3);
		when(adapterProperties.getDbBaseDelayMs()).thenReturn(1L);
		when(adapterProperties.getDbMaxDelayMs()).thenReturn(2L);

		java.lang.reflect.Field propsField = PgrModuleExtractor.class.getDeclaredField("adapterProperties");
		propsField.setAccessible(true);
		propsField.set(extractor, adapterProperties);
