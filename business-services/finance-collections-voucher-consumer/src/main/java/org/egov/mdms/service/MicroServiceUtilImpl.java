...
public class MicroServiceUtilImpl implements MicroServiceUtil {
	...
	private static final String REFUND_PAYMENT_MAPPING_MASTER = "RefundPaymentMapping";
	@Autowired
		...
		try {
			if (finSerMdms.getFinanceServiceMdmsData() != null) {
				list = mapper.convertValue(
						JsonPath.read(finSerMdms.getFinanceServiceMdmsData(),
								"$.MdmsRes.FinanceModule.BusinessServiceMapping"),
						new TypeReference<List<BusinessService>>() {
						});
			}
		} catch (Exception e) {
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Error while parsing mdms data. Check the business/account head mapping json file.");
		}
		return list;
	}
	...
	public List<TaxHeadMaster> getTaxHeadMasters(String tenantId, String code, RequestInfo requestInfo,
		...
		try {
			if (finSerMdms.getFinanceServiceMdmsData() != null) {
				list = mapper.convertValue(
						JsonPath.read(finSerMdms.getFinanceServiceMdmsData(),
								"$.MdmsRes.FinanceModule.TaxHeadMasterGlCodeMapping"),
						new TypeReference<List<TaxHeadMaster>>() {
						});
			}
		} catch (Exception e) {
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Error while parsing mdms data for TaxHeadMasterGlCodeMapping master. Check the business/account head mapping json file.");
		}
		return list;
	}
	 ...
	 * @return
	 * @throws VoucherCustomException Function which is used to fetch the finance
	 *                                service mdms data based on Business Service
	 *                                code.
	 */
	public void getFinanceServiceMdmsData(String tenantId, String businessServiceCode, RequestInfo requestInfo,
			FinanceMdmsModel finSerMdms) throws VoucherCustomException {
		...
		try {
			Map postForObject = mapper
					.convertValue(serviceRequestRepository.fetchResult(mdmsUrl, mdmsrequest, tenantId), Map.class);
			finSerMdms.setFinanceServiceMdmsData(postForObject);
		} catch (ServiceCallException e) {
			LOGGER.error("MDMS call failed for tenant: {}", tenantId, e);
			throw new VoucherCustomException(ProcessStatus.FAILED, "MDMS service call failed for URL: " + mdmsUrl);
		} catch (Exception e) {
	...
	private void addFinanceModule(List<ModuleDetail> moduleDetails, String businessServiceCodes) {
		...
		masterDetailsList.add(new MasterDetail(BUSINESS_SERVICE_MAPP_MASTER,
				businessServiceCodes != null ? "[?(" + prepareFilter(businessServiceCodes, "code") + ")]" : null));
		masterDetailsList.add(new MasterDetail(TAX_HEAD_MAPP_MASTER,
				businessServiceCodes != null ? "[?(" + prepareFilter(businessServiceCodes, "billingservicecode") + ")]"
						: null));
		...
		masterDetailsList.add(new MasterDetail(ONLINE_GLCODE_MAPP_MASTER,
				businessServiceCodes != null ? "[?(" + prepareFilter(businessServiceCodes, "servicecode") + ")]"
						: null));
	...
	private void addBillingServiceModule(List<ModuleDetail> moduleDetails, String businessServiceCode) {
		ArrayList<MasterDetail> masterDetailsList = new ArrayList<>();
		masterDetailsList.add(new MasterDetail(BUSINESS_SERVICE_MASTER,
				businessServiceCode != null ? "[?(@.code=='" + businessServiceCode + "')]" : null));
	...
	@Override
	public String getBusinessServiceName(String tenantId, String code, RequestInfo requestInfo,
			FinanceMdmsModel finSerMdms) throws VoucherCustomException {
		if (finSerMdms.getFinanceServiceMdmsData() == null) {
			this.getFinanceServiceMdmsData(tenantId, code, requestInfo, finSerMdms);
		}
		...
		try {
			...
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Error while parsing mdms data for BusinessService master. Check the business/account head mapping json file.");
		}
		return !list.isEmpty() ? list.get(0).getBusinessService() : code;
	}

	@Override
	public String getGlcodeByInstrumentType(String tenantId, String businessCode, RequestInfo requestInfo,
		...
		try {
			if (finSerMdms.getFinanceServiceMdmsData() != null) {
				if (ONLINE_INSTRUMENT_TYPE.equals(instrumentType)) {
					...
					List<OnlineGLCodeMapping> list = mapper.convertValue(
							JsonPath.read(finSerMdms.getFinanceServiceMdmsData(),
									"$.MdmsRes.FinanceModule.OnlineGLCodeMapping"),
							new TypeReference<List<OnlineGLCodeMapping>>() {
							});
					List<OnlineGLCodeMapping> collect = list.stream()
							.filter(inst -> inst.getServicecode().equals(businessCode)).collect(Collectors.toList());
					return !collect.isEmpty() ? collect.get(0).getGlcode() : null;
				} else {
					List<InstrumentGlCodeMapping> list = mapper.convertValue(
							JsonPath.read(finSerMdms.getFinanceServiceMdmsData(),
									"$.MdmsRes.FinanceModule.InstrumentGLcodeMapping"),
							new TypeReference<List<InstrumentGlCodeMapping>>() {
							});
					List<InstrumentGlCodeMapping> collect = list.stream()
							.filter(inst -> inst.getInstrumenttype().equalsIgnoreCase(instrumentType))
			...
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Error while parsing mdms data for InstrumentGLcodeMapping/OnlineGLCodeMapping master. Check the business/account head mapping json file.");
		}
		return null;
	}

	@Override
	public FinancialStatus getFinancialStatusByCode(String tenantId, RequestInfo requestInfo,
			FinanceMdmsModel finSerMdms, String statusCode) throws VoucherCustomException {
		if (finSerMdms.getFinanceServiceMdmsData() == null) {
		...
		try {
			if (finSerMdms.getFinanceServiceMdmsData() != null) {
				Object raw = JsonPath.read(finSerMdms.getFinanceServiceMdmsData(),
						"$.MdmsRes.FinanceModule.FinanceInstrumentStatusMapping");
				LOGGER.info("RAW TYPE: {}", raw.getClass());
				LOGGER.info("RAW DATA: {}", raw);
				list = mapper.convertValue(
						JsonPath.read(finSerMdms.getFinanceServiceMdmsData(),
								"$.MdmsRes.FinanceModule.FinanceInstrumentStatusMapping"),
						new TypeReference<List<FinancialStatus>>() {
						});
				list.forEach(fs -> LOGGER.info("Parsed FinancialStatus → code: [{}], object: {}", fs.getCode(), fs));
			}
		} catch (Exception e) {
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Error while parsing mdms data for EgfInstrumentStatusMapping master. Check the EgfInstrumentStatusMapping.json file.");
		}
		LOGGER.info("Incoming statusCode: {}", statusCode);
		List<FinancialStatus> collect = list.stream()
				.filter(fs -> fs.getCode() != null && fs.getCode().equals(statusCode)).collect(Collectors.toList());
		return !collect.isEmpty() ? collect.get(0) : null;
	}

	@Override
	public List<Tenant> getFinanceTenantList(String tenantId, String businessCode, RequestInfo requestInfo,
			FinanceMdmsModel finSerMdms) throws VoucherCustomException {
		if (finSerMdms.getFinanceServiceMdmsData() == null) {
			this.getFinanceServiceMdmsData(tenantId, businessCode, requestInfo, finSerMdms);
		}
		...
		try {
			if (finSerMdms.getFinanceServiceMdmsData() != null) {
				list = mapper.convertValue(
						JsonPath.read(finSerMdms.getFinanceServiceMdmsData(), "$.MdmsRes.tenant.citymodule[0].tenants"),
						new TypeReference<List<Tenant>>() {
						});
			}
		} catch (Exception e) {
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Error while parsing mdms data for CityModule master. Check the CityModule.json file.");
		}
		return list;
	}

	@Override
	public List<InstrumentContract> getInstruments(InstrumentSearchContract instrumentSearchContract,
			RequestInfo requestInfo, String tenantId) throws VoucherCustomException {
		...
		InstrumentResponse instrumentResponse = mapper.convertValue(serviceRequestRepository.fetchResult(uri,
				RequestInfoWrapper.builder().requestInfo(requestInfo).build(), tenantId), InstrumentResponse.class);
		return instrumentResponse != null ? instrumentResponse.getInstruments() : null;
	}

	@Override
	public RefundPaymentMapping getRefundPaymentMapping(final String tenantId, final String businessService,
			final String refundMode, final RequestInfo requestInfo) throws VoucherCustomException {

		if (tenantId == null || tenantId.trim().isEmpty()) {
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Tenant ID is mandatory for refund payment MDMS lookup");
		}

		if (businessService == null || businessService.trim().isEmpty()) {
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Business service is mandatory for refund payment MDMS lookup");
		}

		if (requestInfo == null) {
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"RequestInfo is mandatory for refund payment MDMS lookup");
		}

		final StringBuilder mdmsUrl = new StringBuilder(manager.getMdmsHostUrl()).append(manager.getMdmsSearchUrl());

		final List<MasterDetail> masterDetails = new ArrayList<>();
		masterDetails.add(new MasterDetail(REFUND_PAYMENT_MAPPING_MASTER, null));

		final List<ModuleDetail> moduleDetails = new ArrayList<>();
		...
		try {
			final Object rawResponse = serviceRequestRepository.fetchResult(mdmsUrl, mdmsrequest, tenantId);

			final Map<String, Object> mdmsResponse = mapper.convertValue(rawResponse,
					new TypeReference<Map<String, Object>>() {
					});

			final List<RefundPaymentMapping> mappings = mapper.convertValue(
					JsonPath.read(mdmsResponse, "$.MdmsRes.FinanceModule." + REFUND_PAYMENT_MAPPING_MASTER),
					new TypeReference<List<RefundPaymentMapping>>() {
					});

			final List<RefundPaymentMapping> matchingMappings = mappings.stream()
					.filter(mapping -> Boolean.TRUE.equals(mapping.getIsActive()))
					.filter(mapping -> mapping.getTenantId() != null
							&& mapping.getTenantId().equalsIgnoreCase(tenantId))
					.filter(mapping -> mapping.getBusinessService() != null
							&& mapping.getBusinessService().equalsIgnoreCase(businessService))
					.filter(mapping -> refundMode == null || refundMode.trim().isEmpty()
							|| (mapping.getRefundMode() != null
									&& mapping.getRefundMode().equalsIgnoreCase(refundMode)))
					.collect(Collectors.toList());

			if (matchingMappings.isEmpty()) {
				throw new VoucherCustomException(ProcessStatus.FAILED,
						"Active RefundPaymentMapping is not configured " + "for businessService " + businessService
								+ " and refundMode " + refundMode + " under tenant " + tenantId);
			}

			if (matchingMappings.size() > 1) {
				throw new VoucherCustomException(ProcessStatus.FAILED,
						"Multiple active RefundPaymentMapping records " + "found for businessService " + businessService
								+ " and refundMode " + refundMode + " under tenant " + tenantId);
			}

			final RefundPaymentMapping mapping = matchingMappings.get(0);

			if (mapping.getDebitGlCode() == null || mapping.getDebitGlCode().trim().isEmpty()) {

				throw new VoucherCustomException(ProcessStatus.FAILED,
						"Debit GL code is missing in RefundPaymentMapping");
			}

			if (mapping.getPayableGlCode() == null || mapping.getPayableGlCode().trim().isEmpty()) {
			...
			LOGGER.error("Error while fetching RefundPaymentMapping " + "for tenant: {}, businessService: {}, "
					+ "refundMode: {}", tenantId, businessService, refundMode, exception);

			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Error while fetching or parsing " + "RefundPaymentMapping from MDMS");
		}
	}
}
