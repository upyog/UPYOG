...
public class MicroServiceUtilImpl implements MicroServiceUtil {
	...
	public FinancialStatus getFinancialStatusByCode(String tenantId, RequestInfo requestInfo,
		...
		List<FinancialStatus> collect = list.stream().filter(fs -> fs.getCode().equals(statusCode))
				.collect(Collectors.toList());
	...
	public Map<String, String> getGlcodeByTaxHead(String tenantId, String businessServiceCode, RequestInfo requestInfo,
		...
		if (finSerMdms.getFinanceServiceMdmsData() == null) {
			this.getFinanceServiceMdmsData(tenantId, businessServiceCode, requestInfo, finSerMdms);
		}

		try {
			if (finSerMdms.getFinanceServiceMdmsData() != null) {
				List<TaxHeadMaster> taxHeadList = mapper.convertValue(
						JsonPath.read(finSerMdms.getFinanceServiceMdmsData(),
								"$.MdmsRes.FinanceModule.TaxHeadMasterGlCodeMapping"),
						new TypeReference<List<TaxHeadMaster>>() {
						});
				return taxHeadList.stream()
						.collect(Collectors.toMap(TaxHeadMaster::getTaxhead, TaxHeadMaster::getGlcode, (v1, v2) -> v1));
			}
		} catch (Exception e) {
			throw new VoucherCustomException(ProcessStatus.FAILED,
					"Error while parsing MDMS data for TaxHeadMasterGlCodeMapping. Check the json file.");
		}
