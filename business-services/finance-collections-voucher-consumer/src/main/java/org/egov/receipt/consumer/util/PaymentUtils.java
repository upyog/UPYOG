...
public class PaymentUtils {
	 ...
	 * @param payments
	 * @param receipts
	 * @throws VoucherCustomException Method which is used to transform the Payment
	 *                                payload to Receipt payload for collection
	 *                                version-2
	 */
	...
	/**
	 * 
	 * @param receipts Method which is used to aggregate the paymentdetils based on
	 *                 the service. it will consolidate the billaccount_details in a
	 *                 single receipt based on business service
	 */
	private void aggregateReceiptBasedOnService(List<Receipt> receipts) {
		...
		businessRecptMap.entrySet().forEach(entrySet -> {
			...
			BigDecimal totalAmountPaid = entrySet.getValue().stream().map(Receipt::getBill).flatMap(List::stream)
					.map(Bill::getBillDetails).flatMap(List::stream).map(BillDetail::getAmountPaid)
					.reduce((x, y) -> x.add(y)).orElse(BigDecimal.ZERO);

			List<BillAccountDetailV2> billAccDetailList = entrySet.getValue().stream().map(Receipt::getBill)
					.flatMap(List::stream).map(Bill::getBillDetails).flatMap(List::stream)
					.map(BillDetail::getBillAccountDetails).flatMap(List::stream).collect(Collectors.toList());
	...
	private void aggregateBillAccounDetails(List<BillAccountDetailV2> billAccDetailList) {
		Map<String, List<BillAccountDetailV2>> taxHeadBillAccDetailsMap = new HashMap<>();
		for (BillAccountDetailV2 bad : billAccDetailList) {
			if (taxHeadBillAccDetailsMap.containsKey(bad.getTaxHeadCode())) {
				taxHeadBillAccDetailsMap.get(bad.getTaxHeadCode()).add(bad);
			} else {
				List<BillAccountDetailV2> list = new ArrayList<>();
				list.add(bad);
				taxHeadBillAccDetailsMap.put(bad.getTaxHeadCode(), list);
			}
		}
		billAccDetailList.clear();
		for (Entry<String, List<BillAccountDetailV2>> set : taxHeadBillAccDetailsMap.entrySet()) {
			List<BillAccountDetailV2> value = set.getValue();
			BillAccountDetailV2 bad = value.get(0);
			BigDecimal totalAmount = value.stream().map(BillAccountDetailV2::getAmount)
					.reduce((prevAmount, currAmount) -> prevAmount.add(currAmount)).orElse(BigDecimal.ZERO);
			BigDecimal adjustedAmount = value.stream().map(BillAccountDetailV2::getAdjustedAmount)
					.reduce((prevAmount, currAmount) -> prevAmount.add(currAmount)).orElse(BigDecimal.ZERO);
...
//    	bdv1.getBillAccountDetails().stream().
	...
	private void populateBankDetails(Instrument instrument, String ifscCode) throws VoucherCustomException {
		...
		BankContract bank = BankContract.builder().name(bankDetails.getBankName()).code(bankDetails.getBankCode())
				.build();
