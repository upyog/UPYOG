...
@Slf4j
@Component
public class WMSWorkStatusReportApplicationEnrichment {
...
//	    private UserUtil userUtils;
	...
	public void enrichWorkStatusReportApplication(WMSWorkStatusReportRequest wmsWorkStatusReportRequest) { 
        ...
        //Integer index = 0;
		for (WMSWorkStatusReportApplication application : wmsWorkStatusReportRequest.getWmsWorkStatusReportApplications()) {
			// Enrich audit details
//	            AuditDetails auditDetails = AuditDetails.builder().createdBy(birthRegistrationRequest.getRequestInfo().getUserInfo().getUuid()).createdTime(System.currentTimeMillis()).lastModifiedBy(birthRegistrationRequest.getRequestInfo().getUserInfo().getUuid()).lastModifiedTime(System.currentTimeMillis()).build();
//	            application.setAuditDetails(auditDetails);
			
			AuditDetails auditDetails = AuditDetails.builder().createdBy(wmsWorkStatusReportRequest.getRequestInfo().getUserInfo().getUuid()).createdTime(System.currentTimeMillis()).lastModifiedBy(wmsWorkStatusReportRequest.getRequestInfo().getUserInfo().getUuid()).lastModifiedTime(System.currentTimeMillis()).build();
            application.setAuditDetails(auditDetails);

			// Enrich UUID
            Long randomNumber=(long) Math.floor(Math.random() * (9999 - 1000 + 1) + 1000);
			application.setWsrId(Long.toString(randomNumber));
	...
	public void enrichWorkStatusReportApplicationUpdate(WMSWorkStatusReportRequest wmsWorkStatusReportRequest, List<WMSWorkStatusReportApplication> existingApplication) {
		// Enrich lastModifiedTime and lastModifiedBy in case of update
		for (WMSWorkStatusReportApplication application : wmsWorkStatusReportRequest.getWmsWorkStatusReportApplications()) {
			existingApplication.get(0).setActivityName(application.getActivityName());
			...
			existingApplication.get(0).setEndDate(application.getEndDate());
			existingApplication.get(0).setStartDate(application.getStartDate());
			existingApplication.get(0).setRoleName(application.getRoleName());
