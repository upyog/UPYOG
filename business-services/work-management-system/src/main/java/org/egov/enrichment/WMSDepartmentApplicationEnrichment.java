...
@Slf4j
@Component
public class WMSDepartmentApplicationEnrichment {
...
//	    private UserUtil userUtils;
	...
	public void enrichDepartmentApplication(WMSDepartmentRequest wmsDepartmentRequest) { 
        ...
        //Integer index = 0;
		for (WMSDepartmentApplication application : wmsDepartmentRequest.getWmsDepartmentApplications()) {
			// Enrich audit details
//	            AuditDetails auditDetails = AuditDetails.builder().createdBy(birthRegistrationRequest.getRequestInfo().getUserInfo().getUuid()).createdTime(System.currentTimeMillis()).lastModifiedBy(birthRegistrationRequest.getRequestInfo().getUserInfo().getUuid()).lastModifiedTime(System.currentTimeMillis()).build();
//	            application.setAuditDetails(auditDetails);
			
			AuditDetails auditDetails = AuditDetails.builder().createdBy(wmsDepartmentRequest.getRequestInfo().getUserInfo().getUuid()).createdTime(System.currentTimeMillis()).lastModifiedBy(wmsDepartmentRequest.getRequestInfo().getUserInfo().getUuid()).lastModifiedTime(System.currentTimeMillis()).build();
            application.setAuditDetails(auditDetails);

			// Enrich UUID
            Long randomNumber=(long) Math.floor(Math.random() * (9999 - 1000 + 1) + 1000);
			application.setDeptId(Long.toString(randomNumber));
	...
	public void enrichDepartmentApplicationUpdate(WMSDepartmentRequest wmsDepartmentRequest, List<WMSDepartmentApplication> existingApplication) {
		// Enrich lastModifiedTime and lastModifiedBy in case of update
		for (WMSDepartmentApplication application : wmsDepartmentRequest.getWmsDepartmentApplications()) {
			existingApplication.get(0).setDeptName(application.getDeptName());
			existingApplication.get(0).setDeptStatus(application.getDeptStatus());
