...
@Slf4j
@Repository
public class WMSContractorRepository {
	
	@Autowired
    private WMSContractorApplicationQueryBuilder queryBuilder;

    @Autowired
    private JdbcTemplate jdbcTemplate;

    @Autowired
    private WMSContractorApplicationRowMapper rowMapper;

    public List<WMSContractorApplication>getApplications(WMSContractorApplicationSearchCriteria searchCriteria){
        List<Object> preparedStmtList = new ArrayList<>();
        String query = queryBuilder.getContractorApplicationSearchQuery(searchCriteria, preparedStmtList);
