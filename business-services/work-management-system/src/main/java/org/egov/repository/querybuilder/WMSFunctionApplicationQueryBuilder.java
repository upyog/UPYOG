...
@Component
public class WMSFunctionApplicationQueryBuilder {

	
	private static final String BASE_FUNC_QUERY = " SELECT func.function_id as fFunctionId, func.function_name as fFunctionName, func.function_code as fFunctionCode, func.function_level as fFunctionLevel,func.status as fStatus, func.createdby as fCreatedBy, func.lastmodifiedby as fLastmodifiedby, func.createdtime as fCreatedtime, func.lastmodifiedtime as fLastmodifiedtime ";
    ...
    private static final String FROM_TABLES = " FROM function func";

    private final String ORDERBY_CREATEDTIME = " ORDER BY func.function_name DESC ";

    public String getFunctionApplicationSearchQuery(WMSFunctionApplicationSearchCriteria criteria, List<Object> preparedStmtList){
        StringBuilder query = new StringBuilder(BASE_FUNC_QUERY);
       // query.append(ADDRESS_SELECT_QUERY);
        query.append(FROM_TABLES);

        if(!ObjectUtils.isEmpty(criteria.getFunctionId())){
        	 addClauseIfRequired(query, preparedStmtList);
             query.append(" func.function_id IN ( ").append(createQuery(criteria.getFunctionId())).append(" ) ");
             addToPreparedStatement(preparedStmtList, criteria.getFunctionId());
        }
