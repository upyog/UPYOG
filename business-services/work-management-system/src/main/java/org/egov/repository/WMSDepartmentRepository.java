...
@Slf4j
@Repository
public class WMSDepartmentRepository {
	
	@Autowired
    private WMSDepartmentApplicationQueryBuilder queryBuilder;

    @Autowired
    private JdbcTemplate jdbcTemplate;

    @Autowired
    private WMSDepartmentApplicationRowMapper rowMapper;

    public List<WMSDepartmentApplication>getApplications(WMSDepartmentApplicationSearchCriteria searchCriteria){
        List<Object> preparedStmtList = new ArrayList<>();
        String query = queryBuilder.getDepartmentApplicationSearchQuery(searchCriteria, preparedStmtList);
