...
@Slf4j
@Repository
public class WMSVendorTypeRepository {
	
	@Autowired
    private WMSVendorTypeApplicationQueryBuilder queryBuilder;

    @Autowired
    private JdbcTemplate jdbcTemplate;

    @Autowired
    private WMSVendorTypeApplicationRowMapper rowMapper;

    public List<WMSVendorTypeApplication>getApplications(WMSVendorTypeApplicationSearchCriteria searchCriteria){
        List<Object> preparedStmtList = new ArrayList<>();
        String query = queryBuilder.getVendorTypeApplicationSearchQuery(searchCriteria, preparedStmtList);
