	...
	public IngestionResult load(DashboardPayload dashboardPayload) {
		...
		while (true) {
			...
			try {
				...
				log.info("DashboardDataLoaderImpl | attempt {} posting to: {}", retryAttemptCount, adapterProperties.getDashboardIngestUrl());
				...
				if (retryAttemptCount >= adapterProperties.getIngestMaxAttempts() || !adapterProperties.isIngestRetryEnabled()) {
					log.error("DashboardDataLoaderImpl | Ingestion attempts failed. Retry enabled: {}", adapterProperties.isIngestRetryEnabled());
				...
				long retryBackoffDurationMs = RetryUtil.calculateBackoffWithJitter(retryAttemptCount, adapterProperties.getIngestBaseDelayMs(), adapterProperties.getIngestMaxDelayMs());
				log.warn("DashboardDataLoaderImpl | Attempt {}/{} failed. Retrying in {} ms. Error: {}", 
						retryAttemptCount, adapterProperties.getIngestMaxAttempts(), retryBackoffDurationMs, failureReasonMessage);
				try {
