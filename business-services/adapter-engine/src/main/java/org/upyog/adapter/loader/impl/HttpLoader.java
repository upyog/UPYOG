...
public class HttpLoader implements Loader {
	...
	public IngestionResult load(DashboardPayload data) {
		...
		while (true) {
			...
			try {
				...
				log.info("HttpLoader | attempt {} posting to: {}", attempt, adapterProperties.getDashboardIngestUrl());

				responseJson = dashboardFeignClient.ingestMetrics(
						java.net.URI.create(adapterProperties.getDashboardIngestUrl()),
						requestJson
				);
				System.out.println("Response of ingest API : " + responseJson);
				status = "SUCCESS";
				...
				if (e instanceof feign.FeignException feignEx) {
				...
				if (attempt >= adapterProperties.getIngestMaxAttempts() || !adapterProperties.isIngestRetryEnabled()) {
					log.error("HttpLoader | Ingestion attempts failed. Retry enabled: {}", adapterProperties.isIngestRetryEnabled());
				...
				log.warn("HttpLoader | Attempt {}/{} failed. Retrying in {} ms. Error: {}", 
						attempt, adapterProperties.getIngestMaxAttempts(), backoff, failureReason);
				try {
	...
	private long calculateBackoffWithJitter(int attempt) {
		...
		long expDelay = adapterProperties.getIngestBaseDelayMs() * (1L << power);
		if (expDelay < 0) {
			expDelay = adapterProperties.getIngestMaxDelayMs();
		}
		long currentMaxDelay = Math.min(adapterProperties.getIngestMaxDelayMs(), expDelay);
