...
const CardBasedOptions = ({ header, sideOption, options, styles = {}, style={} }) => {
  return (
    <div className="CardBasedOptions" style={style}>
       <div className="employeeCustomCard" style={{ width: "100%", height: "100%", position: "relative",display:"flex",fontSize:"1.2rem",fontWeight:"700" }}>
            <h2 style={{width:"70%",padding:"20px",height:"fit-content",color:"black"}}>{header}</h2>
            {/* <p onClick={sideOption.onClick}></p> */}
            {/* <button type="button" class="inboxButton" onClick={sideOption.onClick} style={{color:"black",fontSize:"1rem"}}>
            {sideOption.name}
                      </button> */}
