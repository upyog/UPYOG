...
export const FormComposerNew = (props) => {
  const {
    register,
    handleSubmit,
    setValue,
                ...
                return (
                  <React.Fragment key={index}>
                    <div style={field.isInsideBox ? getCombinedStyle(field?.placementinbox) : {}}>
                      {!field.withoutLabel && (
                        <CardLabel
                          style={{ color: field.isSectionText ? "#white" : "white", marginBottom: props.inline ? "8px" : "revert", letterSpacing:"2px",fontSize:"18px !important" }}
    ...
    <form onSubmit={handleSubmit(onSubmit)} onKeyDown={(e) => checkKeyDown(e)} id={props.formId} className={props.className}>
      <Card style={getCardStyles()} className={props?.cardClassName ? props.cardClassName : ""}>
        ...
        {props.secondaryActionLabel && (
          <div className="primary-label-btn" style={{ margin: "20px auto 0 auto",color:"white" }} onClick={onSecondayActionClick}>
            {props.secondaryActionLabel}
          </div>
        )}
