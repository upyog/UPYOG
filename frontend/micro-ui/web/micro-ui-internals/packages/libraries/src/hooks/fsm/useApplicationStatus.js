...
const useApplicationStatus = (select, isEnabled = true, statusMap=[]) => {
  ...
  const fetch = async () => {
    let WorkflowService = await Digit.WorkflowService.init(tenantId, "FSM,FSM_POST_PAY_SERVICE,PAY_LATER_SERVICE,FSM_ADVANCE_PAY_SERVICE_V1,FSM_ZERO_PAY_SERVICE");
