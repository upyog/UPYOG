...
export const ComplaintsList = (props) => {
  ...
  return (
    <React.Fragment>
      <div className="applications-list-container" style={{marginLeft:"10px", width:isMobile ?"100%":"70%", marginTop:"20px"}}>
        <Header>{t(LOCALE.MY_COMPLAINTS)}</Header>
        <div style={{ display: "flex", marginBottom: "16px", width:"100%" }}>
          <TextInput
            value={searchInput}
            onChange={(e) => setSearchInput(e.target.value)}
            placeholder={t("Search by Incident Id")}
            style={{ marginRight: "8px" }}
          />
          <ButtonSelector label="Search" onSubmit={(e) =>handleSearch(e)} style={{marginLeft:"20px", width:"30%"}}/>
        </div>
        {complaintsList}
      </div>
    </React.Fragment>
