...
const ADSSearch = ({ t, onSelect, config, userType, formData }) => {
  ...
  let LocationData = [];
  let FaceId = [];
  const { data: AdType } = Digit.Hooks.useCustomMDMSV2(Digit.ULBService.getStateId(), "Advertisement", [{ name: "AdType" }], {
    select: (data) => {
  ...
  const { data: LocationDetails } = Digit.Hooks.useCustomMDMSV2(Digit.ULBService.getStateId(), "Advertisement", [{ name: "Location" }], {
    select: (data) => {
  ...
  const { data: Face } = Digit.Hooks.useCustomMDMSV2(Digit.ULBService.getStateId(), "Advertisement", [{ name: "FaceArea" }], {
    select: (data) => {
  ...
  const { data: CalculationTypeData } = Digit.Hooks.useCustomMDMSV2(Digit.ULBService.getStateId(), "Advertisement", [{ name: "CalculationType" }],
    {
      select: (data) => {
