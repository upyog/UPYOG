...
const CMSearchCertificate = () => {
  ...
  async function SVCertificate(certificate_No) {
    const applicationDetails = await Digit.SVService.search({ tenantId, filters: { applicationNumber: certificate_No } });
    const dataSV = applicationDetails?.SVDetail[0];
    console.log("SV certificate data ::", dataSV);

    if (dataSV?.applicationNo) {
      const Req_data = dataCMObjectConverter(
        dataSV?.vendorDetail[0]?.name,
        getAddress(dataSV?.addressDetails[0]),
        dataSV?.applicationNo,
        convertEpochToDate(dataSV?.issuedDate),
        convertEpochToDate(dataSV?.validTo),
        dataSV?.applicationStatus
      );
      setUpdatedData([Req_data]);
    } else {
      setShowToast({ label: "Application doesn't exist", warning: true })
    }
  }
  async function ADVCertificate(certificate_No) {
    const applicationDetails = await Digit.ADSServices.search({ tenantId, filters: { BookingNo: certificate_No } });
    const dataADV = applicationDetails;
    console.log("advertisemrn certificate data ::", dataADV);
  ...
  function onSubmit() {
    switch (certificate_name.crtNo) {
      ...
      case 15:
        if (certificate_No) {
          SVCertificate(certificate_No);
        }
        break;

      case 16:
        if (certificate_No) {
          ADVCertificate(certificate_No);
        }
        break;
