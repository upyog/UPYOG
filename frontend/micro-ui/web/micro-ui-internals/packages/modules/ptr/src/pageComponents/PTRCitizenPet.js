...
const PTRCitizenPet = ({ t, config, onSelect, userType, formData, ownerIndex }) => {
  ...
  function setpetage(e) {
    // allowing only numeric values , no spaces allowed
  ...
  function setdoctorname(e) {
    // allowing only alphabets with spaces
  ...
  function setvaccinationnumber(e) {
    // any combination of uppercase letters, lowercase letters, numbers, and hyphens is allowed
    const inputValue = e.target.value;
    if (/^[A-Za-z0-9-]*$/.test(inputValue)) {
      setVaccinationNumber(inputValue);
    }
  }
  ...
  return (
    <React.Fragment>
      ...
      <FormStep
        ...
        t={t}
        isDisabled={
          !petType || !breedType || !petGender || !petName || !petAge || !doctorName || !clinicName || !lastVaccineDate || !vaccinationNumber
        }
      >
        <div>
