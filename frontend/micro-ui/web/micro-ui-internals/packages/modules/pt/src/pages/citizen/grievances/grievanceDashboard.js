...
const CitizenGrievanceDashboard = ({
            ...
            .then(data => {
                ...
                if(data?.response?.length > 0) {
                    ...
                    dataList.map((data)=>{
                        setCardDetails(prev => ({
                            ...prev,
                            totalGrievance: prev.totalGrievance + 1
                        }));
                        if(data?.status=='Closed') {
                            setCardDetails(prev => ({ ...prev, closed: prev.closed + 1 }));
                        } else if(data?.status=='Pending' || data?.status=='Pending with citizen') {
                            setCardDetails(prev => ({ ...prev, inProgress: prev.inProgress + 1 }));
                        } else if(data?.status=='Appeal raised') {
                            setCardDetails(prev => ({ ...prev, appealRaised: prev.appealRaised + 1 }));
                        } else if(data?.status=='Approve') {
                            setCardDetails(prev => ({ ...prev, disposed: prev.disposed + 1 }));
                        }
    ...
    const onClickFileGrievance = () => {
        history.replace("/digit-ui/citizen/pt/pt-file-grievance");
    }
    const gotoGrievance = (el) => {
        history.replace(`/digit-ui/citizen/pt/pt-grievance/${el?.grievance_id}`);
...
export default CitizenGrievanceDashboard;
