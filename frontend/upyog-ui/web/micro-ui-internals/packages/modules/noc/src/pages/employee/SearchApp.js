 ...
 * 
 * @usage
 * <SearchApp path="/upyog-ui/employee/noc/firenoc/search" />
 */
...
import FireNocSearchApplications from "../../components/SearchApplications";
...
const SearchApp = ({path}) => {
    ...
    function onSubmit (_data) {
        ...
        const hasSearchFields = payload.applicationNumber || payload.fromDate || payload.mobileNumber || payload.status || payload.toDate;
        
        if(!hasSearchFields)
        setShowToast({ warning: true, label: "ERR_PTR_FILL_VALID_FIELDS" });
        else if((!payload.applicationNumber && !payload.fromDate && !payload.mobileNumber && !payload.toDate))
    ...
    const { isLoading, isSuccess, isError, error, data: {FireNOCs: searchReult, Count: count} = {} } = Digit.Hooks.noc.useFireNOCSearch(tenantId, payload );
    
    return <React.Fragment>
        <FireNocSearchApplications t={t} isLoading={isLoading} tenantId={tenantId} setShowToast={setShowToast} onSubmit={onSubmit} onClear={onClear} data={  isSuccess && !isLoading ? (searchReult.length>0? searchReult : { display: "ES_COMMON_NO_DATA" } ):""} count={count} /> 
        {showToast && (
        <Toast
