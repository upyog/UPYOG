  ...
  const sendOtpToUser = async (record) => {
    ...
    if(onSelect) {  
      ...
      if (!err) {
        ...
        let redirectUrl = '/mycity-ui/citizen/commonPt/property/link-success/'+record.property_id;
        if(redirectToUrl) {
          redirectUrl = redirectToUrl+'?propertyId='+record.property_id+'&tenantId='+tenantId;
        } 

        history.replace(`/mycity-ui/citizen/commonPt/property/citizen-otp`, { from: getFromLocation(location.state, searchParams), mobileNumber: record.owner_mobile, redirectBackTo: redirectUrl });
        return;
      } else {
        history.push(`/mycity-ui/citizen/`, { from: getFromLocation(location.state, searchParams), data:data });
      }
    }
  };
      ...
      {searchResults?.length !== 0 && (searchResults?.length == 5 || searchResults?.length == 50) && (locality || ( searchQuery && searchQuery.locality )) && (
          <div>
            <p style={{ marginLeft: "16px", marginTop: "16px" }}>
              {t("PT_LOAD_MORE_MSG")}{" "}
              <span className="link">{<Link to={`/mycity-ui/citizen/pt/property/search-results?mobileNumber=${mobileNumber || searchQuery.mobileNumber ?mobileNumber || searchQuery?.mobileNumber:""}&propertyIds=${propertyIds || searchQuery?.propertyIds ?propertyIds || searchQuery?.propertyIds:""}&oldPropertyIds=${oldPropertyIds || searchQuery?.oldPropertyIds?oldPropertyIds || searchQuery?.oldPropertyIds:""}&doorNo=${doorNo || searchQuery?.doorNo?doorNo || searchQuery?.doorNo:""}&name=${name || searchQuery?.name?name || searchQuery?.name:""}&city=${city?city:""}&locality=${locality || searchQuery?.locality?locality || searchQuery?.locality:""}&PToffset=${t1}`}>{t("PT_COMMON_CLICK_HERE")}</Link>}</span>
            </p>
          </div>
        )}
    </div>
