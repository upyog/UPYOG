...
const PropertySearchNSummary = ({ config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  ...
  const searchProperty = () => {
    ...
    if(window.location.pathname.includes("/tl/new-application")){
      history.push(`/mycity-ui/employee/tl/new-application?propertyId=${propertyId}`)
      ...
      history.push(`/mycity-ui/employee/ws/new-application?propertyId=${propertyId}`)
  };
  ...
  return (
    <React.Fragment>
     {(window.location.href.includes("/tl/") ? (!(formData?.tradedetils?.[0]?.structureType?.code === "MOVABLE") && (isEmpNewApplication || isEmpRenewLicense) ) : true) && <div>
      ...
      <span onClick={() => history.push(`/mycity-ui/employee/commonpt/search?redirectToUrl=${redirectBackUrl}&${serachParams}`, { ...state })}>
        <LinkButton label={t("CPT_SEARCH_PROPERTY")} style={{ color: "#a82227", display: "inline-block" }} />
      </span>
      &nbsp; | &nbsp;
      <span onClick={() => history.push(`/mycity-ui/employee/commonpt/new-application?redirectToUrl=${redirectBackUrl}&${serachParams}`, { ...state })}>
