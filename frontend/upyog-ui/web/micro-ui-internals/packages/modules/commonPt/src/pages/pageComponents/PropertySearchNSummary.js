...
const PropertySearchNSummary = ({ config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  ...
  return (
    <React.Fragment>
      ...
      <span onClick={() => history.push(`/upyog-ui/employee/commonpt/search?redirectToUrl=${redirectBackUrl}`, { ...state })}>
        <LinkButton label={t("CPT_SEARCH_PROPERTY")} style={{ color: "#a82227", display: "inline-block" }} />
      </span>
      &nbsp; | &nbsp;
      <span onClick={() => history.push(`/upyog-ui/employee/commonpt/new-application?redirectToUrl=${redirectBackUrl}`, { ...state })}>
      ...
      {propertyDetails && propertyDetails?.Properties.length ? (
        <React.Fragment>
          ...
          <Link to={`/upyog-ui/employee/commonpt/view-property?propertyId=${propertyId}&tenantId=${tenantId}&from=${window.location.pathname?.includes("employee/tl/new-application")
