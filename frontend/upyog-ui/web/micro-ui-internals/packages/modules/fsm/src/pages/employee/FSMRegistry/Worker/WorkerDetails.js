...
const WorkerDetails = (props) => {
  ...
  useEffect(() => {
    switch (selectedAction) {
      ...
      case "EDIT":
        return history.push(`/mycity-ui/employee/fsm/registry/edit-worker?id=${id}`);
      case "HOME":
        return history.push(`/mycity-ui/employee/fsm/registry?selectedTabs=WORKER`);
      default:
        break;
    }
  }, [selectedAction]);
  ...
  const handleDeleteWorker = () => {
    ...
    mutateUpdateWorker(formData, {
      ...
      onSuccess: (data, variables) => {
        ...
        setTimeout(() => {
          closeToast, history.push(`/mycity-ui/employee/fsm/registry?selectedTabs=WORKER`);
        }, 5000);
      },
    });
    setShowModal(false);
