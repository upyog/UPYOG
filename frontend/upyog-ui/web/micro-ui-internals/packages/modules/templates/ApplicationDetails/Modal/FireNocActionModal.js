...
import { configFireNocApproverApplication } from "../config";
...
const ActionModal = ({ t, action, tenantId, state, id, closeModal, submitAction, actionData, applicationData, businessService, moduleCode }) => {
  ...
  function submit(data) {
      let workflow = { action: action?.action, comments: data?.comments, businessService };
      applicationData.fireNOCDetails.action=action?.action
      submitAction({
        FireNOCs: [
          {
            ...applicationData,
