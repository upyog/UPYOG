...
const EditDisconnectionByConfig = () => {
  ...
  let {
    ...
    error
  } = Digit.Hooks.ws.useDisConnectionDetails(t, tenantId, details?.applicationNo, details?.applicationData?.serviceType, {
  ...
  useEffect(() => {
    if (!isLoading && !isConfigLoading) {
      ...
      config?.body?.forEach(data => {
        if (data?.isDisonnectionEditByConfig) bodyDetails.push(data);
      });
