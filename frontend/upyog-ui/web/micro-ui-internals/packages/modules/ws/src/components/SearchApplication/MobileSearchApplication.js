...
const MobileSearchApplication = ({
  ...
  const propsMobileInboxCards = useMemo(() => {
    ...
    const getWaterSewerageData = data => {
      if (data?.applicationType == "NEW_WATER_CONNECTION" || data?.applicationType == "MODIFY_WATER_CONNECTION" || data?.applicationType == "DISCONNECT_WATER_CONNECTION") {
        return "WATER";
      } else if (data?.applicationType == "NEW_SEWERAGE_CONNECTION" || data?.applicationType == "MODIFY_SEWERAGE_CONNECTION" || data?.applicationType == "DISCONNECT_SEWERAGE_CONNECTION") {
        return "SEWERAGE";
      }
    };
    ...
    return data?.map(data => ({
      [t("WS_MYCONNECTIONS_CONSUMER_NO")]: /*data?.connectionNo || "NA",*/data?.connectionNo ? <div>
          <span className="link">
            <Link to={`/upyog-ui/employee/ws/connection-details?applicationNumber=${data?.connectionNo}&tenantId=${tenantId}&service=${getWaterSewerageData(data)}`}>
              {data?.connectionNo}
            </Link>
          </span>
        </div> : t("CS_NA"),
      ...
      [t("WS_ACK_COMMON_APP_NO_LABEL")]: <div>
          <span className="link">
            <Link to={`/upyog-ui/employee/ws/${getFlowUrls(data?.applicationType)}-details?applicationNumber=${data?.applicationNo}&tenantId=${tenantId}&service=${getWaterSewerageData(data)}`}>
              {data?.applicationNo}
            </Link>
          </span>
        </div>,
