...
const WSActivationPlumberDetails = ({
  ...
  return <React.Fragment>
            {plumberDetails.map((plumberDetail, index) => <PlumberDetails key={plumberDetail.key} index={index} plumberDetail={plumberDetail} {...commonProps} />)}
        </React.Fragment>;
};
const PlumberDetails = _props => {
  ...
  return <div>
            ...
            marginTop: "-5px",
            fontWeight: "700"
          }} className="card-label-smaller">{`${t("WS_ADDN_DETAILS_PLUMBER_PROVIDED_BY")}`}<span className="check-page-link-button"> *</span></CardLabel>
                        <Controller control={control} name={"detailsProvidedBy"} defaultValue={plumberDetail?.detailsProvidedBy} rules={{
            required: t("REQUIRED_FIELD")
          }} isMandatory={true} render={props => <Dropdown className="form-field" selected={getValues("detailsProvidedBy")} disable={false} option={options} errorStyle={localFormState.touched.detailsProvidedBy && errors?.detailsProvidedBy?.message ? true : false} select={e => {
            if (e.code == "ULB") {
              let obj = {
                ...plumberDetails?.[0],
                detailsProvidedBy: e,
            ...
            props.onChange(e);
                    ...
                    {!plumberDetail?.detailsProvidedBy?.code || plumberDetail?.detailsProvidedBy?.code == "ULB" ? <div>
                            <LabelFieldPair>
                                <CardLabel style={isMobile && isEmployee ? {
              fontWeight: "700",
              width: "100%"
              ...
              fontWeight: "700"
                                ...
                                <div className="field">
                                    <Controller control={control} name="plumberLicenseNo" defaultValue={plumberDetail?.plumberLicenseNo}
              // rules={{ required: t("REQUIRED_FIELD") }}
              rules={{
                validate: e => e && getPattern("WSOnlyNumbers").test(e) || !e ? true : t("ERR_DEFAULT_INPUT_FIELD_MSG"),
                required: t("REQUIRED_FIELD")
              }} isMandatory={true} render={props => <TextInput value={props.value} autoFocus={focusIndex.index === plumberDetail?.key && focusIndex.type === "plumberLicenseNo"} errorStyle={localFormState.touched.plumberLicenseNo && errors?.plumberLicenseNo?.message ? true : false} onChange={e => {
                props.onChange(e.target.value);
                setFocusIndex({
                  index: plumberDetail?.key,
              ...
              fontWeight: "700"
                                ...
                                <div className="field">
                                    <Controller control={control} name="plumberName" defaultValue={plumberDetail?.plumberName}
              // rules={{ required: t("REQUIRED_FIELD") }}
              rules={{
                validate: e => e && getPattern("Name").test(e) || !e ? true : t("ERR_DEFAULT_INPUT_FIELD_MSG"),
                required: t("REQUIRED_FIELD")
              }} isMandatory={true} render={props => <TextInput value={props.value} autoFocus={focusIndex.index === plumberDetail?.key && focusIndex.type === "plumberName"} errorStyle={localFormState.touched.plumberName && errors?.plumberName?.message ? true : false} onChange={e => {
                props.onChange(e.target.value);
                setFocusIndex({
                  index: plumberDetail?.key,
                  type: "plumberName"
              ...
              fontWeight: "700"
                                ...
                                <div className="field">
                                    <Controller control={control} name="plumberMobileNo" defaultValue={plumberDetail?.plumberMobileNo}
              // rules={{ required: t("REQUIRED_FIELD") }}
              rules={{
                validate: e => e && getPattern("MobileNoWithPrivacy").test(e) || !e || e.includes("*") ? true : t("ERR_DEFAULT_INPUT_FIELD_MSG"),
                required: t("REQUIRED_FIELD")
              }} type="mobileNumber" isMandatory={true} render={props => <div className="ws-auto-39">
                                                <div className="ws-auto-40">+91</div>
                                                <TextInput type="mobileNumber" value={props.value} autoFocus={focusIndex.index === plumberDetail?.key && focusIndex.type === "plumberMobileNo"} errorStyle={localFormState.touched.plumberMobileNo && errors?.plumberMobileNo?.message ? true : false} onChange={e => {
                  props.onChange(e.target.value);
                  setFocusIndex({
                    index: plumberDetail?.key,
                  ...
                  marginTop: "unset"
                }} onBlur={props.onBlur} className="ws-auto-41" />
                  ...
                  marginLeft: "10px"
                    ...
                    props.onChange(e);
                  }} iseyevisible={props.value?.includes("*") ? true : false} privacy={{
                    ...
                    loadData: {
                      ...
                      jsonPath: formData?.connectionDetails?.[0]?.formDetails?.applicationData?.serviceType === "WATER" || formData?.disConnectionDetails?.[0]?.consumerNumber?.includes("WS") ? "WaterConnection[0].plumberInfo[0].mobileNumber" : "SewerageConnections[0].plumberInfo[0].mobileNumber",
                      isArray: false
                    }
                  }}>
