...
import { convertDateToEpoch, convertEpochToDate, createPayloadOfWSReSubmitDisconnection,  } from "../../../utils";
  ...
  const CheckPage = () => {
    ...
    let routeLink = `/mycity-ui/citizen/ws/resubmit-disconnect-application`;
    if(window.location.href.includes("/resubmit"))
    routeLink=`/mycity-ui/citizen/ws/resubmit-disconnect-application`
        ...
        if (waterUpdateMutation) {
          ...
          await waterUpdateMutation(payload, {
            ...
            onSuccess: async (data, variables) => {
                ...
                history.push(`/mycity-ui/citizen/ws/disconnect-acknowledge?applicationNumber=${data?.WaterConnection?.[0]?.applicationNo}`);
            },
          });
        }
      }
        ...
        if (sewerageUpdateMutation) {
          ...
          await sewerageUpdateMutation(payload, {
            ...
            onSuccess: async (data, variables) => {
                ...
                history.push(`/mycity-ui/citizen/ws/disconnect-acknowledge?applicationNumber=${data?.SewerageConnections?.[0]?.applicationNo}`);
            },
          });
        }
      }
