      ...
      <div className="hide-seperator">
        ...
        <Card>
          ...
          <StatusTable>
            ...
            />
            <Link
              to={`/mycity-ui/citizen/commonpt/view-property?propertyId=${
        ...
        : (<Card>
          ...
          {data?.WaterConnection && data?.WaterConnection?.length > 0 && (
            <StatusTable>
              ...
              <Link to={`/mycity-ui/citizen/ws/connection/additional/${data?.WaterConnection?.[0]?.applicationNo}`}>
          ...
          {data?.SewerageConnections && data?.SewerageConnections?.length > 0 && (
            <StatusTable>
              ...
              <Row
                ...
                textStyle={{ whiteSpace: "pre" }}
              />
              <Link to={`/mycity-ui/citizen/ws/connection/additional/${data?.SewerageConnections?.[0]?.applicationNo}`}>
                <LinkButton style={{ textAlign: "left" }} label={t("WS_ADDITIONAL_DETAILS")} />
              </Link>
            </StatusTable>
          )}
        ...
        <Card>
          ...
          (!data?.SewerageConnections?.[0]?.applicationType.includes("DISCONNECT") && data?.SewerageConnections?.[0]?.applicationStatus.includes("PENDING_FOR_CITIZEN_ACTION")) ? (
            <Link
              to={{
                pathname: `/mycity-ui/citizen/ws/edit-application/${data?.WaterConnection?.[0]?.tenantId || data?.SewerageConnections?.[0]?.tenantId}`,
          ...
          ( data?.SewerageConnections?.[0]?.applicationType.includes("DISCONNECT") && data?.SewerageConnections?.[0]?.applicationStatus.includes("PENDING_FOR_CITIZEN_ACTION")) ? (
            <Link
              to={{
                pathname: `/mycity-ui/citizen/ws/resubmit-disconnect-application`,
                state: { id: `${data?.WaterConnection?.[0]?.applicationNo || data?.SewerageConnections?.[0]?.applicationNo}` },
              }}
            >
