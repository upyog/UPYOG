import { Card, Header, KeyNote, Loader, StatusTable, SubmitBar, Row } from "@nudmcdgnpm/digit-ui-react-components";
  ...
  return <React.Fragment>
    ...
    <div>
      {meterReadings?.length > 0 && meterReadings.map((application, index) => <div key={index}>
            ...
            <Row className="border-none" label={t("WS_CONSUMPTION_DETAILS_METER_STATUS_LABEL")} text={application?.meterStatus || t("NA")} textStyle={{
              whiteSpace: "pre"
            }} />
            <Row className="border-none" label={t("WS_CONSUMPTION_DETAILS_LAST_READING_LABEL")} text={application?.lastReading || t("NA")} textStyle={{
              whiteSpace: "pre"
            }} />
            <Row className="border-none" label={t("WS_CONSUMPTION_DETAILS_LAST_READING_DATE_LABEL")} text={Digit.DateUtils.ConvertEpochToDate(application?.lastReadingDate) || t("NA")} textStyle={{
              whiteSpace: "pre"
            }} />
            <Row className="border-none" label={t("WS_CONSUMPTION_DETAILS_CURRENT_READING_LABEL")} text={application?.currentReading || t("NA")} textStyle={{
              whiteSpace: "pre"
            }} />
            <Row className="border-none" label={t("WS_CONSUMPTION_DETAILS_CURRENT_READING_DATE_LABEL")} text={Digit.DateUtils.ConvertEpochToDate(application?.currentReadingDate) || t("NA")} textStyle={{
              whiteSpace: "pre"
            }} />
