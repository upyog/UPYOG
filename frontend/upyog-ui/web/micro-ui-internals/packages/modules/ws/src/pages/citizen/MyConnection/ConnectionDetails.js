    ...
    if (!data?.checkWorkFlow){
      ...
      setTimeout(() => {
        ...
        if (paymentDetails?.data?.Bill?.length === 0 ) {
          let pathname = `/mycity-ui/citizen/ws/disconnect-application`;
  ...
  const getRestorationButton = () => {
    ...
    if (!data?.checkWorkFlow){
      ...
      setTimeout(() => {
        ...
        if (paymentDetails?.data?.Bill?.length === 0 ) {
          console.log("Payment",paymentDetails)
          let pathname = `/mycity-ui/citizen/ws/restoration-application`;
          Digit.SessionStorage.set("WS_DISCONNECTION", {...state, serviceType: isSW ? "SEWERAGE" : "WATER"});
          history.push(`${pathname}`);
        } else if(paymentDetails?.data?.Bill?.[0]?.totalAmount < 0)
        {
          let pathname = `/mycity-ui/citizen/ws/restore-application/restoration-application`;
        Digit.SessionStorage.set("WS_DISCONNECTION", {...state, serviceType: isSW ? "SEWERAGE" : "WATER"});
        history.push(`${pathname}`);
        }
        else if (paymentDetails?.data?.Bill?.[0]?.totalAmount !== 0) {
        ...
        <Card>
          ...
          <StatusTable>
            ...
            {state?.connectionType === "Metered" && (
              <Link to={`/mycity-ui/citizen/ws/consumption/details?applicationNo=${state?.connectionNo}`}>
                <LinkButton
          ...
          <StatusTable>
            ...
            />
            <Link to={`/mycity-ui/citizen/commonpt/view-property?propertyId=${state?.propertyId}&tenantId=${state?.tenantId}`}>
          ...
          {showModal ? (
            <Modal
              ...
              actionSingleSubmit={() => {
                history.push(
                  `/mycity-ui/citizen/payment/collect/${isSW ? "SW" : "WS"}/${encodeURIComponent(
                    state?.connectionNo
