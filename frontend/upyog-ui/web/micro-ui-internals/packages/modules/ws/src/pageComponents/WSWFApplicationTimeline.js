...
const WSWFApplicationTimeline = (props) => {
  ...
  const getTimelineCaptions = (checkpoint) => {
    if (checkpoint.state === "OPEN") {
      ...
      return (
        <div>
          <Link to={`/mycity-ui/citizen/commonpt/view-property?propertyId=${props?.application?.propertyId}&tenantId=${props?.application?.tenantId}`}>
    ...
    switch (nextAction?.action) {
      ...
      { if(props?.paymentbuttonenabled !== false)  return (
          <div style={{ marginTop: "1em", bottom: "0px", width: "100%", marginBottom: "1.2em" }}>
            <Link
              to={{ pathname: `/mycity-ui/citizen/payment/collect/${businessService}/${props.id}?consumerCode=${props.id}&&workflow=WNS`, state: { tenantId: props.application.tenantId } }}
            >
