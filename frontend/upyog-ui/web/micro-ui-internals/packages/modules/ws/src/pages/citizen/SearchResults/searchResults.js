...
const ChallanSearchResults = ({ template, header, actionButtonLabel }) => {
  ...
  const onSubmit = (data) => {
    history.push(`/mycity-ui/citizen/payment/my-bills/${data?.ConsumerNumber.split("/")[0]}/${stringReplaceAll(data?.ConsumerNumber,"/","+")}?workflow=WNS&tenantId=${tenantId}&ConsumerName=${data?.ConsumerName}`);
  };
    ...
    <div style={{ marginTop: "16px" }}>
      <div >
        ...
        {searchResults?.length !== 0 && (searchResults?.length == 5 || searchResults?.length == 50) && (locality || ( searchQuery && searchQuery.locality )) && (
          <div>
            <p style={{ marginLeft: "16px", marginTop: "16px" }}>
              {t("WS_LOAD_MORE_MSG")}{" "}
              <span className="link">{<Link to={`/mycity-ui/citizen/ws/search-results?doorNumber=${doorNumber}&consumerName=${consumerName}&tenantId=${tenantId?.split(".")[0]}&locality=${locality.code}&PToffset=${t1}`}>{t("PT_COMMON_CLICK_HERE")}</Link>}</span>
            </p>
          </div>
        )}
        <p style={{ marginLeft: "16px", marginTop: "16px" }}>
        ...
        <span className="link" style={{ display: "block" }}>
          <Link to="/mycity-ui/citizen/ws/create-application/search-property">{t("WS_CLICK_HERE_TO_APPLY")}</Link>
        </span>
        </p>
      </div>
    </div>
