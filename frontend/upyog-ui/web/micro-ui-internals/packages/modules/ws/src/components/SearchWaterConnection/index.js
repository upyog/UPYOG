...
const SearchWaterConnection = ({ tenantId, onSubmit, data, count, resultOk, businessService, isLoading }) => {
          ...
          return (
            <div>
              {row.original["connectionNo"] ? (
                <span className={"link"}>
                  <Link
                    to={`/mycity-ui/employee/ws/connection-details?applicationNumber=${row.original["connectionNo"]}&tenantId=${tenantId}&service=${
  ...
  const generateDemand = async (row) => {
    ...
    history.push(`/mycity-ui/employee/payment/collect/SW/${encodeURIComponent(
          ...
          <div>
            <span className="link">
              {row.original?.service === "WATER" ? (
                <Link
                  to={{
                    pathname: `/mycity-ui/employee/payment/collect/${row.original?.["service"] === "WATER" ? "WS" : "SW"}/${encodeURIComponent(
                      row.original?.["connectionNo"]
