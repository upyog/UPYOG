...
const WSPropertyDetails = ({ t, config, onSelect, userType, formData, setError, formState, clearErrors }) => {
  const redirectBackUrl = `/mycity-ui/${userType}/ws/new-application`;
  ...
  return (
    <React.Fragment>
      ...
      <Link to={`/mycity-ui/employee/pt/search`}>
        <LinkButton label={t("CPT_SEARCH_PROPERTY")} style={{ color: "#a82227", display: "inline-block" }} />
      </Link>
      &nbsp; | &nbsp;
      <Link to={`/mycity-ui/employee/pt/new-application`}>
      ...
      {propertyDetails && propertyDetails?.Properties.length && (
        <React.Fragment>
          ...
          <Link to={`/mycity-ui/employee/commonpt/view-property?propertyId=${propertyId}&tenantId=${tenantId}`}>
