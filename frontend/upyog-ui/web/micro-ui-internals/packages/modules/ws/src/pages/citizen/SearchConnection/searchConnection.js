  ...
  const onConnectionSearch = async (data) => {
    if(searchType.code === "CONSUMER_NUMBER")
    {
      ...
      setShowToast({ key: true, label: "WS_HOME_SEARCH_CONN_RESULTS_DESC" });
        ...
        history.push(
          `/upyog-ui/citizen/ws/search-results?mobileNumber=${mobileNumber}&consumerNumber=${consumerNumber}&oldconsumerNumber=${oldconsumerNumber}&propertyId=${propertyId}&tenantId=${city.code}&locality=${locality.code}`
        );
      }
    }
    else
