...
let actions = [];
const InventoryCreationTimeLine = ({ currentStep = 1, flow = "" }) => {
  const { t } = useTranslation();
  const isMobile = window.Digit.Utils.browser.isMobile();
  
   if(!flow) {
    actions = ["ADD_INVENTORY_CREATION_REQUEST","INVENTORY_CREATION_SUMMARY"];
  }
...
export default InventoryCreationTimeLine;
