...
const SearchVendorList = ({ isLoading, t, onSubmit, data, count, setShowToast, ActionBarStyle = {}, MenuStyle = {}, parentRoute, tenantId }) => {
  ...
  const columns = useMemo(() => ([
    {
      Header: t("INV_VENDOR_LABEL"),
      Cell: (row) => {
        return GetCell(`${row?.row?.original?.["vendorNumber"]}`)
      },
      disableSortBy: true,
    },
    {
      Header: t("INV_VENDOR_NAME"),
      Cell: (row) => {
        return GetCell(`${row?.row?.original?.["vendorName"]}`)
      },
      disableSortBy: true,
    },
    {
      Header: t("INV_CONTACT_PERSON"),
      Cell: (row) => {
        return GetCell(`${row?.row?.original?.["contactPerson"]}`)
      },
      disableSortBy: true,
    },
    {
      Header: t("INV_CONTACT_NUMBER"),
      Cell: ({ row }) => {
        return GetCell(`${row?.original?.["contactNumber"]}`)
      },
      disableSortBy: true,
    ...
    },
    {
      Header: t("INV_GSTN"),
      Cell: ({ row }) => {
        return GetCell(`${row?.original?.["gstin"]}`)
      },
      disableSortBy: true,
    },
    {
      Header: t("INV_EMAIL_ADDRESS"),
      Cell: ({ row }) => {
        return GetCell(`${row?.original?.["contactEmail"]}`)
      },
      disableSortBy: true,
    },
 ...
 //later will convert it into the action bar same as i have iused in ApplicationDetailsActionBar.js file in template
      ...
      Cell: ({ row }) => {
        ...
        return (
          <div ref={menuRef} style={{ display: 'flex', gap: '10px' }}>
            <Link
              to={`/upyog-ui/employee/asset/assetservice/asset-vendor-edit/${row.original?.vendorName}`}
              style={{
                padding: '5px 10px',
                textDecoration: 'none',
                color: 'blue',
  ...
  return <React.Fragment>
    ...
    <div>
      <Header>{t("ASSET_VENDOR_LIST")}</Header>
...
export default SearchVendorList