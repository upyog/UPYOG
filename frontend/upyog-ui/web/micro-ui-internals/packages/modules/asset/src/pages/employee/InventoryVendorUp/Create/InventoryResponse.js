...
import { InventoryVendorData } from "../../../../utils";

const GetActionMessage = (props) => {
  const { t } = useTranslation();
  if (props.isSuccess) {
    return !window.location.href.includes("edit-application") ? t("INV_VENDOR_CREATE_SUCCESSFULLY") : t("INV_UPDATE_APPLICATION_SUCCESS");
  } else if (props.isLoading) {
    return !window.location.href.includes("edit-application") ? t("INV_APPLICATION_PENDING") : t("INV_UPDATE_APPLICATION_PENDING");
  } else if (!props.isSuccess) {
    return !window.location.href.includes("edit-application") ? t("INV_VENDOR_APPLICATION_FAILED") : t("INV_UPDATE_APPLICATION_FAILED");
  }
};
...
const BannerPicker = (props) => {
  return (
    <Banner
      ...
      applicationNumber={props.data?.Vendors?.[0].vendorNumber}
...
const InventoryResponse = ({ data, onSuccess }) => {
  const { t } = useTranslation();
  const tenantId = Digit.ULBService.getCurrentTenantId();
  const mutation = Digit.Hooks.asset.useInventoryApplication(tenantId);
  ...
  useEffect(() => {
    try {
      let payLoadInventoryVendor = InventoryVendorData(data);
      mutation.mutate(payLoadInventoryVendor, {
        onSuccess,
      });
    } catch (err) {}
  }, []);
