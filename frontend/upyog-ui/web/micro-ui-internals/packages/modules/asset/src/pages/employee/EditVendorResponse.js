...
const GetVendorMessage = (type, action, isSuccess, isEmployee, t) => {
  return t(`${isEmployee ? "E" : "C"}S_VENDOR_RESPONSE_${action ? action : "EDIT_SUCCESS"}_${type}${isSuccess ? "" : "_ERROR"}`);
};
...
const BannerPicker = (props) => {
  return (
    <Banner
      ...
      info={GetVendorLabel(props.action, props.isSuccess, props.isEmployee, props.t)}
...
const EditVendorResponse = (props) => {
  ...
  const [successData, setsuccessData, clearSuccessData] = Digit.Hooks.useSessionStorage("EMPLOYEE_MUTATION_SUCCESS_DATAsssss", false);
...
export default EditVendorResponse;