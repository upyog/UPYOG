      ...
      <div className="submenu-container">
        <div className={`sidebar-link  ${pathname === item?.navigationURL ? "active" : ""}`}>
          <div className="actions">
            {leftIcon}
            {item.navigationURL?.indexOf("/digit-ui")||item.navigationURL?.indexOf("/mycity-ui") === -1? (
              <a
                data-tip="React-tooltip"
                data-for={`jk-side-${getModuleName}`}
                className="custom-link"
                href={getOrigin + window.location.href.includes("/mycity-ui") && (item.navigationURL.includes("digit-ui") || item.navigationURL.includes("/workbench-ui"))? item.navigationURL.replace("digit-ui","mycity-ui") : item.navigationURL.includes("mycity-ui") ? item.navigationURL :"/employee/"+item.navigationURL}
              >
        ...
        {subnav &&
          ...
          .sort((a, b) => a.orderNumber - b.orderNumber)
            ...
            .map((item, index) => {
              ...
              if (item.navigationURL.indexOf("/mycity-ui") || item.navigationURL.indexOf("/digit-ui")=== -1) {
                const getOrigin = window.location.origin;
                return (
                  <a
                    key={index}
                    className={`dropdown-link ${pathname === item.link ? "active" : ""}`}
                    href={getOrigin + window.location.href.includes("/mycity-ui") && item.navigationURL.includes("digit-ui") ? item.navigationURL.replace("digit-ui","mycity-ui") :item.navigationURL.includes("mycity-ui") ? item.navigationURL :"/employee/"+item.navigationURL}
                  >
