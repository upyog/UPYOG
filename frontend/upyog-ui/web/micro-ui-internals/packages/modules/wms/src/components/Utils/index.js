...
export const getCityThatUserhasAccess = (cities = []) => {
  ...
  if (roleObject[Digit.Utils?.wmsRoles?.[0]].includes(Digit.ULBService.getStateId())) {
    return cities;
  }
  return cities.filter(city => roleObject[Digit.Utils?.wmsRoles?.[0]]?.includes(city?.code));
}