...
const WmsCMBankBranchIFSC = ({ t, config, onSelect, formData = {}, userType, register, errors }) => {
      ...
      {inputs?.map((input, index) => {
        ...
        return(<React.Fragment key={index}>
          ...
          <LabelFieldPair>
            ...
            {currentValue&&currentValue.length>0&&!currentValue.match(Digit.Utils.getPattern('OldLicenceNo'))&&<CardLabelError style={{ width: "100%", marginTop: '-15px', fontSize: '16px', marginBottom: '12px'}}>{t("Alphanumeric characters are allowed")}</CardLabelError>}
...
export default WmsCMBankBranchIFSC;
