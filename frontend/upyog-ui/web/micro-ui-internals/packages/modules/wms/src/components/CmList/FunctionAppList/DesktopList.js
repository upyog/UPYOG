...
const DesktopInbox = ({ tableConfig, ...props} ) => {
  ...
  const data = props?.data?.WMSFunctionApplications;
  ...
  const columns = React.useMemo(() => {
    return [
      ...
      // },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => {
          return GetCell(`${row.original?.function_name}`);
        },
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => {
          return GetCell(`${row.original?.function_code}`);
        },
      },
      {
        ...
        disableSortBy: true,
        Cell: ({ row }) => {
          return GetCell(`${row.original?.function_level}`);
        },
      },
      {
        Header: "Status",
      ...
      },
      {
        ...
        Cell: ({ row }) => {
          return (
            ...
            {' '} */}
            <span className="link">
            <Link to={`edit/${props.tenantId}/${row?.original?.function_id}`}>Edit</Link>
          </span>
          </React.Fragment>
