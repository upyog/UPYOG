...
import { convertEpochToDate } from "../../components/Utils/index";

const WmsPrjApprovalDate = ({ t, config, onSelect, formData = {}, userType, register, errors }) => {
  const { pathname: url } = useLocation();
  const inputs = [
    {
      label: "WMS_PRJ_APPROVAL_DATE_LABEL",
      type: "date",
      name: "approval_date",
      validation: {
        isRequired: true,
        title: t("WMS_COMMON_END_DATE_INVALID"),
      },
      isMandatory: true,
    },
  ];
...
export default WmsPrjApprovalDate;
