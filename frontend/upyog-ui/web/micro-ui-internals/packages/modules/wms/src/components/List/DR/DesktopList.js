...
const DesktopList = ({ tableConfig, filterComponent, ...props }) => {
  ...
  const columns = React.useMemo(() => {
    return [
      ...
      {
        Header: t("WMS_DR_PROJECT_NAME_LABEL"),
        disableDrtBy: false,
        Cell: ({ row }) => {
      ...
      {
        Header: t("WMS_DR_WORK_NAME_LABEL"),
        disableDrtBy: false,
        Cell: ({ row }) => {
      ...
      {
        Header: t("WMS_DR_PERCENT_NAME_LABEL"),
        disableDrtBy: true,
        Cell: ({ row }) => {
      ...
      {
        Header: t("WMS_DR_CONTROLS_LABEL"),
        disableDrtBy: true,
        Cell: ({ row }) => {
          return (
            <span className="link">
              <Link to={`/upyog-ui/citizen/wms/dr-details/${row.original.dr_id}`}>{t("WMS_DR_EDIT_LABEL")}</Link>
            </span>
          );
        },
      },
      {
        Header: t("WMS_DR_CONTROLS2_LABEL"),
        disableDrtBy: true,
        Cell: ({ row }) => {
          return (
            <span className="link">
              <Link to={`/upyog-ui/citizen/wms/pma-home/${row.original.dr_id}`}>{t("WMS_DR_COMMON_CREATE_ACTIVITY_HEADER")}</Link>
            </span>
          );
        },
      },
    ...
    result = (
      <DataTable
        ...
        getCellProps={(cellInfo) => {
          return {
            style: {
              maxWidth: cellInfo.column.Header == t("WMS_DR_ID_LABEL") ? "150px" : "",
              padding: "20px 18px",
              fontSize: "16px",
              minWidth: "150px",
            },
      ...
      {!props.isSearch && (
        <div className="filters-container">
          <DataLinks
            ...
            allLinks={[
              {
                text: t("WMS_DR_COMMON_CREATE_HEADER"),
                link: "/upyog-ui/citizen/wms/dr-create",
                businessService: "WMS",
                roles: ["CITIZEN"],
              },
/*               {
                text: t("WMS_DR_COMMON_CREATE_ACTIVITY_HEADER"),
                link: "/upyog-ui/citizen/wms/dr-activity-home",
                businessService: "WMS",
                roles: ["CITIZEN"],
              }, */
            ]}
            headerText={t("WMS_DR_CREATE_PAGE_LABEL")}
