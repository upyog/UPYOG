...
const DesktopInbox = ({ tableConfig, ...props} ) => {
  ...
  const data = props?.data?.WMSContractorSubTypeApplications;
...
console.log("data ",data)
  ...
  const columns = React.useMemo(() => {
    return [
      {
        Header: "Vendor Sub Type Name",
        disableSortBy: true,
        Cell: ({ row }) => {
          return (row.original.contractor_stype_name);
        },
      },
      {
        Header: "Status",
        ...
        Cell: ({ row }) => {
          ...
          return GetSlaCell(`${row?.original?.contractor_stype_status==="Active" ? row?.original?.contractor_stype_status.toUpperCase() : row?.original?.contractor_stype_status.toUpperCase()}`);
        },
      },
      {
        Header: "Action",
        ...
        Cell: ({ row }) => {
          return (
            ...
            {' '} */}
            <span className="link">
            <Link to={`edit/${props.tenantId}/${row.original.contractor_id}`}>Edit</Link>
          </span>
          </React.Fragment>
