...
const WmsPhmDetails = () => {
  ...
  const { id: phmId } = useParams();
  const { tenantId: tenantId } = useParams()
  const history = useHistory();
  const [displayMenu, setDisplayMenu] = useState(false);
  const isupdate = Digit.SessionStorage.get("isupdate");
  const { isLoading, isError, error, data, ...rest } = Digit.Hooks.wms.phm.useWmsPhmSearch({ phmeme_id: phmId }, tenantId, null, isupdate);
  ...
  const downloadPhmDetails = async () => {
  ...
  return (
    <React.Fragment>
      ...
      {!isLoading && data?.length > 0 ? (
        <div>
          <Card>
            ...
            <StatusTable>
              <Row label={t("WMS_PHM_ID_LABEL")} text={data?.[0]?.phm_id || "NA"} textStyle={{ whiteSpace: "pre" }} />
      ...
      <ActionBar>
        <SubmitBar label={t("WMS_COMMON_TAKE_ACTION")} onSubmit={() =>  history.push(`/upyog-ui/citizen/wms/phm-edit/${data?.[0]?.phm_id}`)} />
      </ActionBar>
    </React.Fragment>
  );
};

export default WmsPhmDetails;
