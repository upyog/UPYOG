...
const WmsSchUploadDocument = ({ t, config, onSelect, userType, formData, setError: setFormError, clearErrors: clearFormErrors, formState }) => {
  ...
  const isEditScreen = pathname.includes("/schmst-edit/");
  const isMutation = pathname.includes("/schmst-mutate/");

  if (isEditScreen) action = "update";

  const schemeInitialValues = JSON.parse(sessionStorage.getItem("SchInitials"));

  const { isLoading, data } = Digit.Hooks.sch.useSchMDMS(stateId, "Scheme", [
    "Documents",
  ]);


  const mutationDocs = data?.Scheme?.MutationDocuments;
  const commonDocs = data?.Scheme?.Documents;

  const schemeDocuments = isMutation
    ? mutationDocs?.map?.((doc) => commonDocs.find((e) => doc.code === e.code) || doc)
    : data?.Scheme?.Documents;

  const goNext = () => {
    onSelect(config.key, { documents, schemeDocumentsLength: schemeDocuments?.length });
  };
  ...
  return (
    <div>
      {isMutation ? <CardSectionHeader>{t("SCH_MUTATION_DOCUMENTS_HEADER")} </CardSectionHeader> : null}
      {schemeDocuments?.map((document, index) => {
        ...
        return (
          <SelectDocument
            ...
            config={config}
            formState={formState}
            schemeInitialValues={schemeInitialValues}
          />
        );
      })}
...
function SelectDocument({
      ...
      if (file) {
        if (file.size >= 5242880) {
          ...
          try {
            ...
            const response = await Digit.UploadServices.Filestorage("SCH", file, Digit.ULBService.getStateId());
  ...
  if (filterCondition) {
    ...
    if (action === "create") {
      const value = formDataPath?.reduce((acc, key) => {
        if (key.charAt(0).toUpperCase() + key.slice(1) === "SchType") return acc["SchType"];
        return acc?.[key];
      }, formData);
    ...
    if (action === "update") {
      const a = fromRawData ? jsonPath : jsonPath?.split("Schemes[0].schemeDetails[0].")[1];
  ...
  if (dropdownFilter) {
    ...
    if (["create", "update"].includes(action)) {
      if (enabledActions?.[action].disableUpload) {
        if (onArray) {
          const keyForArr = parentArrayJsonPath?.split("Schemes[0].schemeDetails[0].")[1].split(".");
          ...
          const keyForArr = parentJsonpath?.split("Schemes[0].schemeDetails[0].")[1].split(".");
      ...
      {doc?.hasDropdown ? (
        <LabelFieldPair>
          ...
          <Dropdown
            ...
            disable={dropDownData?.length === 0 || (schemeInitialValues?.documents && schemeInitialValues?.documents.length>0 && schemeInitialValues?.documents.filter((document) => document.documentType.includes(doc?.code)).length>0? enabledActions?.[action].disableDropdown : false)}
      ...
      <LabelFieldPair>
        ...
        <div className="field">
          <UploadFile
            ...
            disabled={(schemeInitialValues?.documents && schemeInitialValues?.documents.length>0 && schemeInitialValues?.documents.filter((document) => document.documentType.includes(doc?.code)).length>0? enabledActions?.[action].disableUpload : false) || !selectedDocument?.code}
            buttonType="button"
            error={!uploadedFile}
          />
        </div>
...
export default WmsSchUploadDocument;
