...
const WmsSorEdit = () => {
  ...
  const [sorName, setSorName] = useState(null);
  ...
  const { isLoading, isError, error, data:dataEdit, ...rest } = Digit.Hooks.wms.sor.useWmsSorSearch({ sor_id: sorId }, tenantId, null, isupdate);
  var defaultValues={};
  useEffect(() => {
  ...
  const checkMailNameNum = (formData) => {
    ...
    const item_no = formData?.WmsSorItemNo?.item_no || '';
    const unit = formData?.WmsSorUnit?.unit || '';
    const rate = formData?.WmsSorRate?.rate|| '';
  ...
  const onSubmit = (input) => {
    ...
    requestdata.start_date = Date.parse(input?.WmsSorStartDate?.start_date);
 ...
 // alert(JSON.stringify(dataEdit[0]?.sor_id));
  ...
  return (
    <div>
      <FormComposer
        heading={t("WMS_SOR_FORM_CREATE_HEAD")}
