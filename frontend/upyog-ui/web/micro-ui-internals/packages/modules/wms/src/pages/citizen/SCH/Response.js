...
const GetMessage = (type, action, isSuccess, isEmployee, t) => {
  return t(`SCH_RESPONSE_${action ? action : "CREATE"}_${type}${isSuccess ? "" : "_ERROR"}`);
};
...
const WmsSchResponse = (props) => {
  ...
  const mutation = state.key === "UPDATE" ? Digit.Hooks.wms.sch.useWmsSchUpdate(tenantId) : Digit.Hooks.wms.sch.useWmsSchCreate(tenantId);
  ...
  const DisplayText = (action, isSuccess, isEmployee, t) => {
    if (!isSuccess) {
      ...
      return state.key === "CREATE"?"WMS_CREATE_SCH_INFO" :"";
    }
  };
