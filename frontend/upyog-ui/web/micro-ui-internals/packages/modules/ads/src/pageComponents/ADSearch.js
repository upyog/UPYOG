...
import { FormStep, CardHeader, CardLabel, Dropdown, SubmitBar, Toast, Card, RadioButtons, TextInput } from "@upyog/digit-ui-react-components";
  ...
  return <React.Fragment>
      ...
      <FormStep config={config} onSelect={goNext} onSkip={onSkip} t={t}>
        ...
        <CardLabel>
          ...
          <Controller control={control} name={"adsType"} defaultValue={adsType} rules={{
        required: t("CORE_COMMON_REQUIRED_ERRMSG")
      }} render={props => <Dropdown className="form-field" selected={adsType} select={selected => {
        setAdsType(selected);
      }} placeholder={"Select Advertisement Type"} option={ADSTypeData} optionKey="i18nKey" t={t} />} />
        <CardLabel>
          {`${t("ADS_LOCATION")}`} <span className="check-page-link-button">*</span>
        </CardLabel>
          <Controller control={control} name={"selectedLocation"} defaultValue={selectedLocation} rules={{
        required: t("CORE_COMMON_REQUIRED_ERRMSG")
      }} render={props => <Dropdown className="form-field" selected={selectedLocation} select={selected => {
        setSelectedLocation(selected);
      }} placeholder={"Select Ad Type"} option={LocationData} optionKey="i18nKey" t={t} />} />
          <CardLabel>
            {`${t("ADS_FACE_AREA")}`} <span className="check-page-link-button">*</span>
          </CardLabel>
           <Controller control={control} name={"adsType"} defaultValue={selectedFace} rules={{
        required: t("CORE_COMMON_REQUIRED_ERRMSG")
      }} render={props => <Dropdown className="form-field" selected={selectedFace} select={selected => {
        setSelectedFace(selected);
      }} placeholder={"Select Ad Type"} option={FaceId} optionKey="i18nKey" t={t} />} />
            <CardLabel>{`${t("ADS_FROM_DATE")}`} <span className="astericColor">*</span></CardLabel>
            <TextInput t={t} type={"date"} isMandatory={false} optionKey="i18nKey" name="fromDate" value={fromDate} onChange={SetFromDate} style={{
        width: user.type === "EMPLOYEE" ? "50%" : "86%"
      }} min={new Date(new Date().setDate(new Date().getDate() + 1)).toISOString().split('T')[0]} rules={{
        required: t("CORE_COMMON_REQUIRED_ERRMSG"),
        validDate: val => /^\d{4}-\d{2}-\d{2}$/.test(val) ? true : t("ERR_DEFAULT_INPUT_FIELD_MSG")
      }} />
            <CardLabel>{`${t("ADS_TO_DATE")}`} <span className="astericColor">*</span></CardLabel>
            <TextInput t={t} type={"date"} isMandatory={false} optionKey="i18nKey" name="toDate" value={toDate} onChange={SetToDate} style={{
        width: user.type === "EMPLOYEE" ? "50%" : "86%"
      }} min={new Date(new Date().setDate(new Date().getDate() + 1)).toISOString().split('T')[0]} rules={{
        required: t("CORE_COMMON_REQUIRED_ERRMSG"),
        validDate: val => /^\d{4}-\d{2}-\d{2}$/.test(val) ? true : t("ERR_DEFAULT_INPUT_FIELD_MSG")
