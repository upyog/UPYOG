...
const OBPSSearchApplication = ({ tenantId, t, onSubmit, data, error, searchData, isLoading, Count, setparamerror }) => {
  ...
  const { register, control, handleSubmit, setValue, getValues, reset, formState } = useForm({
    defaultValues: {
      applicationNo: "",
      mobileNumber: window.location.href.includes("/upyog-ui/citizen") ? currentUserPhoneNumber : "",
      fromDate: "",
      toDate: "",
      status: "",
      offset: 0,
  ...
  React.useEffect(() => {
    if (formState.isSubmitSuccessful) {
      ...
      reset({
        applicationNo: "",
       mobileNumber: window.location.href.includes("/upyog-ui/citizen") ? Digit.UserService.getUser()?.info?.mobileNumber : "",
        fromDate: "",
        toDate: "",
        status: "",
        offset: 0,
