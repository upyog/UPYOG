...
function NDCDocument({ value = {}, Code, index }) {
  ...
  return (
    <div className="cg-doc-margin-top-19">
      <React.Fragment>
        {data?.pdfFiles && (
          <div className="cg-doc-flex-wrap">
            {documents?.map((document, index) => {
              let documentLink = pdfDownloadLink(data.pdfFiles, document?.fileStoreId);
              return (
                <a target="_" href={documentLink} className="cg-doc-link" key={index}>
                  <PDFSvg width={85} height={100} className="cg-doc-pdf-bg" />
                </a>
              );
            })}
          </div>
