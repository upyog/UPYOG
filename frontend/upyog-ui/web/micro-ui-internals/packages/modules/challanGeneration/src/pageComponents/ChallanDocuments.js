...
import { CardLabel, UploadFile, Toast, LabelFieldPair, ActionBar } from "@nudmcdgnpm/digit-ui-react-components";
...
const ChallanDocuments = ({
  ...
  const validateDocuments = () => {
    ...
    data?.Challan?.Documents?.forEach((doc) => {
      const documentData = documents?.find((item) => item?.documentType === doc?.code);
      ...
      if (doc?.code === "CHALLAN.EVIDENCE_IMAGE") {
        ...
        const hasLatitude = hasValue(documentData?.latitude);

        const hasLongitude = hasValue(documentData?.longitude);

        if (!hasFile || !hasLatitude || !hasLongitude) {
          isValid = false;
        }
      }
    });
  ...
  }, [documents, data, checkRequiredFields]);
        ...
        <div>
          ...
          */}
          <ActionBar>
            ...
            <button onClick={handleSubmit} className="btn-primary" disabled={enableSubmit}>
              {t("COMMON_NEXT")}
            </button>
          </ActionBar>
        </div>
...
function CGSelectDocument({ t, document: doc, setDocuments, setError, documents, action, formData, handleSubmit, id, onSelect, config }) {
  ...
  const [selectedDocument, setSelectedDocument] = useState(() => {
    if (filteredDocument) {
      ...
      return match
        ? {
            ...match,
            i18nKey: match.code?.replaceAll(".", "_"),
          }
        : {};
    }
    ...
    if (doc?.dropdownData?.length === 1) {
      ...
      return {
        ...onlyOption,
        i18nKey: onlyOption.code?.replaceAll(".", "_"),
      };
    }
   ...
   * Add/update document.
   */
  const updateDocument = (selectedDocument, extraFields = {}) => {
    if (!selectedDocument?.code) {
      console.warn("Cannot update document: document type not selected");
      return;
    }

    setDocuments((prev = []) => {
      const existingIndex = prev.findIndex((item) => item?.documentType === selectedDocument?.code);
      ...
      try {
        ...
        if (doc?.code === "CHALLAN.EVIDENCE_IMAGE") {
          if (!hasValue(latitude) || !hasValue(longitude)) {
            console.warn("Evidence image does not contain GPS coordinates");
          } else {
            setError(null);
          }
        }
  ...
  useEffect(() => {
    if (action === "update") {
      const originalDoc = formData?.originalData?.documents?.find((e) => e.documentType?.includes(doc?.code));
      ...
      if (!docType) {
        ...
        setUploadedFile(originalDoc?.fileStoreId || originalDoc?.filestoreId || null);
        ...
        if (doc?.code === "CHALLAN.EVIDENCE_IMAGE") {
          setDocuments((prev = []) => {
            const exists = prev.some((item) => item?.documentType === originalDoc?.documentType);

            if (exists) {
              return prev.map((item) => {
                if (item?.documentType === originalDoc?.documentType) {
                  return {
                    ...item,
                    latitude: originalDoc?.latitude ?? null,
                    longitude: originalDoc?.longitude ?? null,
                  };
                }
        ...
        <div className="field cg-field-width-100">
          ...
          {doc?.code === "CHALLAN.EVIDENCE_IMAGE" && (
            <span className="challan-note-green">
              <span className="challan-note-red">{t("CHALLAN_NOTE_LABEL")}</span> {t("CHALLAN_UPLOAD_LOCATION_IMAGE")}
            </span>
          )}
          ...
          {doc?.code === "CHALLAN.EVIDENCE_IMAGE" &&
            documents
              ...
              ?.map((item, index) => {
                const hasLatitude = hasValue(item?.latitude);

                const hasLongitude = hasValue(item?.longitude);

                return (
                  <div key={index}>
                    <div className="challan-lat-long">
                      <div>
                        <b>Latitude:</b> {hasLatitude ? item.latitude : "Not available"}
                      </div>

                      <div>
                        <b>Longitude:</b> {hasLongitude ? item.longitude : "Not available"}
                      </div>
                    </div>

                    {(!hasLatitude || !hasLongitude) && <div className="challan-gps-error">{t("CHALLAN_UPLOAD_LOCATION_IMAGE")}</div>}
                  </div>
                );
              })}
        </div>
