 ...
 * SortBy Component
 * 
 * Renders a sort selector with two options: "Latest First" (DESC) and "Latest Last" (ASC).
 * Displays in a popup/modal for mobile or inline for desktop. Sorts by `createdTime` field.
 * 
 * Props:
 * - `sortParams`: Current sort parameters array
 * - `onSort`: Callback invoked when a sort option is selected
 * - `onClose`: Callback to close the popup (mobile view)
