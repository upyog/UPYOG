...
/**
 * ApplicationLinks Component
 * 
 * Renders a card displaying quick action links for the GC module (New Registration, Search Application).
 * Filters the available links based on the current user's role permissions.
 * 
 * Props:
 * - `linkPrefix`: Base URL prefix for navigation links
 * - `classNameForMobileView`: Additional CSS class for mobile view styling
 */
const ApplicationLinks = ({ linkPrefix, classNameForMobileView="" }) => {
  ...
  const GetLogo = () => (
    <div className="header">
      ...
      <span className="text">{t("ES_TITLE_GC_REGISTRATION")}</span>
    </div>
  );
