...
const ApplicationDetails = () => {
  ...
  const showTransfererDetails = React.useCallback(() => {
    if (auditData && Object.keys(appDetailsToShow).length && applicationDetails?.applicationData?.status !== "ACTIVE" && applicationDetails?.applicationData?.creationReason === "MUTATION" && !appDetailsToShow?.applicationDetails.find(e => e.title === "PT_MUTATION_TRANSFEROR_DETAILS")) {
      ...
      applicationDetails.unshift({
        ...
        belowComponent: () => <TransfererDetails userType="employee" formData={{
  ...
  if (PT_CEMP && workflowDetails?.data?.actionState?.isStateUpdatable && !workflowDetails?.data?.actionState?.nextActions?.find(e => e.action === "UPDATE")) {
  ...
  if (!(appDetailsToShow?.applicationDetails?.[0]?.values?.[0].title === "PT_PROPERTY_APPLICATION_NO")) {
    appDetailsToShow?.applicationDetails?.unshift({
      values: [{
        ...
        value: appDetailsToShow?.applicationData?.propertyId
      }, {
        title: "ES_APPLICATION_CHANNEL",
        value: `ES_APPLICATION_DETAILS_APPLICATION_CHANNEL_${appDetailsToShow?.applicationData?.channel}`
      }]
    });
  }
  if (PT_CEMP && workflowDetails?.data?.applicationBusinessService === "PT.MUTATION" && workflowDetails?.data?.actionState?.nextActions?.find(act => act.action === "PAY")) {
    workflowDetails.data.actionState.nextActions = workflowDetails?.data?.actionState?.nextActions.map(act => {
      if (act.action === "PAY") {
        return {
          ...
          redirectionUrl: {
            pathname: `/upyog-ui/employee/payment/collect/PT.MUTATION/${appDetailsToShow?.applicationData?.acknowldgementNumber}`
          }
        };
      }
      return act;
  ...
  const wfDocs = workflowDetails.data?.timeline?.reduce((acc, {
    wfDocuments
  }) => {
    return wfDocuments ? [...acc, ...wfDocuments] : acc;
  }, []);
  let appdetailsDocuments = appDetailsToShow?.applicationDetails?.find(e => e.title === "PT_OWNERSHIP_INFO_SUB_HEADER")?.additionalDetails?.documents;
  ...
  if (applicationDetails?.applicationData?.creationReason === "MUTATION") {
    return <MutationApplicationDetails propertyId={propertyId} acknowledgementIds={appDetailsToShow?.applicationData?.acknowldgementNumber} workflowDetails={workflowDetails} mutate={mutate} showToast={showToast} setShowToast={setShowToast} closeToast={closeToast} />;
  }
