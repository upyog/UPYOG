...
const SearchPTID = ({
  ...
  const columns = useMemo(() => [{
    ...
    Cell: ({
      row
    }) => GetCell(row.original.owners.additionalDetails !== null ? `${row?.original?.owners.sort((a, b) => a?.additionalDetails?.ownerSequence - b?.additionalDetails?.ownerSequence).map(ob => ob.name).join(",")}` : `${row.original.owners.map(ob => ob.name).join(",")}` || "")
  }, {
  ...
  return <React.Fragment>
    ...
    }} />} hideSubmit={true} isDisabled={false} popupStyles={{
      width: "50%",
      marginTop: "auto"
    }}>
