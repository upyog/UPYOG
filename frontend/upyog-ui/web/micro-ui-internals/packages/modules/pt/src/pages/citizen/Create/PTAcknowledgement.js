...
const PTAcknowledgement = ({
  ...
  const mutation = Digit.Hooks.pt.usePropertyAPI(data?.address?.city ? data.address?.city?.code : tenantId, !window.location.href.includes("edit-application") && !isPropertyMutation);
  const {
    data: storeData
  ...
  return mutation.isLoading || mutation.isIdle ? <Loader /> : <Card>
      ...
      {/* {mutation.isSuccess && (
        ...
        />)}*/}
      <StatusTable>
        {mutation.isSuccess && <Row rowContainerStyle={rowContainerStyle} last label={t("PT_COMMON_TABLE_COL_PT_ID")} text={mutation?.data?.Properties[0]?.propertyId} textStyle={{
        whiteSpace: "pre",
        width: "60%"
      }} />}
      </StatusTable>
