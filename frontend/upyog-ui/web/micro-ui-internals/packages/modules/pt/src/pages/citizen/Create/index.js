  ...
  return <div>
    ...
    <Switch>
      {config.map((routeObj, index) => {
          ...
          return <Route path={`${match.path}/${routeObj.route}`} key={index}>
            <Component config={{
              texts,
              inputs,
              key,
              isMandatory
            }} onSelect={handleSelect} onSkip={handleSkip} t={t} formData={params} onAdd={handleMultiple} />
          </Route>;
        })}
    ...
    <div>
      {showToast && <Modal headerBarMain={<Heading label={t("CR_PROPERTY_DUPLICATE")} />} headerBarEnd={<CloseBtn onClick={closeModal} />} actionCancelLabel={"Cancel"} actionCancelOnSubmit={closeModal} actionSaveLabel={"Proceed"} actionSaveOnSubmit={setModal} formId="modal-action">  <div className="pt-auto-105">
    <Card>
        <CardHeader>Property Details</CardHeader>
     
            <StatusTable>
                <Row label={t("CR_PROPERTY_NUMBER")} text={propertyData?.Properties?.[0]?.propertyId || "NA"} textStyle={{
                whiteSpace: "pre"
              }} />
