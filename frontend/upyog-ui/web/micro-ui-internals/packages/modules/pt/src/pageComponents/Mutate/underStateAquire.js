...
const PTPropertyUnderStateAquire = ({
  ...
  if (userType === "employee") {
    return <React.Fragment>
        ...
        <LabelFieldPair>
          ...
          <div className="field">
            <TextInput disable={isPropertyUnderGovtPossession?.code !== "YES"} value={govtAcquisitionDetails} onChange={e => setReason(e.target.value)} />
          </div>
        </LabelFieldPair>
      </React.Fragment>;
  }
  return <React.Fragment>
      ...
      <FormStep t={t} config={config} onSelect={goNext} onSkip={onSkip} isDisabled={!isPropertyUnderGovtPossession?.code}>
        <RadioButtons t={t} optionsKey="i18nKey" isMandatory={config.isMandatory} options={menu} selectedOption={isPropertyUnderGovtPossession} onSelect={v => {
        setSelected(v);
