import { Card, CardSubHeader, EditIcon, Header, LinkButton, Loader, PopUp, Row, StatusTable, SubmitBar, LinkLabel } from "@upyog/digit-ui-react-components";
...
const PropertyInformation = () => {
  ...
  const selectedAction = {
    ...
    customFunctionToExecute: data => {
      ...
      history.replace({
        pathname: `/upyog-ui/citizen/pt/assessment-details/${property.propertyId}`,
        state: {
          ...data
        }
      });
      ...
      <div>
        <Card>
          <StatusTable>
            ...
            <LinkLabel onClick={() => history.push({
            pathname: `/upyog-ui/citizen/pt/payment-details/${property?.propertyId}`
          })} style={isMobile ? {
            marginTop: "15px",
            marginLeft: "0px"
          } : {
          ...
          <div>
            {Array.isArray(units) && units.length > 0 && units.map((unit, index) => <div key={index}>
                  {(flrno !== unit?.floorNo ? i = 1 : i = i + 1) && i === 1 && <CardSubHeader>{t(`PROPERTYTAX_FLOOR_${unit?.floorNo}`)}</CardSubHeader>}
                  <div className="pt-auto-122">
                    <CardSubHeader>
                      {t("ES_APPLICATION_DETAILS_UNIT")} {i}
                    </CardSubHeader>
                    {(flrno = unit?.floorNo) > -5 && <StatusTable>
                        <Row className="border-none" label={t("PT_ASSESSMENT_UNIT_USAGE_TYPE")} text={`${t((property.usageCategory !== "RESIDENTIAL" ? "COMMON_PROPUSGTYPE_NONRESIDENTIAL_" : "COMMON_PROPUSGTYPE_") + (property?.usageCategory?.split(".")[1] ? property?.usageCategory?.split(".")[1] : property.usageCategory)
                /* (property.usageCategory !== "RESIDENTIAL" ? "_" + unit?.usageCategory.split(".").pop() : "") */)}` || t("CS_NA")} />
                        ...
                        {unit.occupancyType == "RENTED" && <Row className="border-none" label={t("PT_FORM2_TOTAL_ANNUAL_RENT")} text={`${unit?.arv && `₹${unit?.arv}` || t("CS_NA")}`} />}
                      </StatusTable>}
                  </div>
                </div>)}
          </div>
          ...
          <div className="owner-details">
            {Array.isArray(owners) && owners.sort((item, item2) => {
            return item?.additionalDetails?.ownerSequence - item2?.additionalDetails?.ownerSequence;
          }).map((owner, index) => <div key={index} className="owner-details-child">
                  <CardSubHeader>
                  ...
                  {property?.institution && property?.institution?.type && <Row className="border-none" label={t("PT_INSTITUTION_TYPE")} text={`${t(`COMMON_MASTERS_OWNERSHIPCATEGORY_${property?.institution?.type}`) || t("CS_NA")}`} />}
                    <Row className="border-none" label={t("PT_COMMON_APPLICANT_NAME_LABEL")} textStyle={isMobile ? {
                marginLeft: "27%",
                marginRight: "5%",
                wordBreak: "break-word"
              } : {
                marginLeft: "18%"
              }} text={`${owner?.name || t("CS_NA")}`} actionButtonStyle={{
                marginRight: "-10px"
              }} actionButton={<ActionButton jumpTo={`/upyog-ui/citizen/pt/property/owner-history/${property.tenantId}/${property.propertyId}`} className="pt-auto-123" />} />
                    ...
                    <Row className="border-none" label={t("PT_FORM3_MOBILE_NUMBER")} text={`${t(owner?.mobileNumber)}` || t("CS_NA")} textStyle={isMobile ? {
                marginLeft: "16%"
              } : {
                marginLeft: "12%"
              }} actionButton={property?.status === "ACTIVE" && owner?.mobileNumber && Digit.UserService.getUser()?.info?.mobileNumber && owner.mobileNumber === Digit.UserService.getUser()?.info?.mobileNumber && <div onClick={() => showPopup({
                name: owner?.name,
                ...
                ownerIndex: index
                    ...
                    <Row className="border-none" label={t("PT_FORM3_OWNERSHIP_TYPE")} text={`${property?.ownershipCategory ? t(`PT_OWNERSHIP_${property?.ownershipCategory}`) : t("CS_NA")}`} />
          ...
          {property?.status === "ACTIVE" && !enableAudit && <div className="pt-auto-125">               
            <button className="submit-bar pt-auto-126" type="button" onClick={handleClickOnPtPgr}>{t("PT_PGR")}</button>
            </div>}
            {property?.status === "ACTIVE" && !enableAudit && <div className="pt-auto-127">
                <Link to={{
              pathname: `/upyog-ui/citizen/pt/property/edit-application/action=UPDATE/${property.propertyId}`
            }}>
                  <SubmitBar label={t("PT_UPDATE_PROPERTY_BUTTON")} />
                </Link>
              </div>}
            {property?.status === "ACTIVE" && !enableAudit && <div className="pt-auto-128">
          ...
          {popup && <PopUp className="updatenumber-warper-citizen">
              <UpdatePropertyNumberComponent showPopup={showPopup} name={popup?.name} UpdateNumberConfig={UpdateNumberConfig} mobileNumber={popup?.mobileNumber} t={t} onValidation={(data, showToast) => {
            let newProp = {
              ...property
            };
           ...
           {showModal ? <ActionModal t={t} action={selectedAction} tenantId={tenantId} state={state} id={property.propertyId} applicationDetails={property} applicationData={property} closeModal={closeModal} submitAction={submitAction} businessService={businessService} moduleCode={moduleCode} /> : null}
        </Card>
      </div>
    </React.Fragment>;
};
