...
const MutationApplicationDetails = ({
  ...
  const showTransfererDetails = () => {
    if (auditResponse && Object.keys(appDetailsToShow).length && applicationDetails?.applicationData?.status !== "ACTIVE" && applicationDetails?.applicationData?.creationReason === "MUTATION" && !appDetailsToShow?.applicationDetails.find(e => e.title === "PT_MUTATION_TRANSFEROR_DETAILS")) {
  ...
  if (data?.Properties?.[0]?.creationReason === "MUTATION" && data?.Properties?.[0]?.status === "ACTIVE") dowloadOptions.push({
