...
const BillDetails = ({ paymentRules, businessService }) => {
  ...
  const onSubmit = () => {
    ...
    if (window.location.href.includes("mcollect")) {
      ...
      history.push(`/upyog-ui/citizen/payment/collect/${businessService}/${consumerCode}?workflow=WNS`, {
        paymentAmount,
        tenantId: billDetails.tenantId,
      });
      ...
      history.push(`/upyog-ui/citizen/payment/collect/${businessService}/${consumerCode}`, { paymentAmount, tenantId: billDetails.tenantId });
    }
  };
