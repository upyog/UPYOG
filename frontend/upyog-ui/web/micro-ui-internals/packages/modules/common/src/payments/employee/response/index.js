...
export const SuccessfulPayment = (props) => {
  ...
  const printNDCReceipt = async () => {
    ...
    const paymentResponse = await Digit.PaymentService.getReciept(tenantId, businessService, { receiptNumbers: receiptNumber });
    let fileStoreId = paymentResponse?.Payments?.[0]?.fileStoreId;
    if (!fileStoreId) {
      const payObj = paymentResponse?.Payments?.[0] || {};
