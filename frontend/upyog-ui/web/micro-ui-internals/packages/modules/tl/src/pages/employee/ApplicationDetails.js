...
const ApplicationDetails = () => {
  ...
  } = Digit.Hooks.tl.useTLPaymentHistory(tenantId, applicationDetails?.applicationData?.applicationNumber);
  useEffect(() => {
    if (applicationDetails?.numOfApplications?.length > 0) {
      ...
      const latestFinancialYear = Math.max.apply(Math, applicationDetails?.numOfApplications?.filter(ob => ob.licenseNumber === applicationDetails?.applicationData?.licenseNumber)?.map(function (o) {
  ...
  const fetchDigiLockerDocuments = async (file, id, res) => {
    ...
    const res1 = await Digit.DigiLockerService.pdfUrl({
      TokenReq
    });
    ...
    try {
      ...
      try {
        ...
        if (digiRes?.Transaction?.length && digiRes.Transaction[0]?.signedFilestoreId) {
          ...
          const fileRes = await Digit.UploadServices.Filefetch([digiRes.Transaction[0].signedFilestoreId], tenant);
