...
const TradeUnitForm = _props => {
  ...
  return <React.Fragment>
            ...
            autoFocus={focusIndex.index === unit?.key && focusIndex.type === "uom"} errorStyle={localFormState.touched.uom && errors?.uom?.message ? true : false} onChange={e => {
            ...
            }} disable={true} onBlur={props.onBlur} className="tl-auto-142" />} />
                        </div>
                    </LabelFieldPair>
              ...
              required: t("REQUIRED_FIELD"),
              validate: {
                pattern: val => (/*/^(0)*[1-9][0-9]{0,5}$/.test(val)*/val > 0 && val < 99999 ? checkRangeForUomValue(val, unit?.tradeSubType?.fromUom, unit?.tradeSubType?.toUom) ? true : `${t("ERR_WRONG_UOM_VALUE")} ${unit?.tradeSubType?.fromUom} - ${unit?.tradeSubType?.toUom}` : t("ERR_DEFAULT_INPUT_FIELD_MSG"))
              }
            ...
            }} disable={!unit?.tradeSubType?.uom} onBlur={props.onBlur} className="tl-auto-143" />} />
                        </div>
                    </LabelFieldPair>
