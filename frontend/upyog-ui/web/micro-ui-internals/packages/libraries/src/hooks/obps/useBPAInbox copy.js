...
const useBPAInbox = ({ tenantId, filters, config = {} }) => {
  ...
  const _filters = {
    ...
    moduleSearchCriteria: {
      ...
      ...(sortOrder ? { sortOrder } : {}),
      ...(locality?.length > 0
        ? { locality: locality.map((item) => item.code.split("_").pop()).join(",") }
        : {}),
    },
    sortBy,
    limit,
  ...
  return queryTemplate({
    queryKey: ["BPA_INBOX", tenantId, _filters],
    ...
    queryFn: async () => {
      ...
      const promises = data?.items?.map((application) => {
        const edcrFilter = {
          edcrNumber: application?.businessObject?.edcrNumber,
        };
        return Search.scrutinyDetails(tenantId, edcrFilter);
      });
      ...
      data.items = data?.items?.map((application) => ({
        ...
        edcr:
          edcrData?.find(
            (edcr) =>
              edcr?.edcrNumber === application?.businessObject?.edcrNumber
          ) || {},
      }));
