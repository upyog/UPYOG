...
export const NOCSearch = {
  ...
  applicationDetails: async (t, tenantId, applicationNumber, userType) => {
    ...
    const buildingDetails = {
      ...
      values: [
        { title: "NOC_APP_NO_LABEL", value: <div><Link to={`/upyog-ui/employee/obps/bpa/${bpaResponse?.[0]?.applicationNo}`}><span className="link noc-search-link">{bpaResponse?.[0]?.applicationNo}</span></Link></div> },
