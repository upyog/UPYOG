...
export const Search = {
  scrutinyDetails: async (tenantId, params, data, all = false) => {
    const response = await WMSService.scrutinyDetails(tenantId, params, data);
  ...
  NOCDetails: async (tenantId, params) => {
    const response = await WMSService.NOCSearch(tenantId, params);
    return response?.Noc;
  },
  BPADetails: async (tenantId, params) => {
    const response = await WMSService.BPASearch(tenantId, params);
    return response?.BPA;
  }
} 
