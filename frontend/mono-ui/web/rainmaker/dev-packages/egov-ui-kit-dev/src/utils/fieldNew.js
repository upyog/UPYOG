import { AutoSuggestDropdown, DropDown, Label, SingleCheckbox, TextField, TextFieldIcon } from "components";
import { AutosuggestContainer } from "egov-ui-framework/ui-containers";
import React from "react";
import { RadioButton } from "../components";
import MobileNumberFieldNew from "../components/MobileNumberFieldNew";
import TextFieldNew from "../components/TextFieldNew";
import UsernameFieldWithIcon from "../components/UsernameFieldWithIcon";
...
const FieldNew = ({ fieldKey, handleFieldChange, field = {}, disabled, onTextFieldIconClick, ...rest }) => {
  const renderField = () => {
    ...
    switch (type) {
      case "textfield":
      case "textarea":
        return <TextFieldNew {...rest} {...fieldProps} onChange={(e, value) => handleFieldChange(fieldKey, value)} multiLine={type === "textarea"} disabled={disabled || fieldProps.disabled} />;
      case "mobilenumber":
        return <MobileNumberFieldNew {...rest} {...fieldProps} onChange={(e, value) => handleFieldChange(fieldKey, value)} disabled={disabled || fieldProps.disabled} />;
      case "number":
      case "password":
        return <UsernameFieldWithIcon {...rest} {...fieldProps} type={type} onChange={(e, value) => handleFieldChange(fieldKey, value)} disabled={disabled || fieldProps.disabled} />;
      case "checkbox":
        return (
