...
export const AppModules = ({ stateCode, userType, modules, appTenants }) => {
  ...
  if (!user || !user?.access_token || !user?.info) {
    return (
      <Navigate to="/cnd-ui/employee/user/login" state={{ from: location.pathname + location.search }} replace />
    );
  }
  ...
  const appRoutes = modules.map(({ code, tenants }, index) => {
    ...
    return Module ? (
      ...
      <Route
        key={index}
        path={`${code.toLowerCase()}`}
        element={<Navigate to="/cnd-ui/employee/user/error?type=notfound" state={{ from: location.pathname + location.search }} replace />}
      />
    );
  });
  ...
  return (
    <div className="ground-container">
      <Routes>
        {appRoutes}
        <Route path="login" element={<Navigate to="/cnd-ui/employee/user/login" state={{ from: location.pathname + location.search }} replace />} />
