...
import CNDSearchApplication from "../../components/CNDSearchApplication";
...
const SearchApp = ({path}) => {
    ...
    function onSubmit (_data) {
        ...
        if(Object.entries(payload).length>0 && !payload.applicationNumber && !payload.fromDate && !payload.mobileNumber && !payload.toDate)
        setShowToast({ warning: true, label: "ES_ERROR_INVALID_DETAILS" });
    ...
    const { isLoading, isSuccess, isError, error, data: {cndApplicationDetail: searchResult, count: count} = {} } = Digit.Hooks.cnd.useCndSearchApplication(
        { 
          tenantId,
          filters: payload
        },
