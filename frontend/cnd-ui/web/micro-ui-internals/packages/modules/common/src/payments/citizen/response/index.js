    ...
    return (
      <Card>
        ...
        {/* {!(business_service?.includes("PT")) ? (
          ...
          <React.Fragment>
            {/* <Link to={(applicationNo && `/cnd-ui/citizen/payment/my-bills/${business_service}/${applicationNo}`) || "/cnd-ui/citizen"}>
              <SubmitBar label={t("CS_PAYMENT_TRY_AGAIN")} />
            </Link> */}
            {/* {business_service?.includes("PT") &&<div style={{marginTop:"10px"}}><Link to={`/cnd-ui/citizen/feedback?redirectedFrom=${"cnd-ui/citizen/payment/success"}&propertyId=${consumerCode? consumerCode : ""}&acknowldgementNumber=${egId ? egId : ""}&tenantId=${tenantId}&creationReason=${business_service?.split(".")?.[1]}`}>
