...
> Generated from repository path `utilities/data-upload`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Generic data upload utility.

## Responsibilities

- Own the `data-upload` business or platform capability within the UPYOG ecosystem.
...
| dataupload | 1 source file(s) | Package area detected from source tree. |
| extensions | 3 source file(s) | Package area detected from source tree. |
...
- `utilities/data-upload`: service root.
...
- `utilities/data-upload/src/main/java/egov/dataupload/DataUploadMain.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /{service}/{mapping}/_upload | DataUploadController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
- `utilities/data-upload/src/main/java/egov/dataupload/repository/ServiceRequestRepository.java`
...
- `utilities/data-upload/src/main/java/egov/dataupload/producer/Producer.java`
...
| --- | --- |
| egov.filestore.host | http://localhost:8096 |
...
- `utilities/data-upload/src/main/resources/application.properties`

### Key properties

| Property | Value / meaning |
| --- | --- |
| server.servlet.contextPath | /data-upload |
| server.port | 9001 |
| app.timezone | UTC |
| egov.root.tenant.id | pb |
| config.repo.path | classpath:case-management.yaml |
| egov.filestore.host | http://localhost:8096 |
