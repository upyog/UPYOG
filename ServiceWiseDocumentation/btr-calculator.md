...
> Generated from repository path `tutorials/backend-developer-guide/btr-calculator`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Birth registration tutorial calculator.

## Responsibilities

- Own the `btr-calculator` business or platform capability within the UPYOG ecosystem.
...
- `tutorials/backend-developer-guide/btr-calculator`: service root.
...
- `tutorials/backend-developer-guide/btr-calculator/src/main/java/digit/Main.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /v1/_calculate | V1ApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/_getbill | V1ApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as btr_calculator
...
- `tutorials/backend-developer-guide/btr-calculator/src/main/java/digit/repository/ServiceRequestRepository.java`
...
- `tutorials/backend-developer-guide/btr-calculator/src/main/resources/application.properties`

### Key properties

| Property | Value / meaning |
| --- | --- |
| server.servlet.contextPath | /birth-calculator |
| server.port | 2345 |
| app.timezone | UTC |
| egov.mdms.host | https://dev.digit.org |
| egov.mdms.search.endpoint | /egov-mdms-service/v1/_search |
...
| btr.taxhead.master.code | BIRTH_CERT_FEE |
| btr.module.code | BIRTH_CERT |
