...
> Generated from repository path `core-services/egov-enc-service`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.
...
- Own the `egov-enc-service` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **1234**
- Servlet/context path: **/egov-enc-service**
...
| enc | 1 source file(s) | Package area detected from source tree. |
| keymanagement | 3 source file(s) | Package area detected from source tree. |
| masterkey | 1 source file(s) | Package area detected from source tree. |
| model | 16 source file(s) | Request, response, DTO, and domain models. |
| providers | 2 source file(s) | Package area detected from source tree. |
...
- `core-services/egov-enc-service`: service root.
...
- `core-services/egov-enc-service/src/main/java/org/egov/enc/Main.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /crypto/v1/_encrypt | CryptoApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /crypto/v1/_decrypt | CryptoApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /crypto/v1/_sign | CryptoApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /crypto/v1/_verify | CryptoApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /crypto/v1/_rotateallkeys | CryptoApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /crypto/v1/_rotatekey | CryptoApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as egov_enc_service
...
- **Tables detected from migrations:** eg_enc_asymmetric_keys_audit, eg_enc_symmetric_keys_audit, public."eg_enc_asymmetric_keys, public."eg_enc_symmetric_keys
- **Migration files:** 2
...
- `core-services/egov-enc-service/src/main/resources/db/migration`
- `core-services/egov-enc-service/src/main/resources/db/migration/main`

### Repository locations

- `core-services/egov-enc-service/src/main/java/org/egov/enc/repository/AuditRepository.java`
- `core-services/egov-enc-service/src/main/java/org/egov/enc/repository/KeyRepository.java`
...
| master.password.provider | <secret-value> |
