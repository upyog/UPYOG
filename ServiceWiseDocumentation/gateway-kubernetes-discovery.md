...
> Generated from repository path `utilities/gateway-kubernetes-discovery`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

gateway-kubernetes-discovery module in the UPYOG platform.

## Responsibilities

- Own the `gateway-kubernetes-discovery` business or platform capability within the UPYOG ecosystem.
...
- `utilities/gateway-kubernetes-discovery`: service root.
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as gateway_kubernetes_discovery
...
- `utilities/gateway-kubernetes-discovery/Dockerfile`
