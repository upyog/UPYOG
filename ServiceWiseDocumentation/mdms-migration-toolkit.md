...
> Generated from repository path `utilities/mdms-migration-toolkit`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

MDMS v1 to v2 migration toolkit.

## Responsibilities

- Own the `mdms-migration-toolkit` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8080**
- Servlet/context path: **/mdms-migration-toolkit**
...
- `utilities/mdms-migration-toolkit`: service root.
...
- `utilities/mdms-migration-toolkit/src/main/java/digit/Main.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /data/v1/_migrate | DataApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /schema/v1/_generate | SchemaApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /schema/v1/_migrate | SchemaApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as mdms_migration_toolkit
...
- `utilities/mdms-migration-toolkit/src/main/java/digit/repository/ServiceRequestRepository.java`
...
| spring.kafka.consumer.group-id | mdms-migration-toolkit |
...
- `utilities/mdms-migration-toolkit/src/main/java/digit/kafka/Producer.java`

### Consumers

- `utilities/mdms-migration-toolkit/src/main/java/digit/kafka/Consumer.java`
...
| masters.config.url | https://raw.githubusercontent.com/egovernments/egov-mdms-data/DEV/master-config.json |
...
- `utilities/mdms-migration-toolkit/src/main/resources/application.properties`

### Key properties

| Property | Value / meaning |
| --- | --- |
| server.contextPath | /mdms-migration-toolkit |
| server.servlet.context-path | /mdms-migration-toolkit |
| server.port | 8080 |
| app.timezone | UTC |
