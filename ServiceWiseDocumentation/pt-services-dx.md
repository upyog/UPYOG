...
> Generated from repository path `dx-services/pt-services-dx`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Property Tax data-exchange service.

## Responsibilities

- Own the `pt-services-dx` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8280**
- Servlet/context path: **/pt-services-dx**
...
| model | 42 source file(s) | Request, response, DTO, and domain models. |
...
- `dx-services/pt-services-dx`: service root.
...
- `dx-services/pt-services-dx/src/main/java/org/egov/dx/PTServicesDxApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /dataexchange/_searchReceipt | DataExchangeController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /user/authorization/url | UserController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /user/token | UserController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /user/details | UserController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /user/issuedfiles | UserController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /user/file/{id} | UserController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as pt_services_dx
...
- `dx-services/pt-services-dx/src/main/java/org/egov/dx/repository/Repository.java`
...
| --- | --- |
| egov.services.hostname | http://pdf-service:8080/ |
| egov.collectionservice.host | http://localhost:8082/ |
| egov.payment.search.endpoint | collection-services/payments/PT/_search |
| egov.filestore.host | http://localhost:8084/ |
| egov.filestore.search.endpoint | filestore/v1/files/url?tenantId=pg&fileStoreIds= |
| egov.mdms.host | http://egov-mdms-service:8080/ |
| egov.mdms.search.endpoint | egov-mdms-service/v1/_search |
| user.service.host | http://localhost:8081/ |
| user.search.endpoint | <secret-value> |
| authorization.url | https://digilocker.meripehchaan.gov.in/public/oauth2/1/authorize |
| pt.redirect.url | https://upyog-test.niua.org/digit-ui/citizen/pt/property/new-application/info |
...
- `dx-services/pt-services-dx/src/main/resources/application.properties`
