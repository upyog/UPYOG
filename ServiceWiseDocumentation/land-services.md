...
> Generated from repository path `municipal-services/land-services`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Land registry and land application service.

## Responsibilities

- Own the `land-services` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8199**
- Servlet/context path: **/land-services**
...
| land | 1 source file(s) | Package area detected from source tree. |
...
- `municipal-services/land-services`: service root.
...
- `municipal-services/land-services/src/main/java/org/egov/land/LandServicesApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /v1/land/_create | LandController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/land/_update | LandController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/land/_search | LandController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as land_services
...
- **Tables detected from migrations:** eg_land_Address, eg_land_GeoLocation, eg_land_address_auditdetails, eg_land_document, eg_land_geolocation_auditdetails, eg_land_institution, eg_land_institution_auditdetails, eg_land_landInfo, eg_land_ownerInfo, eg_land_owner_auditdetails, eg_land_unit, eg_land_unit_auditdetails, public.eg_land_auditdetails
- **Migration files:** 4
...
- `municipal-services/land-services/src/main/resources/db/migration`
- `municipal-services/land-services/src/main/resources/db/migration/main`

### Repository locations

- `municipal-services/land-services/src/main/java/org/egov/land/repository/LandRepository.java`
- `municipal-services/land-services/src/main/java/org/egov/land/repository/ServiceRequestRepository.java`
...
| municipal-services/land-services/src/main/resources/land-persister.yml topic | save-landinfo |
| municipal-services/land-services/src/main/resources/land-persister.yml topic | update-landinfo |
| municipal-services/land-services/src/main/resources/land-persister.yml topic | save-landinfo |
| municipal-services/land-services/src/main/resources/land-persister.yml topic | update-landinfo |

### Producers

- `municipal-services/land-services/src/main/java/org/egov/land/producer/Producer.java`
...
| --- | --- |
| spring.flyway.url | jdbc:postgresql://localhost:5432/land |
...
- `municipal-services/land-services/src/main/resources/application.properties`
- `municipal-services/land-services/src/main/resources/land-persister.yml`
...
| spring.datasource.url | jdbc:postgresql://localhost:5432/land |
| spring.datasource.username | land |
| spring.datasource.password | <secret-value> |
| spring.datasource.platform | postgresql |
| spring.flyway.url | jdbc:postgresql://localhost:5432/land |
| spring.flyway.user | land |
...
- `municipal-services/land-services/src/main/resources/db/Dockerfile`
