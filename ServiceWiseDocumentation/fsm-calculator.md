...
> Generated from repository path `municipal-services/fsm-calculator`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

FSM fee calculator and billing slab service.

## Responsibilities

- Own the `fsm-calculator` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8099**
- Servlet/context path: **/fsm-calculator**
...
| --- | --- | --- |
| broker | 2 source file(s) | Package area detected from source tree. |
...
| demand | 6 source file(s) | Package area detected from source tree. |
| mapper | 1 source file(s) | DTO/entity conversion. |
| model | 27 source file(s) | Request, response, DTO, and domain models. |
...
- `municipal-services/fsm-calculator`: service root.
...
- `municipal-services/fsm-calculator/src/main/java/org/egov/fsm/calculator/CalculatorApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /v1/billingSlab/_create | BillingSlabController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/billingSlab/_update | BillingSlabController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/billingSlab/_search | BillingSlabController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/_calculate | CalculatorController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/_advancebalancecalculate | CalculatorController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/_cancellationfee | CalculatorController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/_estimate | CalculatorController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as fsm_calculator
...
- **Tables detected from migrations:** eg_billing_slab, eg_billing_slab_auditlog
- **Migration files:** 2
...
- `municipal-services/fsm-calculator/src/main/resources/db/migration`
- `municipal-services/fsm-calculator/src/main/resources/db/migration/main`
- `municipal-services/fsm-calculator/src/main/resources/db/migration/seed`

### Repository locations

- `municipal-services/fsm-calculator/src/main/java/org/egov/fsm/calculator/repository/BillingSlabRepository.java`
- `municipal-services/fsm-calculator/src/main/java/org/egov/fsm/calculator/repository/DemandRepository.java`
- `municipal-services/fsm-calculator/src/main/java/org/egov/fsm/calculator/repository/ServiceRequestRepository.java`
...
- `municipal-services/fsm-calculator/src/main/java/org/egov/fsm/calculator/kafka/broker/BillingSlabProducer.java`
- `municipal-services/fsm-calculator/src/main/java/org/egov/fsm/calculator/kafka/broker/CalculatorProducer.java`
...
- `municipal-services/fsm-calculator/src/main/resources/application.properties`
...
- `municipal-services/fsm-calculator/src/main/resources/db/Dockerfile`
