...
> Generated from repository path `core-services/gis-service`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

GIS and geospatial integration service.

## Responsibilities

- Own the `gis-service` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8291**
- Servlet/context path: **/gis-service**
...
| --- | --- | --- |
| adapters | 2 source file(s) | Package area detected from source tree. |
...
| gis | 1 source file(s) | Package area detected from source tree. |
| interfaces | 1 source file(s) | Package area detected from source tree. |
...
- `core-services/gis-service`: service root.
...
- `core-services/gis-service/src/main/java/org/egov/gis/GisServiceApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /gis/v1/{businessService}/points/_search | GisController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /gis/v1/{businessService}/polygons/_search | GisController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| GET | /gis/v1/business-services | GisController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
- `core-services/gis-service/src/main/resources/db/migration`
- `core-services/gis-service/src/main/resources/db/migration/main`

### Repository locations

- `core-services/gis-service/src/main/java/org/egov/gis/repository/ServiceRequestRepository.java`
...
- `core-services/gis-service/src/main/resources/application.properties`
...
| spring.application.name | gis-service |
| egov.property.host | http://localhost:8280/ |
| egov.property.search.endpoint | property-services/property/_search |
| egov.asset.host | http://localhost:8281/ |
| egov.asset.search.endpoint | asset-services/v1/assets/_search |
