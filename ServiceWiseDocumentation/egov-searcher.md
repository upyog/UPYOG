...
> Generated from repository path `core-services/egov-searcher`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

YAML-driven search facade over database queries.

## Responsibilities

- Own the `egov-searcher` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8093**
- Servlet/context path: **/egov-searcher**
...
| impl | 10 source file(s) | Package area detected from source tree. |
...
- `core-services/egov-searcher`: service root.
...
- `core-services/egov-searcher/src/main/java/org/egov/InfraSearchApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /{moduleName}/{searchName}/_get | SearchController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as egov_searcher
...
- `core-services/egov-searcher/src/main/java/org/egov/search/repository/SearchRepository.java`
