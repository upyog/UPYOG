...
> Generated from repository path `business-services/dashboard-analytics`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.
...
- Own the `dashboard-analytics` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8289**
- Servlet/context path: **/dashboard-analytics**
...
| --- | --- | --- |
| analytics | 2 source file(s) | Package area detected from source tree. |
| config | 5 source file(s) | Spring beans, properties, and runtime configuration. |
| constant | 3 source file(s) | Package area detected from source tree. |
| controller | 1 source file(s) | HTTP endpoints and request/response orchestration. |
| dao | 1 source file(s) | Package area detected from source tree. |
| dto | 21 source file(s) | Package area detected from source tree. |
| enums | 1 source file(s) | Package area detected from source tree. |
| exception | 1 source file(s) | Custom exceptions and handlers. |
| handler | 13 source file(s) | Package area detected from source tree. |
| helper | 10 source file(s) | Package area detected from source tree. |
| impl | 6 source file(s) | Package area detected from source tree. |
...
- `business-services/dashboard-analytics`: service root.
...
- `business-services/dashboard-analytics/src/main/java/com/tarento/analytics/AnalyticApp.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| DELETE | / | DashboardController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| GET | / | DashboardController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| GET | /{dashboardId} | DashboardController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as dashboard_analytics
...
- `business-services/dashboard-analytics/src/main/java/com/tarento/analytics/repository/ElasticSearchRepository.java`
...
- `business-services/dashboard-analytics/src/main/resources/application.properties`
...
| spring.autoconfigure.exclude | org.springframework.boot.autoconfigure.jdbc.DataSourceAutoConfiguration, org.springframework.boot.autoconfigure.orm.j... |
...
| egov.es.password | <secret-value> |
| aws.access.key.id |  |
| aws.access.key.secret | <secret-value> |
