...
> Generated from repository path `utilities/kuberhealthy-checks`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

kuberhealthy-checks module in the UPYOG platform.

## Responsibilities

- Own the `kuberhealthy-checks` business or platform capability within the UPYOG ecosystem.
...
- `utilities/kuberhealthy-checks`: service root.
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as kuberhealthy_checks
...
- `utilities/kuberhealthy-checks/Dockerfile`
