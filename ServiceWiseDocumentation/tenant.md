...
> Generated from repository path `core-services/tenant`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Tenant and city registry service.

## Responsibilities

- Own the `tenant` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8092**
- Servlet/context path: **/tenant**
- Detected controllers/API mappings: **3**
- Detected migrations: **15**
- Detected tests: **31** files

## Packages

| Package area | Files | Role |
| --- | --- | --- |
| adapter | 4 source file(s) | Package area detected from source tree. |
| contract | 14 source file(s) | Package area detected from source tree. |
...
| exception | 3 source file(s) | Custom exceptions and handlers. |
...
- `core-services/tenant`: service root.
...
- `core-services/tenant/src/main/java/org/egov/EgovTenantApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /v1/tenant/_search | TenantController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/tenant/_create | TenantController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/tenant/_update | TenantController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
- **Tables detected from migrations:** city, tenant
- **Migration files:** 15
...
- `core-services/tenant/src/main/resources/db/migration`
- `core-services/tenant/src/main/resources/db/migration/dev`
- `core-services/tenant/src/main/resources/db/migration/main`
- `core-services/tenant/src/main/resources/db/migration/qa`
- `core-services/tenant/src/main/resources/db/migration/seed`

### Repository locations

- `core-services/tenant/src/main/java/org/egov/tenant/persistence/repository/CityRepository.java`
- `core-services/tenant/src/main/java/org/egov/tenant/persistence/repository/MdmsRepository.java`
- `core-services/tenant/src/main/java/org/egov/tenant/persistence/repository/TenantRepository.java`
...
- `core-services/tenant/src/main/resources/application.properties`
