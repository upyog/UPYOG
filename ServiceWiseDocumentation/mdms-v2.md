...
> Generated from repository path `core-services/mdms-v2`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Schema-backed master-data management service.

## Responsibilities

- Own the `mdms-v2` business or platform capability within the UPYOG ecosystem.
...
| enrichment | 2 source file(s) | Package area detected from source tree. |
| errors | 1 source file(s) | Package area detected from source tree. |
...
- `core-services/mdms-v2`: service root.
...
- `core-services/mdms-v2/src/main/java/org/egov/MDMSApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /v1/_search | MDMSController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v2/_create/{schemaCode} | MDMSControllerV2.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v2/_search | MDMSControllerV2.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v2/_update/{schemaCode} | MDMSControllerV2.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /schema/v1/_create | SchemaDefinitionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /schema/v1/_search | SchemaDefinitionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /schema/v1/_update | SchemaDefinitionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
- **Tables detected from migrations:** eg_mdms_data, eg_mdms_schema_definition
- **Migration files:** 2
...
- `core-services/mdms-v2/src/main/resources/db/migration`
- `core-services/mdms-v2/src/main/resources/db/migration/main`

### Repository locations

- `core-services/mdms-v2/src/main/java/org/egov/infra/mdms/repository/MdmsDataRepository.java`
- `core-services/mdms-v2/src/main/java/org/egov/infra/mdms/repository/SchemaDefinitionRepository.java`
- `core-services/mdms-v2/src/main/java/org/egov/infra/mdms/repository/impl/MdmsDataRepositoryImpl.java`
- `core-services/mdms-v2/src/main/java/org/egov/infra/mdms/repository/impl/SchemaDefinitionDbRepositoryImpl.java`
...
| core-services/mdms-v2/src/main/resources/mdms-persister.yml topic | save-mdms-schema-definition |
| core-services/mdms-v2/src/main/resources/mdms-persister.yml topic | save-mdms-data |
| core-services/mdms-v2/src/main/resources/mdms-persister.yml topic | save-mdms-schema-definition |
| core-services/mdms-v2/src/main/resources/mdms-persister.yml topic | save-mdms-data |
...
| spring.flyway.url | jdbc:postgresql://localhost:5432/microdb |
...
- `core-services/mdms-v2/src/main/resources/application.properties`
- `core-services/mdms-v2/src/main/resources/mdms-persister.yml`
