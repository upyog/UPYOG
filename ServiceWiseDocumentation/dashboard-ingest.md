...
> Generated from repository path `business-services/dashboard-ingest`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Dashboard data ingest and transformation pipeline.

## Responsibilities

- Own the `dashboard-ingest` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8080**
- Servlet/context path: **/dashboard-ingest**
...
| consumer | 9 source file(s) | Kafka/event consumers. |
| controller | 2 source file(s) | HTTP endpoints and request/response orchestration. |
| ingestpipeline | 2 source file(s) | Package area detected from source tree. |
| model | 17 source file(s) | Request, response, DTO, and domain models. |
| producer | 4 source file(s) | Kafka/event producers. |
| repository | 2 source file(s) | Database or remote-service data access. |
| service | 15 source file(s) | Business orchestration and domain logic. |
| util | 12 source file(s) | Reusable helpers and cross-cutting functions. |

## Folder Structure

- `business-services/dashboard-ingest`: service root.
...
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/IngestApp.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /update/publish | ProducerController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /update/collection/test | ProducerController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| GET | /pause/{consumerId} | RestApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| GET | /resume/{consumerId} | RestApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /migrate/{indexName}/{version} | RestApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| GET | /migrateV2/{indexName}/{version} | RestApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as dashboard_ingest
...
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/repository/ElasticSearchRepository.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/repository/TargetDataDao.java`

### Entity mapping locations

- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/model/TargetData.java`
...
| kafka.topics.enriched.key | <secret-value> |
| kafka.topics.bypass.update.data | egov-dss-update-receive |
| kafka.topics.bypass.update.post | egov-dss-update-post |
| kafka.topics.bypass.update.key | <secret-value> |

### Producers

- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/controller/ProducerController.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/producer/IngestProducer.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/producer/IngestProducerConfig.java`

### Consumers

- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/consumer/ConsumerConfigurations.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/consumer/DigressionConsumer.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/consumer/EnrichmentConsumer.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/consumer/IngestConsumer.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/consumer/KafkaConsumer.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/consumer/TransformConsumer.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/consumer/UpdateConsumer.java`
- `business-services/dashboard-ingest/src/main/java/com/ingestpipeline/consumer/ValidatorConsumer.java`
...
- `business-services/dashboard-ingest/src/main/resources/application.properties`
...
| services.esindexer.password | <secret-value> |
| kafka.topics.incoming.data | tarento-dss-collection |
| kafka.topics.incoming.collection | dss-collection-update |
| kafka.topics.enriched.data | egov-dss-ingest-enriched |
