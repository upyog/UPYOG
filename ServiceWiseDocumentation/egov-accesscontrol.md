...
> Generated from repository path `core-services/egov-accesscontrol`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Role-action access-control and authorization metadata service.

## Responsibilities

- Own the `egov-accesscontrol` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8091**
- Servlet/context path: **/access**
- Detected controllers/API mappings: **12**
- Detected migrations: **13**
- Detected tests: **68** files

## Packages

| Package area | Files | Role |
| --- | --- | --- |
| access | 1 source file(s) | Package area detected from source tree. |
| action | 8 source file(s) | Package area detected from source tree. |
| authorize | 3 source file(s) | Package area detected from source tree. |
| controller | 3 source file(s) | HTTP endpoints and request/response orchestration. |
| criteria | 3 source file(s) | Package area detected from source tree. |
...
| model | 5 source file(s) | Request, response, DTO, and domain models. |
...
| role | 3 source file(s) | Package area detected from source tree. |
...
| validateaction | 5 source file(s) | Package area detected from source tree. |

## Folder Structure

- `core-services/egov-accesscontrol`: service root.
...
- `core-services/egov-accesscontrol/src/main/java/org/egov/access/EgovAccesscontrolApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /v1/actions/_search | ActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/actions/_list | ActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/actions/_get | ActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/actions/mdms/_get | ActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/actions/_validate | ActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/actions/_authorize | ActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/actions/_create | ActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/actions/_update | ActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/role-actions/_create | RoleActionController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/roles/_search | RoleController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/roles/_create | RoleController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/roles/_update | RoleController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as egov_accesscontrol
...
- **Tables detected from migrations:** eg_action, eg_ms_role, eg_roleaction, service
- **Migration files:** 13
...
- `core-services/egov-accesscontrol/src/main/resources/db/migration`
- `core-services/egov-accesscontrol/src/main/resources/db/migration/ddl`
- `core-services/egov-accesscontrol/src/main/resources/db/migration/dev`

### Repository locations

- `core-services/egov-accesscontrol/src/main/java/org/egov/access/persistence/repository/ActionRepository.java`
- `core-services/egov-accesscontrol/src/main/java/org/egov/access/persistence/repository/BaseRepository.java`
- `core-services/egov-accesscontrol/src/main/java/org/egov/access/persistence/repository/MdmsRepository.java`
- `core-services/egov-accesscontrol/src/main/java/org/egov/access/persistence/repository/RoleActionRepository.java`
- `core-services/egov-accesscontrol/src/main/java/org/egov/access/persistence/repository/RoleRepository.java`
