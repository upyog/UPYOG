...
> Generated from repository path `tutorials/backend-developer-guide/btr-services`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Birth registration tutorial/reference service.

## Responsibilities

- Own the `btr-services` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8280**
- Servlet/context path: **/birth-registration**
...
| enrichment | 1 source file(s) | Package area detected from source tree. |
| mapper | 1 source file(s) | DTO/entity conversion. |
| model | 45 source file(s) | Request, response, DTO, and domain models. |
...
- `tutorials/backend-developer-guide/btr-services`: service root.
...
- `tutorials/backend-developer-guide/btr-services/src/main/java/digit/Main.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /birth-services/v1/registration/_create | V1ApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /birth-services/v1/registration/_search | V1ApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /birth-services/v1/registration/_update | V1ApiController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
- **Tables detected from migrations:** eg_bt_address, eg_bt_registration
- **Migration files:** 1
...
- `tutorials/backend-developer-guide/btr-services/src/main/resources/db/migration`
- `tutorials/backend-developer-guide/btr-services/src/main/resources/db/migration/main`

### Repository locations

- `tutorials/backend-developer-guide/btr-services/src/main/java/digit/repository/BirthRegistrationRepository.java`
- `tutorials/backend-developer-guide/btr-services/src/main/java/digit/repository/ServiceRequestRepository.java`
...
- `tutorials/backend-developer-guide/btr-services/src/main/java/digit/producer/Producer.java`

### Consumers

- `tutorials/backend-developer-guide/btr-services/src/main/java/digit/consumer/NotificationConsumer.java`
- `tutorials/backend-developer-guide/btr-services/src/main/java/digit/consumer/PaymentBackUpdateConsumer.java`
...
| --- | --- |
| spring.flyway.url | jdbc:postgresql://localhost:5432/birthdb |
| egov.mdms.host | https://dev.digit.org |
| egov.mdms.search.endpoint | /egov-mdms-service/v1/_search |
| egov.user.host | http://localhost:8081 |
| egov.idgen.host | http://localhost:8012/ |
| egov.workflow.host | http://localhost:8013 |
| egov.btrcalculator.host | http://localhost:2345 |
| egov.btrcalculator.endpoint | /birth-calculator/v1/_calculate |
...
- `tutorials/backend-developer-guide/btr-services/src/main/resources/application.properties`
...
| spring.datasource.url | jdbc:postgresql://localhost:5432/birthdb |
