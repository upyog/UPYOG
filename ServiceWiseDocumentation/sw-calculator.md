...
> Generated from repository path `municipal-services/sw-calculator`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.

## Purpose

Sewerage charge calculator service.

## Responsibilities

- Own the `sw-calculator` business or platform capability within the UPYOG ecosystem.
...
- HTTP port: **8084**
- Servlet/context path: **/sw-calculator**
...
| model | 74 source file(s) | Request, response, DTO, and domain models. |
...
| swcalculation | 1 source file(s) | Package area detected from source tree. |
...
- `municipal-services/sw-calculator`: service root.
...
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/SwServiceCalculationApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /sewerageCalculator/_calculate | SWCalculationController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /sewerageCalculator/_estimate | SWCalculationController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /sewerageCalculator/_updateDemand | SWCalculationController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /sewerageCalculator/_jobscheduler | SWCalculationController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /sewerageCalculator/_getConnectionForDemand | SWCalculationController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /sewerageCalculator/_generateDemand | SWCalculationController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /sewerageCalculator/_applyAdhocTax | SWCalculationController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
sequenceDiagram
    ...
    participant Client
    participant Gateway
    participant S as sw_calculator
...
- **Tables detected from migrations:** eg_sw_bulkbill_audit
- **Migration files:** 3
...
- `municipal-services/sw-calculator/src/main/resources/db/migration`
- `municipal-services/sw-calculator/src/main/resources/db/migration/ddl`

### Repository locations

- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/repository/DemandRepository.java`
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/repository/Repository.java`
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/repository/ServiceRequestRepository.java`
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/repository/SewerageCalculatorDaoImpl.java`
...
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/producer/SWCalculationProducer.java`
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/service/BulkDemandAndBillGenService.java`
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/service/DemandService.java`

### Consumers

- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/consumer/BillingNotificationConsumer.java`
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/consumer/DemandGenerationConsumer.java`
- `municipal-services/sw-calculator/src/main/java/org/egov/swcalculation/consumer/DemandNotificationConsumer.java`
...
- `municipal-services/sw-calculator/src/main/resources/application.properties`
...
- `municipal-services/sw-calculator/src/main/resources/db/Dockerfile`
