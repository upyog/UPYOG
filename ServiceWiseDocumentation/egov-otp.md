...
> Generated from repository path `core-services/egov-otp`. This page documents detected runtime configuration and source-code structure. Validate deployment-specific values against the environment/Helm chart used outside this repository.
...
- Own the `egov-otp` business or platform capability within the UPYOG ecosystem.
...
| --- | --- | --- |
| contract | 5 source file(s) | Package area detected from source tree. |
...
| error | 7 source file(s) | Package area detected from source tree. |
| exception | 6 source file(s) | Custom exceptions and handlers. |
...
- `core-services/egov-otp`: service root.
...
- `core-services/egov-otp/src/main/java/org/egov/OtpApplication.java`

## APIs

| Method | Endpoint | Controller | Input | Output | Authentication | Exceptions |
| --- | --- | --- | --- | --- | --- | --- |
| POST | /v1/_create | OtpController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/_validate | OtpController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
| POST | /v1/_search | OtpController.java | Request body follows service model/Swagger contract; validation is typically Bean Validation plus service validators. | Response follows DIGIT ResponseInfo pattern or service-specific model. | Gateway-authenticated unless listed in gateway open/mixed whitelist or explicitly anonymous. | Controller/service/repository/custom validation exceptions propagate through tracer/global handlers. |
...
- **Tables detected from migrations:** eg_token
- **Migration files:** 2
...
- `core-services/egov-otp/src/main/resources/db/migration`
- `core-services/egov-otp/src/main/resources/db/migration/main`
- `core-services/egov-otp/src/main/resources/db/migration/seed`

### Repository locations

- `core-services/egov-otp/src/main/java/org/egov/persistence/repository/TokenRepository.java`
...
| --- | --- |
| server.port | 8089 |
| server.context-path | /otp |
| server.servlet.context-path | /otp |
