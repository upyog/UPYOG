...
const generateQRCodes = async (
  ...
  for (var i = 0, len = qrcodeMappings.length; i < len; i++) {
    ...
    let urlQR='',qrtext;
    if(key == 'mcollect-challan' || key == 'property-receipt' || key == 'bpa-receipt' || key == 'consolidatedreceipt' || key == 'tradelicense-receipt' || key == 'fsm-receipt' || key == 'misc-receipt' || key == 'ws-onetime-receipt')
    {
      urlQR=egov_host;
