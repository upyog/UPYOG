...
import static org.egov.inbox.util.PGRConstants.*;

@Slf4j
@Service
public class SWACHInboxFilterService {
    ...
    @Value("${egov.searcher.swach.search.path}") 
    private String swachInboxSearcherEndpoint;
    
    @Value("${egov.searcher.swach.search.desc.path}")
    private String swachInboxSearcherDescEndpoint;

    @Value("${egov.searcher.swach.count.path}")
    private String swachInboxSearcherCountEndpoint;
    ...
    public List<String> fetchApplicationNumbersFromSearcher(InboxSearchCriteria criteria, HashMap<String, String> StatusIdNameMap, RequestInfo requestInfo){
        ...
        if(!isSearchResultEmpty){
            ...
            if(moduleSearchCriteria.containsKey(SORT_ORDER_PARAM) && moduleSearchCriteria.get(SORT_ORDER_PARAM).equals(DESC_PARAM)){
                uri.append(searcherHost).append(swachInboxSearcherDescEndpoint);
            }else {
                uri.append(searcherHost).append(swachInboxSearcherEndpoint);
            }
...
//            acknowledgementNumbers = JsonPath.read(result, "$.Licenses.*.applicationnumber"); 
    ...
    public Integer fetchApplicationCountFromSearcher(InboxSearchCriteria criteria, HashMap<String, String> StatusIdNameMap, RequestInfo requestInfo){
        ...
        if(!isSearchResultEmpty){
            ...
            StringBuilder uri = new StringBuilder();
            uri.append(searcherHost).append(swachInboxSearcherCountEndpoint);
