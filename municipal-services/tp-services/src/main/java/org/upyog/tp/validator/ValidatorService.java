package org.upyog.tp.validator;
...
public class ValidatorService {
    ...
    public <T> void validateRequest(T request) {
        ...
        if (!violations.isEmpty()) {
            // Extract field names, removing "treePruningBookingDetail."
            String nullFields = violations.stream()
                    .filter(violation -> violation.getMessage().contains("must not be null"))
                    .map(violation -> violation.getPropertyPath().toString().replace("treePruningBookingDetail.", ""))
