...
import org.egov.ewst.web.contracts.*;
...
import static org.egov.ewst.util.EwasteConstants.*;
...
public class NotificationUtil {
	...
	@Autowired
	public NotificationUtil(ServiceRequestRepository serviceRequestRepository, EwasteConfiguration config,
	...
	public static final String ACTION_STATUS_APPLY = "CREATE";

	public static final String ACTION_STATUS_VERIFY = "VERIFYPRODUCT";

	public static final String ACTION_STATUS_SENDPICKUPALERT = "SENDPICKUPALERT";

	public static final String ACTION_STATUS_REJECT = "REJECT";

	public static final String ACTION_STATUS_COMPLETEREQUEST = "COMPLETEREQUEST";
	/**
	...
	public String getCustomizedMsg(RequestInfo requestInfo, EwasteApplication ewasteApplication,
		...
		case ACTION_STATUS_APPLY:
			...
			message = getMessageWithNumber(ewasteApplication, messageTemplate);
			break;

		case ACTION_STATUS_VERIFY:
			messageTemplate = getMessageTemplate(EwasteConstants.NOTIFICATION_VERIFY, localizationMessage);
			message = getMessageWithNumber(ewasteApplication, messageTemplate);
			break;

		case ACTION_STATUS_SENDPICKUPALERT:
			messageTemplate = getMessageTemplate(EwasteConstants.NOTIFICATION_PICKUPALERT, localizationMessage);
			message = getMessageWithNumberAndPickupdate(ewasteApplication, messageTemplate);
			break;

		case ACTION_STATUS_REJECT:
			messageTemplate = getMessageTemplate(EwasteConstants.NOTIFICATION_REJECT, localizationMessage);
			message = getMessageWithNumber(ewasteApplication, messageTemplate);
			break;
	...
	private String getMessageWithNumber(EwasteApplication ewasteApplication, String message) {
		message = message.replace("{1}", ewasteApplication.getApplicant().getApplicantName());
		message = message.replace("{2}", ewasteApplication.getRequestId());
		return message;
	}
	
	private String getMessageWithNumberAndPickupdate(EwasteApplication ewasteApplication, String message) {
		message = message.replace("{1}", ewasteApplication.getApplicant().getApplicantName());
		message = message.replace("{2}", ewasteApplication.getPickUpDate());
		message = message.replace("{3}", ewasteApplication.getRequestId());
		return message;
	}
	
	private String getMessageWithNumberAndFinalDetails(EwasteApplication ewasteApplication, String message) {
		message = message.replace("{1}", ewasteApplication.getApplicant().getApplicantName());
		message = message.replace("{2}", ewasteApplication.getRequestId());
		message = message.replace("{3}", ewasteApplication.getFinalAmount());
		message = message.replace("{4}", ewasteApplication.getTransactionId());
		return message;
	}
