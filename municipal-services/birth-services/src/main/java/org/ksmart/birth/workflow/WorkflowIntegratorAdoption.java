...
@Service
@Slf4j
public class WorkflowIntegratorAdoption {
	
	 private   final BirthConfiguration bndConfig;
	 private   final RestTemplate restTemplate;
	 
	 @Autowired
	    public WorkflowIntegratorAdoption(RestTemplate restTemplate, BirthConfiguration bndConfig) {
	 ...
	 /**
	     ...
	     *
	     * takes the adoption details request  as
	     * parameter constructs the work-flow request
	     *
	     * and sets the resultant status from wf-response back to birth details object
	     *
	     * @param request the {@link adoptionDetailRequest}
	     */
	    public  void callWorkFlow(AdoptionDetailRequest request) {

	        AdoptionApplication currentFile = request.getAdoptionDetails().get(0);
	        ...
	        for (AdoptionApplication adoption : request.getAdoptionDetails()) {
	            if (businessServiceFromMDMS.equals(BirthDeathConstants.BUSINESS_SERVICE_ADPTN) || !request.getAdoptionDetails()
	                    .get(0).getAction().equalsIgnoreCase(BirthDeathConstants.TRIGGER_NOWORKFLOW)) {

	                JSONObject obj = new JSONObject();
	                List<Map<String, String>> uuidmaps = new LinkedList<>();

	                if (!CollectionUtils.isEmpty(adoption.getAssignee())) {

	                    // Adding assignes to processInstance

	                	adoption.getAssignee().forEach(assignee -> {
	                ...
	                obj.put(BirthDeathConstants.BUSINESSIDKEY, adoption.getApplicationNo());
	                ...
	                obj.put(BirthDeathConstants.ACTIONKEY, adoption.getAction());
	                obj.put(BirthDeathConstants.COMMENTKEY, adoption.getComment());
	                if (!CollectionUtils.isEmpty(adoption.getAssignee())) {
	                    obj.put(BirthDeathConstants.ASSIGNEEKEY, uuidmaps);
	                }
	                obj.put(BirthDeathConstants.DOCUMENTSKEY, adoption.getWfDocuments());
	                array.add(obj);
	            }
	        }
