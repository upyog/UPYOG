...
@Component
public class CorrectionApplicationRowMapper implements ResultSetExtractor<List<CorrectionApplication>>, CorrectionBaseRowMapper, CorrectionParentDetailRowMapper,  CorrectionParentAddressRowMapper {

    @Override
    public List<CorrectionApplication> extractData(ResultSet rs) throws SQLException, DataAccessException { //how to handle null
        List<CorrectionApplication> result = new ArrayList<>();
        while (rs.next()) {
            result.add(CorrectionApplication.builder()
