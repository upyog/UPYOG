...
@Service
@Slf4j
public class WorkflowIntegratorCorrection {
    private   final BirthConfiguration bndConfig;
    private   final RestTemplate restTemplate;

    @Autowired
    public WorkflowIntegratorCorrection(RestTemplate restTemplate, BirthConfiguration bndConfig) {
     ...
     */
    public  void callWorkFlow(CorrectionRequest request) {

        CorrectionApplication currentFile = request.getCorrectionDetails().get(0);
        ...
        for (CorrectionApplication birth : request.getCorrectionDetails()) {
            if (businessServiceFromMDMS.equals(BirthDeathConstants.BUSINESS_SERVICE_BND) || !request.getCorrectionDetails()
