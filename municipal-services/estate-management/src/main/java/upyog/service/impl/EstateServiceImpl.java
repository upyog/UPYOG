...
public class EstateServiceImpl implements EstateService {
    ...
    @Override
    public AllotmentResponse updateAllotment(AllotmentRequest request) {
        if (request.getAllotments() == null || request.getAllotments().isEmpty()) {
            log.error("Allotment update failed: Request does not contain any allotments");
            throw new CustomException(
                    ServiceConstants.EstateConstants.INVALID_REQUEST,
                    "Allotment request must contain at least one allotment"
            );
        ...
        Allotment allotment = request.getAllotments().get(0);
        if (!StringUtils.hasText(allotment.getAllotmentId())) {
            throw new CustomException(
                    "ALLOTMENT_ID_REQUIRED",
                    "Allotment ID is required for update operation"
            );
        }

        log.info("Processing allotment update for asset: {}, allotmentId: {}",
                allotment.getAssetNo(), allotment.getAllotmentId());

        AllotmentSearchCriteria searchCriteria = new AllotmentSearchCriteria();
        searchCriteria.setTenantId(allotment.getTenantId());
        searchCriteria.setAssetNo(allotment.getAssetNo());

        List<Allotment> existingAllotments = estateRepository.searchAllotments(searchCriteria);
        if (existingAllotments == null || existingAllotments.isEmpty()) {
            throw new CustomException(
                    "ALLOTMENT_NOT_FOUND",
                    "No allotment found for asset " + allotment.getAssetNo()
            );
        }

        Allotment existingAllotment = existingAllotments.stream()
                .filter(item -> allotment.getAllotmentId().equals(item.getAllotmentId()))
                .findFirst()
                .orElse(existingAllotments.get(0));

        allotment.setAllotmentId(existingAllotment.getAllotmentId());
        if (!StringUtils.hasText(allotment.getUserUuid())) {
            allotment.setUserUuid(existingAllotment.getUserUuid());
        }

        String userUuid = request.getRequestInfo().getUserInfo().getUuid();
        org.egov.common.contract.models.AuditDetails existingAudit = existingAllotment.getAuditDetails();
        org.egov.common.contract.models.AuditDetails auditDetails =
                org.egov.common.contract.models.AuditDetails.builder()
                        .createdBy(existingAudit != null ? existingAudit.getCreatedBy() : userUuid)
                        .createdTime(existingAudit != null ? existingAudit.getCreatedTime() : EstateUtil.getCurrentTimestamp())
                        .lastModifiedBy(userUuid)
                        .lastModifiedTime(EstateUtil.getCurrentTimestamp())
                        .build();
        allotment.setAuditDetails(auditDetails);

        // Update allotment only — do not create a new billing demand for the same consumer code.
        estateRepository.save(estateConfiguration.getEstateAllotmentUpdateTopic(), request);

        log.info("Allotment updated successfully. ID: {}", allotment.getAllotmentId());
