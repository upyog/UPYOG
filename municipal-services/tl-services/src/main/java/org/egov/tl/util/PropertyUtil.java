...
public class PropertyUtil {
	...
	@Autowired
	public PropertyUtil(ServiceRequestRepository serviceRequestRepository, NotificationUtil util, ObjectMapper mapper,
			TLConfiguration config, TradeUtil tradeUtil) {
		this.util = util;
	 ...
	 */
	public String getPropertySearchMsg(TradeLicense license, String localizationMessages, String channel,
			String propertyId, String source) {
		String messageTemplate = "";
		if (CHANNEL_NAME_EMAIL.equals(channel)) {
			if (TL_BUSINESSSERVICE.equals(source)) {
				messageTemplate = util.getMessageTemplate(NOTIFICATION_PROPERTY_CREATED + ".email",
						localizationMessages);
				if (license.getTradeLicenseDetail().getOwners().get(0).getGender().equals(GENDER_MALE)) {
					messageTemplate = messageTemplate.replace(NOTIF_OWNER_NAME_KEY,
							SALUTATION_MR + license.getTradeLicenseDetail().getOwners().get(0).getName());
				} else {
					messageTemplate = messageTemplate.replace(NOTIF_OWNER_NAME_KEY,
							SALUTATION_MS + license.getTradeLicenseDetail().getOwners().get(0).getName());
				}
			} else {
				messageTemplate = util.getMessageTemplate(TLConstants.NOTIFICATION_PROPERTY_TAGGED + ".email",
						localizationMessages);
				messageTemplate = messageTemplate.replace(NOTIF_PROPERTY_OWNER_NAME_KEY,
						license.getTradeLicenseDetail().getOwners().get(0).getName());
			}
		} else {
			if (TL_BUSINESSSERVICE.equals(source)) {
				messageTemplate = util.getMessageTemplate(TLConstants.NOTIFICATION_PROPERTY_CREATED,
						localizationMessages);
				if (license.getTradeLicenseDetail().getOwners().get(0).getGender().equals(GENDER_MALE)) {
					messageTemplate = messageTemplate.replace(NOTIF_OWNER_NAME_KEY,
							SALUTATION_MR + license.getTradeLicenseDetail().getOwners().get(0).getName());
				...
				messageTemplate = util.getMessageTemplate(TLConstants.NOTIFICATION_PROPERTY_TAGGED,
						localizationMessages);
				messageTemplate = messageTemplate.replace(NOTIF_PROPERTY_OWNER_NAME_KEY,
						license.getTradeLicenseDetail().getOwners().get(0).getName());
			}
	...
	public Property getPropertyDetails(TradeLicense license, String propertyId, RequestInfo requestInfo) {
		...
		if (!StringUtils.isBlank(propertyId)) {
			...
			try {
				HashMap<String, Object> result = (HashMap<String, Object>) serviceRequestRepository.fetchResult(
						new StringBuilder(url), RequestInfoWrapper.builder().requestInfo(requestInfo).build());
				if (null != result && null != JsonPath.read(result, PROPERTY_JSON_KEY)) {
					...
					throw new CustomException("INVALID PROPERTY",
							" The propertyId " + license.getPropertyId() + " does not exist");
				}
			} catch (Exception e) {
				throw new CustomException("INVALID PROPERTY",
						" Failed to parse the response from property search on id " + license.getPropertyId());
			}
		}
		return property;
	}
	 ...
	 */
	public Collection<? extends EmailRequest> createPropertyEmailRequest(RequestInfo requestInfo, String message,
			Map<String, String> mapOfPhnoAndEmail, Property property) {
	...
	public EventRequest getEventsForPropertyOwner(Property property, String message, TradeLicenseRequest request) {
		...
		for (String mobile : mobileNumbers) {
			...
			Action action = null;
			events.add(Event.builder().tenantId(request.getLicenses().get(0).getTenantId())
					.description(mobileNumberToMsg.get(mobile)).eventType(TLConstants.USREVENTS_EVENT_TYPE)
					.name(USREVENTS_EVENT_NAME).postedBy(TLConstants.USREVENTS_EVENT_POSTEDBY).source(Source.WEBAPP)
