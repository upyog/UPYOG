...
public class ChallanValidator {
	...
	public void validateFields(ChallanRequest request, Object mdmsData, String type) {
		...
		/*
		 * This valication will be handled at Zuul level. If a employee doesn't have
		 * access to that tenant the create API wont be called
		 * 
		 * if(!challan.getTenantId().equalsIgnoreCase(request.getRequestInfo().
		 * getUserInfo().getTenantId())) errorMap.put("Invalid Tenant",
		 * "Invalid tenant id");
		 */
