package org.egov.tobacco.service.notification;
...
import org.egov.tobacco.repository.ServiceRequestRepository;
import org.egov.tobacco.util.*;
import org.egov.tobacco.web.models.*;
import org.egov.tobacco.web.models.property.Property;
...
import static org.egov.tobacco.util.BPAConstants.APPROVED_STATUS;
import static org.egov.tobacco.util.BPAConstants.BUSINESS_SERVICE_BPAREG;
import static org.egov.tobacco.util.TLConstants.*;
import static org.egov.tobacco.util.TLConstants.PROPERTY_ID;
...
public class TLNotificationService {
	...
	@Autowired
	public TLNotificationService(TLConfiguration config, ServiceRequestRepository serviceRequestRepository,
			NotificationUtil util, BPANotificationUtil bpaNotificationUtil,
			TLRenewalNotificationUtil tlRenewalNotificationUtil, PropertyUtil propertyUtil) {
		this.config = config;
...
//		List<String> configuredChannelNames = Arrays.asList(new String[]{"SMS","EVENT","EMAIL"});
		...
		case businessService_NewTL:
			...
			if(request.getLicenses().get(0).getPropertyId()!=null && !request.getLicenses().get(0).getPropertyId().isEmpty() )
				propertyId = request.getLicenses().get(0).getPropertyId();
				//propertyId = request.getLicenses().get(0).getTradeLicenseDetail().getAdditionalDetail().get(PROPERTY_ID)
						//.asText();
			...
			if (config.getIsTLSMSEnabled()) {
				if (!propertyId.isEmpty() && ACTION_STATUS_INITIATED.equalsIgnoreCase(ACTION_STATUS)) {
					...
					String message = propertyUtil.getPropertySearchMsg(license, localizationMessages, CHANNEL_NAME_SMS,
							propertyId, source);
			...
			if (config.getIsUserEventsNotificationEnabledForTL()) {
				if (!propertyId.isEmpty() && ACTION_STATUS_INITIATED.equalsIgnoreCase(ACTION_STATUS)) {
					String localizationMessages = util.getLocalizationMessages(tenantId, request.getRequestInfo());
					String message = propertyUtil.getPropertySearchMsg(license, localizationMessages,
							CHANNEL_NAME_EVENT, propertyId, source);
					log.info("Message to be sent: ", message);
			...
			if (config.getIsEmailNotificationEnabled()) {
				if (!propertyId.isEmpty() && ACTION_FORWARD_TO_VERIFIER.equalsIgnoreCase(ACTION_STATUS)) {
					...
					Map<String, String> mapOfPhnoAndEmail = util.fetchUserEmailIds(propertyMobileNumbers,
							request.getRequestInfo(), tenantId);
					String message = propertyUtil.getPropertySearchMsg(license, localizationMessages,
							CHANNEL_NAME_EMAIL, String.valueOf(request.getLicenses().get(0).getTradeLicenseDetail()
									.getAdditionalDetail().get(PROPERTY_ID)),
							source);
					emailRequestsPT.addAll(propertyUtil.createPropertyEmailRequest(request.getRequestInfo(), message,
							mapOfPhnoAndEmail, property));
	...
	public void enrichEmailRequest(TradeLicenseRequest request, List<EmailRequest> emailRequests,
			Map<String, String> mapOfPhnoAndEmail, Map<Object, Object> configuredChannelList) {
		String tenantId = request.getLicenses().get(0).getTenantId();
		...
		for (TradeLicense license : request.getLicenses()) {
			...
			if (businessService.equals(businessService_TL)) {
				if (applicationType.equals(APPLICATION_TYPE_RENEWAL)) {
					String localizationMessages = tlRenewalNotificationUtil.getLocalizationMessages(tenantId,
							request.getRequestInfo());
					message = tlRenewalNotificationUtil.getEmailCustomizedMsg(request.getRequestInfo(), license,
			...
			if (businessService.equals(businessService_DIRECT_RENEWAL)
					|| businessService.equals(businessService_EDIT_RENEWAL)) {
				String localizationMessages = tlRenewalNotificationUtil.getLocalizationMessages(tenantId,
						request.getRequestInfo());
				message = tlRenewalNotificationUtil.getEmailCustomizedMsg(request.getRequestInfo(), license,
						localizationMessages);
			...
			if (businessService.equals(businessService_BPA)) {
				...
				if (!CollectionUtils.isEmpty(configuredChannelNames)
						&& configuredChannelNames.contains(CHANNEL_NAME_EMAIL)) {
					String localizationMessages = bpaNotificationUtil.getLocalizationMessages(tenantId,
							request.getRequestInfo());
					message = bpaNotificationUtil.getEmailCustomizedMsg(request.getRequestInfo(), license,
							localizationMessages);
				}
			}
		...
		for (TradeLicense license : request.getLicenses()) {
			...
			if (businessService.equals(businessService_TL)) {
				if (applicationType.equals(APPLICATION_TYPE_RENEWAL)) {
					String localizationMessages = tlRenewalNotificationUtil.getLocalizationMessages(tenantId,
							request.getRequestInfo());
					message = tlRenewalNotificationUtil.getCustomizedMsg(request.getRequestInfo(), license,
							localizationMessages);
				} else {
					String localizationMessages = util.getLocalizationMessages(tenantId, request.getRequestInfo());
			...
			if (businessService.equals(businessService_BPA)) {
				...
				List<String> configuredChannelNames = (List<String>) configuredChannelList.get(action);
				if (!CollectionUtils.isEmpty(configuredChannelNames)
						&& configuredChannelNames.contains(CHANNEL_NAME_SMS)) {
					String localizationMessages = bpaNotificationUtil.getLocalizationMessages(tenantId,
							request.getRequestInfo());
					message = bpaNotificationUtil.getCustomizedMsg(request.getRequestInfo(), license,
							localizationMessages);
				}
			}
			if (businessService.equals(businessService_DIRECT_RENEWAL)
	...
	private EventRequest getEventsForTL(TradeLicenseRequest request) {
		...
		for (TradeLicense license : request.getLicenses()) {
			...
			Map<String, String> mapOfPhnoAndUUIDs = util.fetchUserUUIDs(mobileNumbers, request.getRequestInfo(),
			...
			for (String mobile : mobileNumbers) {
				...
				if (viewTriggerList.contains(license.getStatus())) {
					...
					String actionLink = config.getViewApplicationLink().replace("$mobile", mobile)
							...
							.replace("$tenantId", license.getTenantId());
					actionLink = config.getUiAppHost() + actionLink;
					ActionItem item = ActionItem.builder().actionUrl(actionLink).code(config.getViewApplicationCode())
							.build();
					items.add(item);
					action = Action.builder().actionUrls(items).build();
	...
	public EventRequest getEventsForBPA(TradeLicenseRequest request, boolean isStatusPaid, String paidMessage) {
		Map<Object, Object> configuredChannelList = fetchChannelList(new RequestInfo(),
				request.getLicenses().get(0).getTenantId(), "BPAREG", request.getLicenses().get(0).getAction());
		List<Event> events = new ArrayList<>();
		String tenantId = request.getLicenses().get(0).getTenantId();
		for (TradeLicense license : request.getLicenses()) {
			...
			if (!CollectionUtils.isEmpty(configuredChannelNames)
					&& configuredChannelNames.contains(CHANNEL_NAME_EVENT)) {
				String message = null;
				if (isStatusPaid) {
					message = paidMessage;
				} else {
					String localizationMessages = bpaNotificationUtil.getLocalizationMessages(tenantId,
							request.getRequestInfo());
					message = bpaNotificationUtil.getEventCustomizedMsg(request.getRequestInfo(), license,
							localizationMessages);
				}
				if (message == null)
					continue;
				...
				for (String mobile : mobileNumbers) {
					...
					if (license.getStatus().equals(APPROVED_STATUS)) {
						...
						ActionItem item = ActionItem.builder().actionUrl(actionLink).code(config.getPortalUrlCode())
								.build();
						items.add(item);
						action = Action.builder().actionUrls(items).build();
					}

					events.add(Event.builder().tenantId(license.getTenantId())
							.description(mobileNumberToMsg.get(mobile)).eventType(BPAConstants.USREVENTS_EVENT_TYPE)
							.name(BPAConstants.USREVENTS_EVENT_NAME).postedBy(BPAConstants.USREVENTS_EVENT_POSTEDBY)
	 ...
	 */
	public Map<Object, Object> fetchChannelList(RequestInfo requestInfo, String tenantId, String moduleName,
			String action) {
		...
		Filter masterDataFilter = filter(where(MODULENAME).is(moduleName));
