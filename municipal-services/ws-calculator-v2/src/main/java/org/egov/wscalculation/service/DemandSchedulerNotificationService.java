...
public class DemandSchedulerNotificationService {
	...
	public void sendStartEmail(String tenantId, Long from, Long to, int totalCount, RequestInfo requestInfo) {
		if (configs.getIsEmailEnabled() == null || !configs.getIsEmailEnabled() || StringUtils.isEmpty(configs.getMailRecipients())) return;
		...
		String template = notificationUtil.getMessageTemplate("WS_DEMAND_GEN_START_EMAIL_TEMPLATE", localizationMessage);
		if (StringUtils.isEmpty(template)) {
			log.warn("⚠️ WS_DEMAND_GEN_START_EMAIL_TEMPLATE not found in localization service. Skipping start email.");
			return;
		}
		...
		Email email = Email.builder()
				...
				.subject("Water Demand Generation Started - " + tenantId)
				.body(htmlBody)
				.isHTML(true)
				.build();
		wsCalculationProducer.push(configs.getEmailNotifyTopic(), EmailRequest.builder().email(email).build());
	...
	public void sendCompletionEmail(String tenantId, Long from, Long to, List<String> allConnectionNos, long startTime, RequestInfo requestInfo) {
		...
		while (System.currentTimeMillis() - pollStartTime < 15 * 60 * 1000) { // 15 mins timeout
			try {
				...
				if (successCount >= totalCount) {
					log.info("🎯 All Water connections processed successfully for tenant: {}", tenantId);
					break;
				}
			} catch (Exception e) {
		...
		String template = notificationUtil.getMessageTemplate("WS_DEMAND_GEN_COMPLETION_EMAIL_TEMPLATE", localizationMessage);
		if (StringUtils.isEmpty(template)) {
			log.warn("⚠️ WS_DEMAND_GEN_COMPLETION_EMAIL_TEMPLATE not found in localization service. Skipping completion email.");
			return;
		}
		...
		Email email = Email.builder()
				.emailTo(Arrays.stream(configs.getMailRecipients().split(",")).map(String::trim).collect(Collectors.toSet()))
				.subject("Water Demand Generation Completed - " + tenantId)
				.body(htmlBody)
				.isHTML(true)
				.build();
		wsCalculationProducer.push(configs.getEmailNotifyTopic(), EmailRequest.builder().email(email).build());
	...
	private Map<String, String> getFailureReasons(String tenantId, Long from, Long to) {
		...
		// Try to query eg_ws_demand_generation_error if it exists
