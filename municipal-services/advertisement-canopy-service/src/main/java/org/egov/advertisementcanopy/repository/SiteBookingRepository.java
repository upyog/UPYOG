...
public class SiteBookingRepository {
    ...
    private StringBuilder addWhereClause(StringBuilder searchQuery, List preparedStatementValues,
		...
		if (!siteBookingSearchCriteria.getSkipValidation()) {
		...
		if (!CollectionUtils.isEmpty(siteBookingSearchCriteria.getSiteStatus())) {
			isAppendAndClause = addAndClauseIfRequired(isAppendAndClause, searchQuery);
			searchQuery.append(" site.status IN ( ")
					.append(getQueryForCollection(siteBookingSearchCriteria.getSiteStatus(), preparedStatementValues))
					.append(" )");
		}
