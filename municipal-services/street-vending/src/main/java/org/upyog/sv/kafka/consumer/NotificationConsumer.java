...
public class NotificationConsumer {
	...
	public void listen(final HashMap<String, Object> record, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic) {
		...
		/*
		 * StreetVendingRequest vendingRequest = new StreetVendingRequest(); try {
		 * 
		 * log.info("Consuming record in CHB for notification: " + record.toString() +
		 * " from topic: " + topic); log.info("Strigifed json : " +
		 * StreetVendingUtil.beuatifyJson(record)); vendingRequest =
		 * mapper.convertValue(record, StreetVendingRequest.class); } catch (final
		 * Exception e) { log.error("Error while processing CHB notification to value: "
		 * + record + " on topic: " + topic + ": " + e); }
		 * 
		 * String bookingStatus =
		 * vendingRequest.getStreetVendingDetail().getApplicationStatus();
