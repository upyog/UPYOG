...
public class MdmsUtil {
	...
	public List<ModuleDetail> getCHBModuleRequest() {
		...
		/*
		 * svMasterDtls
		 * .add(MasterDetail.builder().name(CommunityHallBookingConstants.CHB_PURPOSE).
		 * filter(filterCode).build());
		 * 
		 * svMasterDtls.add(MasterDetail.builder().name(CommunityHallBookingConstants.
		 * CHB_SPECIAL_CATEGORY) .filter(filterCode).build());
		 * svMasterDtls.add(MasterDetail.builder().name(CommunityHallBookingConstants.
		 * CHB_COMMNUITY_HALLS) .filter(filterCode).build()); svMasterDtls.add(
		 * MasterDetail.builder().name(CommunityHallBookingConstants.CHB_HALL_CODES).
		 * filter("$.[?(@.active==true)].HallCode").build()); svMasterDtls.add(
		 * MasterDetail.builder().name(CommunityHallBookingConstants.CHB_DOCUMENTS).
		 * filter(filterCode).build());
		 */
	...
	/*
	 * public List<CalculationTyp> getcalculationType(RequestInfo requestInfo,
	 * String tenantId, String moduleName, CommunityHallBookingDetail bookingDetail)
	 * {
	 * 
	 ...
	 * uri.append(config.getMdmsHost()).append(config.getMdmsPath());
	 * 
	 * MdmsCriteriaReq mdmsCriteriaReq = getMdmsRequestCalculationType(requestInfo,
	 * tenantId, moduleName, bookingDetail.getCommunityHallCode()); MdmsResponse
	 * mdmsResponse = mapper.convertValue(serviceRequestRepository.fetchResult(uri,
	 * mdmsCriteriaReq), MdmsResponse.class);
	 * if(mdmsResponse.getMdmsRes().get(config.getModuleName()) == null) { throw new
	 * CustomException("FEE_NOT_AVAILABLE", "Community Hall Fee not available."); }
	 * JSONArray jsonArray =
	 * mdmsResponse.getMdmsRes().get(config.getModuleName()).get(
	 * getCalculationTypeMasterName(bookingDetail.getCommunityHallCode()));
	 * 
	 * try { calculationTypes = mapper.readValue(jsonArray.toJSONString(),
	 * mapper.getTypeFactory().constructCollectionType(List.class,
	 * CalculationType.class)); } catch (JsonProcessingException e) {
