...
public class BillingslabService {
	...
	public BillingSlabRes createSlabs(BillingSlabReq billingSlabReq) {
		...
		billingSlabReq.getBillingSlab().parallelStream().forEach(slab -> {
			...
			slabs.add(slab);
			BillingSlabReq req = BillingSlabReq.builder().requestInfo(billingSlabReq.getRequestInfo())
					.billingSlab(slabs).build();
	...
	public void enrichSlabsForCreate(BillingSlabReq billingSlabReq) {
		AuditDetails audit = AuditDetails.builder().createdBy(billingSlabReq.getRequestInfo().getUserInfo().getUuid())
				...
				.lastModifiedBy(billingSlabReq.getRequestInfo().getUserInfo().getUuid())
	...
	public Map<String, List<String>> getMDMSDataForValidation(BillingSlabReq billingSlabReq) {
		...
		// String[] masters = {BillingslabConstants.TL_MDMS_TRADETYPE,
		// BillingslabConstants.TL_MDMS_ACCESSORIESCATEGORY,
		// BillingslabConstants.TL_MDMS_STRUCTURETYPE,
		// BillingslabConstants.TL_MDMS_UOM};
		String[] masters = { BillingslabConstants.TL_MDMS_ENTERPRISETYPE };
		for (String master : Arrays.asList(masters)) {
			StringBuilder uri = new StringBuilder();
			String module = BillingslabConstants.TL_MDMS_MODULE_NAME;
			// if (master.equals(BillingslabConstants.TL_MDMS_STRUCTURETYPE)
			// || master.equals(BillingslabConstants.TL_MDMS_UOM))
			// module = BillingslabConstants.COMMON_MASTERS_MDMS_MODULE_NAME;
			MdmsCriteriaReq request = util.prepareMDMSSearchReq(uri,
					billingSlabReq.getBillingSlab().get(0).getTenantId(), module, master, null,
					billingSlabReq.getRequestInfo());
			try {
				Object response = restTemplate.postForObject(uri.toString(), request, Map.class);
				if (null != response) {
					String jsonPath = BillingslabConstants.MDMS_JSONPATH_FOR_MASTER_CODES.replaceAll("#module#", module)
