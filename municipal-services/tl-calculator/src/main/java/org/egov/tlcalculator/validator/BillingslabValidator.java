...
public class BillingslabValidator {
	...
	public void validateCreate(BillingSlabReq billingSlabReq) {
		...
		validateType(billingSlabReq, errorMap); // New Validation add for Validate ApplnType, TradeType, StructureType
		...
		billingSlabReq.getBillingSlab().parallelStream()
				.forEach(slab -> validateMDMSCodes(slab, mdmsDataMap, errorMap));
	...
	public void validateUpdate(BillingSlabReq billingSlabReq) {
		...
		dataIntegrityCheck(billingSlabReq, errorMap);
		duplicateCheck(billingSlabReq, errorMap); // Suppose slab s is being updated to s'. If that s' is already
													// available, the update shouldn't be allowed.
		Map<String, List<String>> mdmsDataMap = service.getMDMSDataForValidation(billingSlabReq);
		billingSlabReq.getBillingSlab().parallelStream()
				.forEach(slab -> validateMDMSCodes(slab, mdmsDataMap, errorMap));
		if (!CollectionUtils.isEmpty(errorMap.keySet())) {
	...
	public void tenantIdCheck(BillingSlabReq billingSlabReq, Map<String, String> errorMap) {
		...
		System.out.println(" Tenant size " + tenantIds.size());
		if (tenantIds.size() > 1) {
	...
	public void duplicateCheck(BillingSlabReq billingSlabReq, Map<String, String> errorMap) {
		...
		billingSlabReq.getBillingSlab().parallelStream().forEach(slab -> {
			BillingSlabSearchCriteria criteria = BillingSlabSearchCriteria.builder().tenantId(slab.getTenantId())
					.accessoryCategory(slab.getAccessoryCategory())
					...
					.from(slab.getFromUom()).to(slab.getToUom()).enterpriseType(slab.getEnterpriseType()).build();
		...
		if (!CollectionUtils.isEmpty(duplicateSlabs)) {
			errorMap.put(ErrorConstants.DUPLICATE_SLABS_CODE,
					ErrorConstants.DUPLICATE_SLABS_MSG + ": " + duplicateSlabs);
			throw new CustomException(errorMap);
		}
	}
	...
	public void areRecordsExisiting(BillingSlabReq billingSlabReq, Map<String, String> errorMap) {
		BillingSlabSearchCriteria criteria = BillingSlabSearchCriteria.builder()
				.tenantId(billingSlabReq.getBillingSlab().get(0).getTenantId())
				.ids(billingSlabReq.getBillingSlab().parallelStream().map(BillingSlab::getId)
						.collect(Collectors.toList()))
				.build();
		BillingSlabRes slabRes = service.searchSlabs(criteria, billingSlabReq.getRequestInfo());
		List<String> ids = new ArrayList<>();
		if (billingSlabReq.getBillingSlab().size() != slabRes.getBillingSlab().size()) {
			List<String> responseIds = slabRes.getBillingSlab().parallelStream().map(BillingSlab::getId)
					.collect(Collectors.toList());
	...
	public void validateMDMSCodes(BillingSlab billingSlab, Map<String, List<String>> mdmsDataMap,
		...
		// errorMap.put(ErrorConstants.INVALID_TRADETYPE_CODE,
		// ErrorConstants.INVALID_TRADETYPE_MSG + ": " + billingSlab.getTradeType());
		// }
		// if (!StringUtils.isEmpty(billingSlab.getAccessoryCategory())) {
		// if (!mdmsDataMap.get(BillingslabConstants.TL_MDMS_ACCESSORIESCATEGORY)
		// .contains(billingSlab.getAccessoryCategory()))
		// errorMap.put(ErrorConstants.INVALID_ACCESSORIESCATEGORY_CODE,
		// ErrorConstants.INVALID_ACCESSORIESCATEGORY_MSG + ": " +
		// billingSlab.getAccessoryCategory());
		// }
		// if (!StringUtils.isEmpty(billingSlab.getStructureType())) {
		// if
		// (!mdmsDataMap.get(BillingslabConstants.TL_MDMS_STRUCTURETYPE).contains(billingSlab.getStructureType()))
		// errorMap.put(ErrorConstants.INVALID_STRUCTURETYPE_CODE,
		// ErrorConstants.INVALID_STRUCTURETYPE_MSG + ": " +
		// billingSlab.getStructureType());
		// }
		// if (!StringUtils.isEmpty(billingSlab.getUom())) {
		// if
		// (!mdmsDataMap.get(BillingslabConstants.TL_MDMS_UOM).contains(billingSlab.getUom()))
		// errorMap.put(ErrorConstants.INVALID_UOM_CODE,
		// ErrorConstants.INVALID_UOM_MSG + ": " + billingSlab.getUom());
		// }
		if (!StringUtils.isEmpty(billingSlab.getEnterpriseType())) {
			if (!mdmsDataMap.get(BillingslabConstants.TL_MDMS_ENTERPRISETYPE).contains(billingSlab.getEnterpriseType()))
				errorMap.put(ErrorConstants.INVALID_ENTERPRISETYPE_CODE,
						ErrorConstants.INVALID_ENTERPRISETYPE_MSG + ": " + billingSlab.getEnterpriseType());
		}
	}

	/**
	 * Checks ApplnType, TradeType, StructureType
	 * 
	 * @param billingSlabReq
	 * @param errorMap
	 */
	public void validateType(BillingSlabReq billingSlabReq, Map<String, String> errorMap) {
		for (BillingSlab billingSlab : billingSlabReq.getBillingSlab()) {
			if (!billingSlab.getApplicationType().equals(BillingslabConstants.TL_APPLICATION_TYPE)) {
				errorMap.put(ErrorConstants.INVALID_APPLICATIONTYPE_CODE, ErrorConstants.INVALID_APPLICATIONTYPE_MSG);
				throw new CustomException(errorMap);
			}
			if (!billingSlab.getTradeType().equals(TLCalculatorConstants.TRADE_TYPE)) {
				errorMap.put(ErrorConstants.INVALID_TRADETYPE_CODE,
						ErrorConstants.INVALID_TRADETYPE_MSG + ": " + billingSlab.getTradeType());
				throw new CustomException(errorMap);
			}
			if (!billingSlab.getStructureType().equals(TLCalculatorConstants.STRUCTURE_TYPE)) {
				errorMap.put(ErrorConstants.INVALID_STRUCTURETYPE_CODE,
						ErrorConstants.INVALID_STRUCTURETYPE_MSG + ": " + billingSlab.getStructureType());
				throw new CustomException(errorMap);
			}
		}
	}
