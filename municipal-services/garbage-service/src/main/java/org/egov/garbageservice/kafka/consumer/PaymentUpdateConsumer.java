package org.egov.garbageservice.kafka.consumer;
...
public class PaymentUpdateConsumer {
    ...
    public void listen(final HashMap<String, Object> record, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic) {
        log.info("Garbage Service received payment update from topic: {}", topic);
        try {
            paymentUpdateService.process(record, topic);
        } catch (JsonProcessingException e) {
            log.error("Error processing payment update in Garbage Service", e);
        }
    }
}
