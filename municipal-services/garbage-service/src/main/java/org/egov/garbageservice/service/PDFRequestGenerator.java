...
public class PDFRequestGenerator {
    ...
    @Autowired
    private GarbageServiceConfig applicationPropertiesAndConstant;

    /**
     * Constructs the PDF generation request payload for a garbage tax bill.
     *
     * <p>This method aggregates data from the garbage account, billing history, and bill trackers to
     * populate a comprehensive payload. The process follows these steps:
     * <ol>
     *   <li>Extracts basic account and ULB information.</li>
     *   <li>Iterates through bill trackers to map application IDs to their respective tax amounts.</li>
     *   <li>Builds associative maps for owner names and unit categories, including child accounts.</li>
     *   <li>Compiles line-item lists for bill tables (e.g., serial numbers, bill numbers, arrears, tax amounts, and payment statuses).</li>
     *   <li>Calculates aggregate financial totals (total tax, arrears, paid amounts, due amounts, and rebates).</li>
     *   <li>Formats relevant dates and constructs the final JSON data tree.</li>
     *   <li>Generates a QR code URI for citizen payment and builds the final {@link PDFRequest}.</li>
     * </ol>
     *
     * @param requestInfoWrapper wrapper containing the contextual request information
     * @param grbgAccount        the garbage account associated with the bill
     * @param bill               the list of existing {@link Bill} records
     * @param grbgBillTracker    the list of bill tracking records for historical context
     * @return a populated {@link PDFRequest} ready to be sent to the PDF generation service
     */
    ...
    /**
     * Sanitizes strings by escaping characters with special meaning in HTML.
     *
     * <p>This is crucial for preventing XSS vulnerabilities when generating PDF templates
     * that render input data as HTML.
     *
     * @param input the raw string to be sanitized
     * @return the HTML-escaped string, or {@code null} if the input was null
     */
