package org.ksmart.birth.death.service;
...
import org.ksmart.birth.common.calculation.collections.models.Payment;
import org.ksmart.birth.common.calculation.collections.models.PaymentDetail;
import org.ksmart.birth.common.calculation.collections.models.PaymentResponse;
import org.ksmart.birth.common.calculation.collections.models.PaymentSearchCriteria;
import org.ksmart.birth.common.consumer.ReceiptConsumer;
...
public class DeathService {
	...
	public DeathCertificate download(SearchCriteria criteria, RequestInfo requestInfo) {
		...
		if(deathDtls.get(0).getCounter()>0){
			...
			deathCertificate.setApplicationStatus(DeathCertificate.StatusEnum.ACTIVE);
		}
		else{
	...
	public List<DeathCertAppln> searchApplications(RequestInfoWrapper requestInfoWrapper) {
		...
		for (DeathCertAppln certAppln : certApplns) {
			if (certAppln.getStatus().equalsIgnoreCase(DeathCertificate.StatusEnum.PAID.toString())) {
				try {
			...
			} else if (certAppln.getStatus().equalsIgnoreCase(DeathCertificate.StatusEnum.ACTIVE.toString())) {
	...
	public void updateDownloadStatus(DeathCertRequest certRequest) {
		if(null!=certRequest.getRequestInfo() && null!=certRequest.getRequestInfo().getUserInfo() && null!=certRequest.getRequestInfo().getUserInfo().getUuid())
		{
			...
			deathCert.setApplicationStatus(DeathCertificate.StatusEnum.PAID_DOWNLOAD);
