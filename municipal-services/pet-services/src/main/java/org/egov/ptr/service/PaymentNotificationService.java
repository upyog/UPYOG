...
//        }
//
//        else if(topic.equalsIgnoreCase(petConfiguration.getPgTopic())){
...
//                if (null == petConfiguration.getIsUserEventsNotificationEnabled() || petConfiguration.getIsUserEventsNotificationEnabled()) {
...
//
//            if(CollectionUtils.isEmpty(properties)){
//                log.error("PROPERTY_NOT_FOUND","Unable to send payment notification to applicantId: "+consumerCode);
//                continue;
//            }
//
//            Property property = properties.get(0);
...
/*    private List<SMSRequest> addOldpropertyIdAbsentSMS(String messagejson,Map<String,String> valMap,List<String> mobileNumbers){
    ...
    private void addUserNumber(String topic,RequestInfo requestInfo,Map<String,String> valMap,List<String> mobileNumbers)
    {
      //  If the requestInfo is of citizen add citizen's MobileNumber
        if((topic.equalsIgnoreCase(petConfiguration.getReceiptTopic())
                || topic.equalsIgnoreCase(petConfiguration.getPgTopic())) && !mobileNumbers.contains(valMap.get("mobileNumber")))
...
//        builder.append(petConfiguration.getReceiptDownloadLink());
...
//    }
    ...
    private String getReceiptLinkForInApp(Map<String,String> valMap,String mobileNumber, String businessService){
        StringBuilder builder = new StringBuilder(petConfiguration.getUiAppHost());
        builder.append(petConfiguration.getUserEventReceiptDownloadLink());
        String link = builder.toString();
        link = link.replace("$consumerCode", valMap.get("propertyId"));
        link = link.replace("$tenantId", valMap.get("tenantId"));
        link = link.replace("$receiptNumber", valMap.get("receiptNumber"));
    ...
    private String getCustomizedOfflinePaymentMessage(String message,Map<String,String> valMap){
        ...
        message = message.replace("{insert inactive citizen application web URL}.",petConfiguration.getNotificationURL());
...
//                {
//                    String actionLink = petConfiguration.getPayLink().replace("$mobile", mobile)
//                            .replace("propertyId", property.getPropertyId())
//                            .replace("$tenantId", property.getTenantId())
//                            .replace("$businessService" , businessService);
//
//                    actionLink = petConfiguration.getUiAppHost() + actionLink;
//
...
//        StringBuilder builder = new StringBuilder(petConfiguration.getUiAppHost());
//        builder.append(petConfiguration.getPayLink());
