...
public class PropertyQueryBuilder {
	...
	public final static String TOTAL_PROPERTY_COUNT_WITH_WARD="select epp.usagecategory,count(epp.propertyid),epp.tenantid,epa.ward_no from eg_pt_property epp join\r\n"
			...
			+ "AND EXTRACT(EPOCH FROM CURRENT_DATE + INTERVAL '1 day' - INTERVAL '1 second') * 1000\r\n"
			+ "group by epp.tenantid ,epa.ward_no ,epp.usagecategory";
	public final static String TOTAL_REBATE_COLLECTION_WITH_WARD="select edv.id,epp.usagecategory,sum(edv2.taxamount) as todayrebategiven,epp.tenantid,epa.ward_no\r\n"
			+ "from egbs_demand_v1 edv join \r\n"
			+ "egbs_demanddetail_v1 edv2 on edv.id =edv2.demandid \r\n"
			+ "join eg_pt_property epp on epp.propertyid =edv.consumercode\r\n"
			+ "join eg_pt_address epa on epp.id =epa.propertyid\r\n"
			+ "join egbs_billdetail_v1 ebv on edv.id =ebv.demandid \r\n"
			+ "join egbs_bill_v1 ebv2 on ebv2.id =ebv.billid \r\n"
			+ "where edv2.taxheadcode in('PT_COMPLEMENTARY_REBATE','PT_MODEOFPAYMENT_REBATE') and \r\n"
			+ "epa.ward_no !='' and edv2.taxamount<0 and ebv2.status ='PAID' \r\n"
			+ "and ebv.paymentperiod IN ('Q4', 'H2', 'YR') \r\n"
			+ "and edv2.createdtime BETWEEN EXTRACT(EPOCH FROM CURRENT_DATE) * 1000\r\n"
			+ "AND EXTRACT(EPOCH FROM CURRENT_DATE + INTERVAL '1 day' - INTERVAL '1 second') * 1000\r\n"
			+ "group by epp.tenantid ,epa.ward_no,epp.usagecategory,edv.id";
	public final static String TOTAL_PENALTY_COLLECTED_WITH_WARD="select epp.usagecategory,sum(edv2.taxamount) as todaypenaltycollection,epp.tenantid,epa.ward_no\r\n"
			+ "from egbs_demand_v1 edv join \r\n"
			+ "egbs_demanddetail_v1 edv2 on edv.id =edv2.demandid \r\n"
			+ "join eg_pt_property epp on epp.propertyid =edv.consumercode \r\n"
			+ "join eg_pt_address epa on epp.id =epa.propertyid \r\n"
			...
			+ "and epa.ward_no !='' and edv2.taxamount>0 and ebv2.status ='PAID' \r\n"
			+ "and ebv.paymentperiod in ('Q4','H2','YR')\r\n"
			+ "and edv2.createdtime BETWEEN EXTRACT(EPOCH FROM CURRENT_DATE) * 1000\r\n"
			+ "AND EXTRACT(EPOCH FROM CURRENT_DATE + INTERVAL '1 day' - INTERVAL '1 second') * 1000\r\n"
			+ "group by epp.tenantid ,epa.ward_no,epp.usagecategory";
