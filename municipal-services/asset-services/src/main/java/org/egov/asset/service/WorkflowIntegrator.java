...
public class WorkflowIntegrator {
	...
	private static final String MODULENAMEVALUE = "Asset";
	...
	@Autowired
	public WorkflowIntegrator(RestTemplate rest, AssetConfiguration config) {
		this.rest = rest;
		this.config = config;
	}
	 ...
	 *
	 * takes the asset request as parameter constructs the work-flow request
	 *
	 * and sets the resultant status from wf-response back to asset object
	 *
	 * @param bpaRequest
	 */
	public void callWorkFlow(AssetRequest assetRequest) {
		String wfTenantId = assetRequest.getAsset().getTenantId();
		JSONArray array = new JSONArray();
		Asset asset = assetRequest.getAsset();
		JSONObject obj = new JSONObject();
		obj.put(BUSINESSIDKEY, asset.getApplicationNo());
		obj.put(TENANTIDKEY, wfTenantId);
		obj.put(BUSINESSSERVICEKEY, asset.getWorkflow().getBusinessService());
		obj.put(MODULENAMEKEY, MODULENAMEVALUE);
		obj.put(ACTIONKEY, asset.getWorkflow().getAction());
		obj.put(COMMENTKEY, asset.getWorkflow().getComment());

		if (!CollectionUtils.isEmpty(asset.getWorkflow().getAssignes())) {
			List<Map<String, String>> uuidmaps = new LinkedList<>();
			asset.getWorkflow().getAssignes().forEach(assignee -> {
		...
		obj.put(DOCUMENTSKEY, asset.getWorkflow().getDocuments());
		array.add(obj);
		JSONObject workFlowRequest = new JSONObject();
		workFlowRequest.put(REQUESTINFOKEY, assetRequest.getRequestInfo());
		...
		try {
			...
			try {
				...
				log.error(AssetErrorConstants.EG_ASSET_WF_ERROR_KEY_NOT_FOUND,
						" Unable to read the json path in error object : " + pnfe.getMessage());
				throw new CustomException(AssetErrorConstants.EG_ASSET_WF_ERROR_KEY_NOT_FOUND,
						" Unable to read the json path in error object : " + pnfe.getMessage());
			}
			throw new CustomException(AssetErrorConstants.EG_WF_ERROR, errros.toString());
		} catch (Exception e) {
			throw new CustomException(AssetErrorConstants.EG_WF_ERROR,
		...
		asset.setStatus(idStatusMap.get(asset.getApplicationNo()));
