...
class SewerageBillScreen extends StatefulWidget {
  const SewerageBillScreen({super.key});

  @override
  State<SewerageBillScreen> createState() => _SewerageBillScreenState();
}

class _SewerageBillScreenState extends State<SewerageBillScreen> {
  ...
  void init() async {
    try {
      ...
      if (isNotNullOrEmpty(_waterController.sewerage?.sewerageConnections)) {
        ...
        sewerageBillInfo = await _paymentController.getPayment(
          token: token!,
      ...
      filteredBills = sewerageBillInfo?.bill?.where((bill) {
        return _waterController.sewerage?.sewerageConnections?.any(
              (connection) =>
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: isNotNullOrEmpty(_waterController.sewerage?.sewerageConnections)
          ? SizedBox(
              height: Get.height,
              width: Get.width,
                              ...
                              : ListView.builder(
                                  ...
                                  itemBuilder: (context, index) {
                                    if (bills.isNotEmpty) {
                                      ...
                                      return _sewerageMyBillsCard(bill);
                                    } else {
                                      return const NoApplicationFoundWidget();
                                    }
                                  },
